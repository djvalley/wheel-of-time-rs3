...
import net.wheeloftime.game.item.ItemsContainerNew;
...
import net.wheeloftime.game.player.HintIcon;
...
import net.wheeloftime.game.player.content.grandExchange.Offer;
import net.wheeloftime.login.WorldInformation;
	...
	public void sendWorldList(int clientChecksum, int[] online) {
		...
		if (ourChecksum != clientChecksum) {
			...
			for (int i = 0; i < Settings.WORLDS_INFORMATION.length; i++) {
				...
				packet.writeVersionedString(Settings.HOST_IP);
			}
