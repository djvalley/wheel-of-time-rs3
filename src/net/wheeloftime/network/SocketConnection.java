...
public class SocketConnection {
	...
	public boolean isDead() {
		return (ioReadThread != null && !ioReadThread.isAlive())
				|| (ioWriteThread != null && !ioWriteThread.isAlive());
	}
	...
	public void connect(final Socket sock) {
		if (socket != null)
			throw new RuntimeException(
					"Use disconnect() to close existing connection.");
				...
				try {
					...
					while (true) {
						...
						if (space > 0) {
							...
							if (count > 0) {
								synchronized (ioReadLock) {
									System.arraycopy(personalBuffer, 0,
											readBuffer, readBufferLength, count);
