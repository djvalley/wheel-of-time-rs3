package net.wheeloftime.api.rewindscript;

import java.util.HashMap;
import java.util.Map;
import java.util.Map.Entry;

public class RewindScriptNode {
	 ...
	 * @return The node
	 */
	public RewindScriptNode nodeFor(String name) {
		if (children.containsKey(name)) {
			Object value = children.get(name);
			if (value.getClass() != this.getClass())
				throw new RewindScriptException("Invalid node " + name);
			return (RewindScriptNode) value;
		}
		return null;
	}
	...
	public String listChildren() {
		...
		for (Entry<String, Object> entry : children.entrySet()) {
			...
			if (entry.getValue() instanceof RewindScriptNode) {
				builder.append(((RewindScriptNode) entry.getValue())
						.listChildren());
			} else {
