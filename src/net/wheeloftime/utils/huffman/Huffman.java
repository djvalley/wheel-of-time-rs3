...
public final class Huffman {
	...
	public static int encodeString(OutputStream stream, String string) {
		try {
			...
			StringUtilities
					.encodeString(strdata, 0, string, 0, string.length());
			stream.writeSmart(strdata.length);
			stream.expand(stream.getOffset() + strdata.length * 2);
			stream.skip(huffmanEncode(stream.getBuffer(), stream.getOffset(),
					strdata, 0, strdata.length));
	...
	public static final String decodeString(int maxlength, InputStream stream) {
		try {
			...
			stream.skip(huffmanDecode(stream.getBuffer(), stream.getOffset(),
					strbuff, 0, strlen));
	...
	private static final int huffmanEncode(byte[] buff, int buffoffset,
			byte[] str, int stroffset, int strlength) {
		int i = 0;
		strlength += stroffset;
	...
	private static final int huffmanDecode(byte[] buff, int buffoffset,
			byte[] strbuff, int strbuffoffset, int strbufflength) {
