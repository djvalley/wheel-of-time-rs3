...
import net.wheeloftime.game.player.content.grandExchange.OfferHistory;
import net.wheeloftime.login.DisplayNames;
import net.wheeloftime.login.Offences;

public class SerializableFilesManager {

	private static final String ACCOUNT_PATH = "./data/accounts/";
	private static final String CLAN_PATH = Settings.HOSTED ? Settings
			.getDropboxLocation() + "/clans/" : "data/clans";
	...
	public synchronized static boolean containsPlayer(String username) {
		return new File(ACCOUNT_PATH + username + ".p").exists();
	}

	public synchronized static Player loadPlayer(String username) {
		try {
			return (Player) loadSerializedFile(new File(ACCOUNT_PATH + username
					+ ".p"));
		} catch (Throwable e) {
