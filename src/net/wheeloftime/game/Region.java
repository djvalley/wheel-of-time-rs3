...
public class Region {
	...
	public void loadRegionMap() {
		...
		byte[] mapSettingsData = Cache.STORE.getIndexes()[5].getFile(archiveId,
				3);
		...
		/*
		 * if (Settings.DEBUG && landContainerData == null && landArchiveId !=
		 * -1 && MapArchiveKeys.getMapKeys(regionId) != null) Logger.log(this,
		 * "Missing xteas for region " + regionId + ".");
		 */
	}
	...
	public void loadMapObjects(byte[] data, int regionX, int regionY,
			byte[][][] mapSettings) {
		...
		while ((incr = landStream.readSmart2()) != 0) {
			...
			while ((incr2 = landStream.readUnsignedSmart()) != 0) {
				...
				if (objectPlane < 0 || objectPlane >= 4 || plane < 0
						|| plane >= 4)
					continue;
				spawnObject(new WorldObject(objectId, type, rotation, localX
						+ regionX * 64, localY + regionY * 64, objectPlane),
	...
	public void handleEntityClipping(String name, Integer[] integers,
			boolean add) {
