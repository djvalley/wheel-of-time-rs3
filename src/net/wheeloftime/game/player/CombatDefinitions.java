...
import net.wheeloftime.game.player.actions.PlayerCombat;
...
import net.wheeloftime.game.player.content.activities.minigames.clanwars.ClanWarRequestController;
import net.wheeloftime.game.player.content.activities.minigames.dominon.DTController;
...
public final class CombatDefinitions implements Serializable {
	...
	public void setAutoCastSpell(int id) {
		...
		player.getPackets().sendGameMessage(mainHandSpell == 0 ? "Auto-cast spell cleared." : "Main-hand spell set to: " + Magic.getSpellName(Magic.getSpellData(id)) + ".");
		refreshAutoCastSpell();
	...
	public void setDefaultAbilityMenu() {
		setMagicAbilityMenu(player.isInLegacyCombatMode() ? 0 : 1);
	}
	...
	public void refreshSpellBook() {
		...
		player.getVarsManager().sendVar(3226, -1); // OVERCHARGE staff option
	...
	public void unlockSheatheButton() {
		player.getPackets().sendIComponentSettings(1477, 50, 1, 1, 4);
	}

	public void unlockMagicAbilities() {
		player.getPackets().sendIComponentSettings(1461, 1, 0, 300, 536870910);
		player.getPackets().sendIComponentSettings(1461, 8, 0, 300, 536870910);
	...
	public void refreshBonuses() {
		...
		for (int i = 0; i < player.getEquipment().getItems().getSize(); i++) {
			...
			if (item == null || (item.getId() >= 22346 && item.getId() <= 22348 && !(player.getControllerManager().getController() instanceof DTController)))
				continue;
			ItemDefinitions defs = item.getDefinitions();
			int type = getType(i);
			...
			if (i != Equipment.SLOT_SHIELD && !(i == Equipment.SLOT_ARROWS && mainHandType == Combat.RANGE_TYPE && player.getEquipment().getItem(Equipment.SLOT_WEAPON).getDefinitions().getCSOpcode(2940) != 0)) {
				...
				int damage = (int) (defs.getDamage(mainHandType) * Utils.random(1.55, 2.00));
				...
				if (i == Equipment.SLOT_WEAPON) {
					...
					if (mainHandType == Combat.RANGE_TYPE && weapon != null && weapon.getDefinitions().getCSOpcode(2940) == 0) {
						...
						if (ammo != null) {
							...
							if (player.getEquipment().hasTwoHandedWeapon())
								abilityDamage *= 1.5;
							else if (offHandType == Combat.RANGE_TYPE && !hasShield) {
						...
						if (player.getEquipment().hasTwoHandedWeapon())
							...
							Item offhand = player.getEquipment().getItem(Equipment.SLOT_SHIELD);
							abilityDamage += offhand == null ? 0 : (mainHandType == Combat.RANGE_TYPE ? offhand.getDefinitions().getRangedLevel() : offhand.getDefinitions().getAttackLevel()) * 96 / 2;
						}
					}
			...
			if (!hasShield && i != Equipment.SLOT_WEAPON && !(i == Equipment.SLOT_ARROWS && offHandType == Combat.RANGE_TYPE && player.getEquipment().getItem(Equipment.SLOT_SHIELD).getDefinitions().getCSOpcode(2940) != 0)) {
				...
				int damage = (int) (defs.getDamage(offHandType) * Utils.random(1.55, 2.00));
			...
			if (armor > 0) {
				...
				if (type != Combat.ALL_TYPE && (i == Equipment.SLOT_CHEST || i == Equipment.SLOT_LEGS) && player.getEquipment().getWeaponId() != -1 && type != mainHandType) {
					if (mainHandType == Combat.MELEE_TYPE)
						stats[MELEE_ACCURACY_PENALTY] += armor * (mainHandType == Combat.RANGE_TYPE ? 1.5 : 0.8);
					else if (mainHandType == Combat.RANGE_TYPE)
						stats[RANGE_ACCURACY_PENALTY] += armor * (mainHandType == Combat.MAGIC_TYPE ? 1.5 : 0.8);
					else if (mainHandType == Combat.MAGIC_TYPE)
						stats[MAGE_ACCURACY_PENALTY] += armor * (mainHandType == Combat.MELEE_TYPE ? 1.5 : 0.8);
				}
	...
	public void resetSpecialAttack() {
		...
		specialAttackPercentage = (byte) (player.getCombatDefinitions().getCombatMode() == CombatDefinitions.LEGACY_COMBAT_MODE ? 100 : 0);
	...
	public void refreshShowCombatModeIcon() {
		player.getVarsManager().sendVarBit(21686, !player.isInLegacyCombatMode() && showCombatModeIcon ? 1 : 0);
	}
	...
	public void refreshAllowAbilityQueueing() {
		player.getVarsManager().sendVarBit(21684, !player.isInLegacyCombatMode() && allowAbilityQueueing ? 0 : 1);
	}

	public void setCombatMode(int mode) {
		/*
		 * if (mode != LEGACY_COMBAT_MODE) { player.getPackets().sendGameMessage(
		 * "This mode is currently disabled."); mode = LEGACY_COMBAT_MODE; }
		 */
		if (player.isInLegacyCombatMode() && mode != LEGACY_COMBAT_MODE) // someone
			// trying to
			// hack to use
			// another mode
			// while in
	...
	public int getCombatMode() {
		return player.isInLegacyCombatMode() ? LEGACY_COMBAT_MODE : combatMode; // legacy
	}
	...
	private boolean isForceNoSheathe() {
		return player.isCanPvp() || player.getControllerManager().getController() instanceof ClanWarRequestController;
	}
	...
	private void updateTargetBuffs(boolean update) {
		...
		if (targetPlayer != null) {
			...
			if (maxHp != currentTargetMaxHP)
				player.getPackets().sendCSVarInteger(3700, currentTargetMaxHP = currentTarget.getMaxHitpoints());
			int hp = currentTarget.getHitpoints();
			if (hp != currentTargetHP)
				player.getPackets().sendCSVarInteger(3701, currentTargetHP = hp);
		}
		// prayer normal
		update |= player.getVarsManager().sendVarBit(1953, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.STEEL_SKIN) && player.getSkills().getLevel(Skills.PRAYER) < 10 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1954, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.STEEL_SKIN) && player.getSkills().getLevel(Skills.PRAYER) < 28 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1955, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.STEEL_SKIN) && player.getSkills().getLevel(Skills.PRAYER) >= 28 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1956, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.ULTIMATE_STRENGTH) && player.getSkills().getLevel(Skills.PRAYER) < 13 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1957, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.ULTIMATE_STRENGTH) && player.getSkills().getLevel(Skills.PRAYER) < 31 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1958, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.ULTIMATE_STRENGTH) && player.getSkills().getLevel(Skills.PRAYER) >= 31 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1959, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.INCREDIBLE_REFLEXES) && player.getSkills().getLevel(Skills.PRAYER) < 16 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1960, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.INCREDIBLE_REFLEXES) && player.getSkills().getLevel(Skills.PRAYER) < 34 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1961, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.INCREDIBLE_REFLEXES) && player.getSkills().getLevel(Skills.PRAYER) >= 34 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1962, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.ULTIMATE_STRENGTH) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1963, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.INCREDIBLE_REFLEXES) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1964, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.RAPID_HEAL) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1965, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.PROTECT_FROM_MAGIC) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1966, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.PROTECT_FROM_MISSILES) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1967, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.PROTECT_FROM_MELEE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1968, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.RETRIBUTION) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1969, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.REDEMPTION) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1970, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.SMITE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1971, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.EAGLE_EYE) && player.getSkills().getLevel(Skills.PRAYER) < 26 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1972, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.EAGLE_EYE) && player.getSkills().getLevel(Skills.PRAYER) < 44 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1973, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.EAGLE_EYE) && player.getSkills().getLevel(Skills.PRAYER) >= 44 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1974, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.MYSTIC_MIGHT) && player.getSkills().getLevel(Skills.PRAYER) < 27 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1975, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.MYSTIC_MIGHT) && player.getSkills().getLevel(Skills.PRAYER) < 45 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1976, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.MYSTIC_MIGHT) && player.getSkills().getLevel(Skills.PRAYER) >= 45 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1977, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.OVERPOWERING_FORCE) && player.getSkills().getLevel(Skills.PRAYER) < 26 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1978, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.OVERPOWERING_FORCE) && player.getSkills().getLevel(Skills.PRAYER) < 44 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1979, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.OVERPOWERING_FORCE) && player.getSkills().getLevel(Skills.PRAYER) >= 44 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1980, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.OVERCHARGE) && player.getSkills().getLevel(Skills.PRAYER) < 27 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1981, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.OVERCHARGE) && player.getSkills().getLevel(Skills.PRAYER) < 45 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1982, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.OVERCHARGE) && player.getSkills().getLevel(Skills.PRAYER) >= 45 ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1983, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.PROTECT_FROM_SUMMONING) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1984, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.CHIVALRY) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1985, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.PIETY) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1986, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.RAPID_RENEWAL) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1987, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.MYSTIC_MIGHT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1988, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.EAGLE_EYE) ? 1 : 0);
		// prayer curses
		update |= player.getVarsManager().sendVarBit(1989, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.SAP_STRENGTH) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1990, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.SAP_RANGE_STRENGTH) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1991, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.SAP_MAGE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1992, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.SAP_DEFENCE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1993, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.LEECH_STRENGTH) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1994, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.LEECH_RANGE_STRENGTH) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1995, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.LEECH_MAGIC_STRENGTH) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1996, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.ANGUISH) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1997, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.TORMENT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1998, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.PROTECT_ITEM) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1999, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.SAP_WARRIOR) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2000, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.SAP_RANGER) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2001, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.SAP_MAGE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2002, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.SAP_SPIRIT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2003, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.BERSERKER) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2004, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.DEFLECT_SUMMONING) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2005, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.DEFLECT_MAGIC) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2006, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.DEFLECT_MISSILES) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2007, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.DEFLECT_MELEE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2008, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.LEECH_ATTACK) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2009, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.LEECH_RANGED) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2010, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.LEECH_MAGIC) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2011, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.LEECH_DEFENCE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2013, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.LEECH_ENERGY) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2014, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.LEECH_ADRENALINE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2015, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.WRATH) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2016, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.SOUL_SPLIT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2017, targetPlayer != null && targetPlayer.getPrayer().isActive(Prayer.TURMOIL) ? 1 : 0);
		...
		update |= player.getVarsManager().sendVarBit(2024, targetPlayer != null && !targetPlayer.getEffectsManager().hasActiveEffect(EffectType.OVERLOAD) && targetPlayer.getSkills().getLevel(Skills.RANGED) > targetPlayer.getSkills().getLevelForXp(Skills.RANGED) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2025, targetPlayer != null && targetPlayer.getSkills().getLevel(Skills.RANGED) < targetPlayer.getSkills().getLevelForXp(Skills.RANGED) ? 1 : 0);
		...
		update |= player.getVarsManager().sendVarBit(2030, targetPlayer != null && targetPlayer.getPrayer().getPoints() > targetPlayer.getPrayer().max() ? 1 : 0);
		...
		update |= player.getVarsManager().sendVarBit(2041, targetPlayer != null && targetPlayer.getMeleeStrengthRating() > 0 ? 1 : 0);
		...
		update |= player.getVarsManager().sendVarBit(1915, currentTarget.getEffectsManager().hasActiveEffect(EffectType.REGENERATE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1916, currentTarget.getEffectsManager().hasActiveEffect(EffectType.ANTICIPATION) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1917, currentTarget.getEffectsManager().hasActiveEffect(EffectType.FREEDOM) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1918, currentTarget.getEffectsManager().hasActiveEffect(EffectType.PROVOKE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1919, currentTarget.getEffectsManager().hasActiveEffect(EffectType.RESONANCE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1920, currentTarget.getEffectsManager().hasActiveEffect(EffectType.PREPARATION) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1921, currentTarget.getEffectsManager().hasActiveEffect(EffectType.REFLECT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1922, currentTarget.getEffectsManager().hasActiveEffect(EffectType.DEBILITATE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1923, currentTarget.getEffectsManager().hasActiveEffect(EffectType.REVENGE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1924, currentTarget.getEffectsManager().hasActiveEffect(EffectType.BARRICADE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1925, currentTarget.getEffectsManager().hasActiveEffect(EffectType.REJUVINATE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1926, currentTarget.getEffectsManager().hasActiveEffect(EffectType.IMMORTALITY) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1927, currentTarget.getEffectsManager().hasActiveEffect(EffectType.DISMEMBER) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1928, currentTarget.getEffectsManager().hasActiveEffect(EffectType.FURY) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1929, currentTarget.getEffectsManager().hasActiveEffect(EffectType.DESTROY) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1930, currentTarget.getEffectsManager().hasActiveEffect(EffectType.BERSERK) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1931, currentTarget.getEffectsManager().hasActiveEffect(EffectType.FRENZY) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1932, currentTarget.getEffectsManager().hasActiveEffect(EffectType.PULVERISE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1933, currentTarget.getEffectsManager().hasActiveEffect(EffectType.COMBUST) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1934, currentTarget.getEffectsManager().hasActiveEffect(EffectType.DETONATE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1935, currentTarget.getEffectsManager().hasActiveEffect(EffectType.METAMORPHISIS) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1936, currentTarget.getEffectsManager().hasActiveEffect(EffectType.CONFUSE_EFFECT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1937, currentTarget.getEffectsManager().hasActiveEffect(EffectType.WEAKEN_EFFECT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1938, currentTarget.getEffectsManager().hasActiveEffect(EffectType.CURSE_EFFECT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1939, currentTarget.getEffectsManager().hasActiveEffect(EffectType.VULNERABILITY_EFFECT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1940, currentTarget.getEffectsManager().hasActiveEffect(EffectType.ENFEEBLE_EFFECT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1941, currentTarget.getEffectsManager().hasActiveEffect(EffectType.STAGGER_EFFECT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1942, currentTarget.getEffectsManager().hasActiveEffect(EffectType.BINDING_SHOT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1943, currentTarget.getEffectsManager().hasActiveEffect(EffectType.FRAGMENTATION) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1944, currentTarget.getEffectsManager().hasActiveEffect(EffectType.INCENDIARY_SHOT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1945, currentTarget.getEffectsManager().hasActiveEffect(EffectType.UNLOAD) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1946, currentTarget.getEffectsManager().hasActiveEffect(EffectType.DEADSHOT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1947, currentTarget.getEffectsManager().hasActiveEffect(EffectType.ASSAULT) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1948, currentTarget.getEffectsManager().hasActiveEffect(EffectType.FLURRY) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1949, currentTarget.getEffectsManager().hasActiveEffect(EffectType.SNIPE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1950, currentTarget.getEffectsManager().hasActiveEffect(EffectType.RAPID_FIRE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1951, currentTarget.getEffectsManager().hasActiveEffect(EffectType.TELEPORT_BLOCK) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(1952, currentTarget.getEffectsManager().hasActiveEffect(EffectType.INCITE) ? 1 : 0);
		update |= player.getVarsManager().sendVarBit(2036, currentTarget.getEffectsManager().hasActiveEffect(EffectType.ASPHYXIATE) ? 1 : 0);
		if (update)
			player.updateBuffs();
	...
	public int getWeaknessType() {
		if (stats[ARMOR_MELEE_WEAKNESS] > stats[ARMOR_RANGE_WEAKNESS] && stats[ARMOR_MELEE_WEAKNESS] > stats[ARMOR_MAGIC_WEAKNESS])
			return Combat.MELEE_TYPE;
		if (stats[ARMOR_RANGE_WEAKNESS] > stats[ARMOR_MELEE_WEAKNESS] && stats[ARMOR_RANGE_WEAKNESS] > stats[ARMOR_MAGIC_WEAKNESS])
			return Combat.RANGE_TYPE;
		if (stats[ARMOR_MAGIC_WEAKNESS] > stats[ARMOR_RANGE_WEAKNESS] && stats[ARMOR_MAGIC_WEAKNESS] > stats[ARMOR_MELEE_WEAKNESS])
			return Combat.MAGIC_TYPE;
	...
	public int getAbilitiesDamage() {
		...
		if (mainHandType == Combat.MAGIC_TYPE && this.mainHandSpell != 0) {
			...
			if (spell > 0) {
				...
				Item weapon = player.getEquipment().getItem(Equipment.SLOT_WEAPON);
				if (weapon != null) {
					Integer weaponLvl = (Integer) weapon.getDefinitions().clientScriptData.get(750);
					if (weaponLvl == null)
						weaponLvl = 1;
					...
					int spellDamage = (levelCap > weaponLvl ? weaponLvl : levelCap) * 96;
					...
					if (player.getEquipment().hasTwoHandedWeapon()) {
						...
						if (offhand != null && getType(Equipment.SLOT_SHIELD) == Combat.MAGIC_TYPE && !player.getEquipment().hasShield()) {
							...
							if (weaponLvl == null)
								weaponLvl = 1;
							spellDamage = (levelCap > weaponLvl ? weaponLvl : levelCap) * 96;
							if (spellDamage > spellMaxDamage)
								spellDamage = spellMaxDamage;
		...
		return (int) (stats[ABILITY_DAMAGE] + (Combat.getAccuracy(player.getSkills().getLevel(mainHandType == Combat.RANGE_TYPE ? Skills.RANGED : Skills.STRENGTH)) / 2.7));
	}
	...
	public int getHandDamage(boolean offhand) {
		...
		if (mainHandType == Combat.MAGIC_TYPE && this.mainHandSpell != 0) {
			...
			return (int) (getStats()[offhand ? OFFHAND_DAMAGE : MAINHAND_DAMAGE] + (Combat.getAccuracy(player.getSkills().getLevel(Skills.RANGED)) / 2.7));
		}
		// melee
	...
	public int getSkillAccuracy(boolean offhand) {
		...
		return (int) (Combat.getAccuracy(player.getSkills().getLevel(handType == Combat.MAGIC_TYPE ? Skills.MAGIC : handType == Combat.RANGE_TYPE ? Skills.RANGED : Skills.ATTACK)) * 1);
	}

	public int getDefenceArmor() {
		return (int) Combat.getAccuracy(player.getSkills().getLevel(Skills.DEFENCE));
	}
