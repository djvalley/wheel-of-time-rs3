...
import net.wheeloftime.game.npc.others.Ugi;
...
public class TreasureTrailsManager implements Serializable {
    ...
    private int getScrollLevel(int id) {
    ...
    public void setNextClue(int source) {
        ...
        if (cluePhase == lastPhase) {
            ...
            if (cluePhase == 0
                ...
                // spawn combat npc
                boolean inWilderness = player.getControllerManager()
                        .getController() instanceof Wilderness;
    ...
    public boolean useItem(Item item, int slot) {
        ...
        if (level != -1) {
            ...
            if (currentClue.details.type == SIMPLE
                ...
                for (int i = 0; i < currentClue.details.parameters.length; i++)
    ...
    void useEmote(int id) {// returned it all true because it wont stop
        // emote on rs.
        if (currentClue == null)
            return;
        else if (currentClue.details.type != EMOTE)
    ...
    public boolean useNPC(final NPC npc) {
        ...
        /*
         * if (currentClue.details.type == ANAGRAM && currentClue.dificulty >=
         * HARD && !player.containsOneItem(PUZZLES)) {
         * player.getDialogueManager().startDialogue(new Dialogue() {
         *
         * @Override public void start() {
         * sendNPCDialogue(currentClue.details.id, Dialogue.NORMAL,
         * "I have a puzzle for you!"); stage = 0; }
         *
         * @Override public void run(int interfaceId, int componentId, int
         * slotId) { if (stage == 0) { int puzzle_id = currentClue.dificulty ==
         * ELITE ? PUZZLES[3] : PUZZLES[Utils.random(PUZZLES.length - 1)];
         * sendEntityDialogue(IS_ITEM, puzzle_id, -1, "", npc.getName() +
         * " has given you a puzzle box!");
         * player.getInventory().addItem(puzzle_id, 1); stage = 1; } else {
         * end(); } }
         *
         ...
         * });
         *
         * return true; } else if (currentClue.details.type == ANAGRAM &&
        ...
        /*
         * if (currentClue.details.type == ANAGRAM && currentClue.dificulty >=
         * HARD) { player.getDialogueManager().startDialogue(new Dialogue() {
         *
         * @Override public void start() {
         * sendNPCDialogue(currentClue.details.id, Dialogue.NORMAL,
         * "Good job!"); }
    ...
    private void openReward(int level) {
            ...
            public void run() {
                for (Item item : rewards) {
                    ...
                    player.getInventory().addItemDrop(item.getId(),
                            item.getAmount());
                    if ((item.getId() >= 10330 && item.getId() <= 10353) // 3rd
                            // age
                            || (item.getId() >= 19308 && item.getId() <= 19322) // 3rd
                        // age
                        // druidic
                    )
    ...
    private Item[] generateRewards(int level) {
        ...
        for (Item reward : rewards)
            rewards[count++] = new Item(reward.getId(),
                    reward.getAmount() > 1 ? 1 + Utils.random(reward
                            .getAmount()) : 1);
        return rewards;
    }

    private boolean hasSextantItems() {
    ...
    private WorldTile getTile(int degreeY, int minY, int dirY, int degreeX,
                              int minX, int dirX) {
    ...
    private enum ClueDetails {
        SIMPLE_A_1(EASY, SIMPLE, OBJECT, 34585,
                ...
                MEDIUM, COORDINATE, TILE, UNUSED, 0, 13, SOUTH, 14, 0, EAST), COORDINATE_00_4(
