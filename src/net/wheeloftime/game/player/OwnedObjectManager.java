...
public class OwnedObjectManager {
	...
	public static boolean removeObjectWithCoords(Player player,
			WorldObject object) {
		for (Iterator<String> it = player.getOwnedObjectManagerKeys()
				.iterator(); it.hasNext();) {
			final OwnedObjectManager manager = ownedObjects.get(it.next());
	...
	public static String addOwnedObjectManager(Player player,
			WorldObject object, long cycleTime) {
		return addOwnedObjectManager(player, new WorldObject[] { object },
				new long[] { cycleTime });
	}
