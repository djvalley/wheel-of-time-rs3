...
public final class QuestManager implements Serializable {
	...
	public void checkCompleted() {
		...
		if (!completedQuests.contains(Quests.AS_A_FIRST_RESORT)
				&& player.getSkills().hasRequiriments(Skills.HUNTER, 41,
						Skills.FIREMAKING, 51, Skills.WOODCUTTING, 58))
			completeQuest(Quests.AS_A_FIRST_RESORT);
		if (!completedQuests.contains(Quests.RECIPE_FOR_DISASTER)
				&& player.getSkills().hasRequiriments(Skills.COOKING, 70,
						Skills.MAGIC, 59, Skills.THIEVING, 53, Skills.FISHING,
						53, Skills.MINING, 52, Skills.CRAFTING, 50,
						Skills.FIREMAKING, 50, Skills.WOODCUTTING, 50,
						Skills.AGILITY, 48, Skills.RANGED, 40, Skills.HERBLORE,
						45, Skills.FLETCHING, 10, Skills.SLAYER, 10,
						Skills.SMITHING, 40))
	...
	public void sendCompletedQuestsData(Quests quest) {
		...
		case PERIL_OF_ICE_MONTAINS:
			...
			break;
		case NOMADS_REQUIEM:
			player.getVarsManager().sendVarBit(10095, 0);
			break;
		case KINGS_RANSOM:
			player.getVarsManager().sendVarBit(3910, 0);
			break;
		case FAIRY_RING_III:
		...
		case AS_A_FIRST_RESORT:
			...
			break;
		case SONG_FROM_THE_DEPTHS:
			player.getVarsManager().sendVarBit(12689, 75);
			break;
		default:
			break;
		}
	...
	private void sendStageData(Quests quest, int stage) {
		...
		case EASTER_2014:
			...
			break;
		case SONG_FROM_THE_DEPTHS:
			player.getVarsManager().sendVarBit(12689, 5);
			break;
		default:
			break;
		}
