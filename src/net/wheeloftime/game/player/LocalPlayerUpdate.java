...
import net.wheeloftime.game.Colour;
import net.wheeloftime.game.Graphics;
import net.wheeloftime.game.Hit;
import net.wheeloftime.game.HitBar;
...
public final class LocalPlayerUpdate {
	...
	private boolean needsAdd(Player p) {
		return p != null
				...
				&& (player.withinDistance(p, player.hasLargeSceneView() ? 182
						: 14) && /* (player.hasLargeSceneView() || */player
						.getMapRegionsIds().contains(p.getRegionId()))
	...
	private void applyColourMask(Player p, OutputStream data) {
		...
		data.writeByte(colours >> 16 & 0xFF);
	...
	private void applyHitsMask(Player p, OutputStream data) {
		data.writeByte128(p.getNextHits().size());
	...
	private void applyClanMemberMask(Player p, OutputStream data) {
		data.writeByteC((player.getClanManager() != null && player
	...
	private void applyTargetReticuleMask(Player p, OutputStream data) {
		...
		data.write128Byte(reticle.getSettings2Hash());
	}
	...
	private void applyAppearanceMask(Player p, OutputStream data) {
		...
		data.writeBytes128Reverse(renderData);
	}
	...
	private void applyForceMovementMask(Player p, OutputStream data) {
		data.writeByte(p.getNextForceMovement().getToFirstTile().getX()
				- p.getX());
		data.write128Byte(p.getNextForceMovement().getToFirstTile().getY()
				- p.getY());
		...
		data.writeByte(p.getNextForceMovement().getToFirstTile().getPlane()
				- p.getPlane());// Start
		// Height
		data.writeByteC(p.getNextForceMovement().getToSecondTile() == null ? 0
				: p.getNextForceMovement().getToSecondTile().getPlane()
						- p.getPlane());
