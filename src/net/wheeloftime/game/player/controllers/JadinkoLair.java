...
public class JadinkoLair extends Controller { // 10, 13, 50
				...
				if (ticks % 10 == 0) {
					if (playersCount != 0) {
						for (int i = 0; i < Utils.random(Utils
							...
							createJadeRoot(JADE_ROOTS[Utils
									.random(JADE_ROOTS.length)],
									JADE_WORLDTILE[Utils
											.random(JADE_WORLDTILE.length)],
									false);
					}
				}
			} catch (Throwable e) {
	...
	private static void createJadeRoot(int id, WorldTile tile, boolean permanent) {
	...
	public boolean processObjectClick1(final WorldObject object) {
		if (object.getId() == 12284) {
			if (!player.getInventory().containsItemToolBelt(590)
					|| !player.getInventory().containsItem(21350, 1)) {
				...
				public void run() {
					...
					if (ticks == 2) {
						...
						World.spawnObjectTemporary(
								new WorldObject(object.getId() + 1, object
										.getType(), object.getRotation(),
										object), 2000);
					} else if (ticks == 3) {
						player.setNextAnimation(new Animation(16700));
					} else if (ticks == 6) {
						World.spawnObjectTemporary(
								new WorldObject(object.getId() + 2, object
										.getType(), object.getRotation(),
										object), 30000);
	...
	public void sendInterfaces() {
		player.getInterfaceManager().sendMinigameInterface(715);
		player.refreshFavorPoints();
	}
