...
public final class QueenBlackDragonController extends Controller {
			...
			public void run() {
				try {
					...
					base = new WorldTile(regionBase[0] << 3,
							regionBase[1] << 3, 1);
					MapBuilder.copyAllPlanesMap(176, 792, regionBase[0],
							regionBase[1], 8, 8);
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							...
							npc = new QueenBlackDragon(player, base.transform(
									31, 37, 0), base);
							player.setNextAnimation(new Animation(16754, 20));
							player.getPackets().sendGraphics(
									new Graphics(3447), player);
							player.faceEntity(npc);
							player.setNextWorldTile(base.transform(33, 28, 0));
							player.getPackets().sendCSVarInteger(184, 150);
							player.getPackets().sendCSVarInteger(1925, 0);
							player.getPackets().sendCSVarInteger(1924, 0);
							player.setLargeSceneView(true);
							player.getMusicsManager().playMusic(1082); // AWOKEN
							player.getInterfaceManager().sendFadingInterface(
									1285);
	...
	public boolean processObjectClick1(WorldObject object) {
		...
		if (object.getId() == 70790) {
			...
			FadingScreen.fade(player, new Runnable() {
				...
				public void run() {
					...
					player.getPackets().sendCSVarInteger(184, -1);
					npc.finish();
					MapBuilder.copyAllPlanesMap(160, 760, regionBase[0],
							regionBase[1], 8, 8);
					...
					player.getPackets().sendRemoveObject(
							new WorldObject(70840, 10, 0, base.transform(34,
									24, -1)));
					World.removeObject(new WorldObject(70775, 10, 0, base
							.transform(31, 29, -1)));
					player.getPackets().sendRemoveObject(
		...
		if (object.getId() == npc.getActiveArtifact().getId()) {
			player.getMusicsManager().playMusic(1083); // QUEEN BLACK DRAGON
			...
			switch (object.getId()) {
			case 70777:
				player.getPackets().sendCSVarInteger(1924, 2);
				player.getPackets().sendAddObject(
						new WorldObject(70843, 10, 0, base
								.transform(24, 21, -1)));
				break;
			case 70780:
				player.getPackets().sendCSVarInteger(1924, 4);
				player.getPackets().sendAddObject(
						new WorldObject(70845, 10, 0, base
								.transform(24, 21, -1)));
				break;
			case 70783:
				player.getPackets().sendCSVarInteger(1924, 6);
				player.getPackets().sendAddObject(
						new WorldObject(70847, 10, 0, base
								.transform(24, 21, -1)));
				break;
			case 70786:
				player.getPackets().sendCSVarInteger(1924, 8);
				player.getPackets().sendAddObject(
	...
	public void process() {
		...
		if (player.getY() < base.getY() + 28) {
			...
			if (platformStand++ == 3) {
				...
				player.applyHit(new Hit(npc, 1500, HitLook.REGULAR_DAMAGE));
				platformStand = 0;
			}
		} else {
			platformStand = 0;
