...
import net.wheeloftime.game.player.dialogues.impl.Mandrith_Nastroth;
...
public class Wilderness extends Controller {
	...
	public boolean login() {
		...
		if (isAtWild(player)) {
			...
			if (count > 0)
				player.getPackets()
						.sendGameMessage(
								"Any resources you were carrying from the warbands event have been removed.");
		}
		return false;
	}
	...
	public boolean canAttack(Entity target) {
		if (target instanceof Player) {
			...
			if (((Player) target).getControllerManager().getController() instanceof Wilderness) {
				if (!multi
						|| !(((Wilderness) ((Player) target)
								.getControllerManager().getController()).multi)) {
	...
	public boolean canHit(Entity target) {
		...
		if (((Player) target).getControllerManager().getController() instanceof Wilderness) {
			if (!multi
					|| !(((Wilderness) ((Player) target).getControllerManager()
							.getController()).multi)) {
				if ((player.getAttackedBy() != target && player
						.getAttackedByDelay() > Utils.currentTimeMillis())
						|| (target.getAttackedBy() != player && target
	...
	public boolean processMagicTeleport(WorldTile toTile) {
		if (Warbands.warband != null) {
			if (Warbands.warband.isCarryingResources(player)) {
				player.getPackets().sendGameMessage(
						"You cannot teleport whilst carrying resources!");
				return false;
			}
		}
	...
	private static final Map<String, Set<Long>> killNames = new ConcurrentHashMap<>();
	private static final Map<String, Set<Long>> killIPS = new ConcurrentHashMap<>();
	private static final Map<String, Set<Long>> killMACS = new ConcurrentHashMap<>();

	private void dropArtefact(Player killer) {
		String[] keys = new String[] {
				killer.getUsername() + "@" + player.getUsername(),
		...
		Set<Long>[] times = new Set[] { killNames.get(keys[0]),
				killIPS.get(keys[1]), killMACS.get(keys[2]) };
		...
		for (Set<Long> time1 : times) {
			int count = 0;
			for (Iterator<Long> iterator = time1.iterator(); iterator.hasNext();) {
		...
		if (getWildLevel(killer) >= 30)
			rareChance--;
		int artefact = Mandrith_Nastroth.ARTEFACTS[Utils
				.random(Mandrith_Nastroth.ARTEFACTS.length - 3)
				+ (Utils.random(rareChance) != 0 ? 3 : 0)];
		killer.setLastArtefactTime(current);

		World.addGroundItem(new Item(artefact), new WorldTile(player), killer,
				true, 60);
	}

	private static long getRiskedWealth(Player player) {
		Integer[][] slots = GraveStone.getItemSlotsKeptOnDeath(player, true,
				player.hasSkull(), player.getPrayer().isProtectingItem());
		Item[][] items = GraveStone.getItemsKeptOnDeath(player, slots);
		if (items.length <= 1) // risking just 1 item or 0
			return 0;
		long riskedWealth = 0;
		for (Item item : items[1])
			riskedWealth += GrandExchange.getPrice(item.getId())
					* item.getAmount();
		return riskedWealth;
	}
	...
	public void moved() {
		...
		if (!showingSkull && isAtWild && !isAtWildSafe) {
			...
			removeIcon();
			removeController();
		} else if ((player.getX() == 3386 || player.getX() == 3387)
				&& player.getY() == 3615) {
			removeIcon();
			player.setCanPvp(false);
			removeController();
	...
	private void removeIcon() {
		if (showingSkull) {
			showingSkull = false;
			player.setCanPvp(false);
			player.getPackets().sendHideIComponent(745, 1, true);
	...
	public static boolean isAtWild(WorldTile tile) {
		...
		return (isAtBlackKnights(tile) || tile.getX() >= 3011
				&& tile.getX() <= 3132 && tile.getY() >= 10052
				&& tile.getY() <= 10175)
				|| (tile.getX() >= 2940 && tile.getX() <= 3395
						&& tile.getY() >= 3525 && tile.getY() <= 4000)
				|| (tile.getX() >= 3012 && tile.getX() <= 3150
						&& tile.getY() >= 10053 && tile.getY() <= 10172)
				|| (tile.getX() >= 3012 && tile.getX() <= 3059
						&& tile.getY() >= 10301 && tile.getY() <= 10354)
				|| (tile.getX() >= 3012 && tile.getX() <= 3059
						&& tile.getY() >= 10303 && tile.getY() <= 10351);
	}

	private static boolean isAtWildSafe(WorldTile tile) {
		return (tile.getX() >= 2940 && tile.getX() <= 3395
				&& tile.getY() <= 3524 && tile.getY() >= 3523);
	}
	...
	private static int getWildLevel(WorldTile tile) {
		int wildLevel = tile.getY() > 9900 ? ((tile.getY() - 9912) / 8 + 1)
				: ((tile.getY() - 3520) / 8 + 1);
		return wildLevel < 7 ? 7 : wildLevel;
	}

	private static boolean isAtBlackKnights(WorldTile tile) {
		return (tile.getX() >= 2998
				&& tile.getX() <= 3038
				&& tile.getY() >= 3521
				&& tile.getY() <= 3530
				|| (tile.getX() >= 3004 && tile.getX() <= 3026
						&& tile.getY() >= 3531 && tile.getY() <= 3541)
				|| (tile.getX() >= 2996 && tile.getX() <= 3005
						&& tile.getY() >= 3531 && tile.getY() <= 3533)
				|| (tile.getX() >= 3006 && tile.getX() <= 3022
						&& tile.getY() >= 3541 && tile.getY() <= 3544) || (tile
				.getX() >= 3002 && tile.getX() <= 3005 && tile.getY() >= 3534 && tile
				.getY() <= 3539));
	}
