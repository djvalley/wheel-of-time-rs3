...
import net.wheeloftime.game.player.actions.skills.construction.*;
...
import net.wheeloftime.game.player.actions.skills.prayer.Scattering.Ash;
		...
		if (object.getId() == HouseConstants.HObject.EXIT_PORTAL.getId())
			...
			player.getDialogueManager().startDialogue(
					...
					"Hello, having a nice day " + object.getDefinitions().name
							+ "?");
		} else if (object.getId() == HouseConstants.HObject.DUNGEON_ENTRANCE
				.getId()
			...
			if (object.getDefinitions().getOption(1).equals("Climb"))
				...
				house.climbStaircase(
						...
						object.getId() == HouseConstants.HObject.DUNGEON_ENTRANCE
								.getId());
	...
	public boolean handleItemOnObject(WorldObject object, Item item) {
		if (object.getId() == HouseConstants.HObject.CLAY_FIREPLACE.getId()
			...
			if (bone != null || ash != null) {
				...
				if (bone != null)
					player.getActionManager().setAction(
							new BoneOffering(object, bone, house
									.getBurnerCount()));
				else if (ash != null)
					player.getActionManager().setAction(
							new BoneOffering(object, ash, house
									.getBurnerCount()));
				return false;
			}
		}
