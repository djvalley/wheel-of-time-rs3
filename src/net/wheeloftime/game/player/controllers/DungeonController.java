...
import net.wheeloftime.game.npc.dungeonnering.Blink;
import net.wheeloftime.game.npc.dungeonnering.DivineSkinweaver;
import net.wheeloftime.game.npc.dungeonnering.DungeonBoss;
import net.wheeloftime.game.npc.dungeonnering.DungeonFishSpot;
import net.wheeloftime.game.npc.dungeonnering.DungeonSlayerNPC;
import net.wheeloftime.game.npc.dungeonnering.FrozenAdventurer;
import net.wheeloftime.game.npc.dungeonnering.Gravecreeper;
import net.wheeloftime.game.npc.dungeonnering.MastyxTrap;
import net.wheeloftime.game.npc.dungeonnering.NightGazerKhighorahk;
import net.wheeloftime.game.npc.dungeonnering.RuneboundBehemoth;
import net.wheeloftime.game.npc.dungeonnering.Stomp;
import net.wheeloftime.game.npc.dungeonnering.YkLagorThunderous;
import net.wheeloftime.game.npc.familiar.Familiar;
import net.wheeloftime.game.player.Inventory;
...
import net.wheeloftime.game.player.actions.skills.dungeoneering.Door;
import net.wheeloftime.game.player.actions.skills.dungeoneering.DungeonConstants;
import net.wheeloftime.game.player.actions.skills.dungeoneering.DungeonConstants.KeyDoors;
import net.wheeloftime.game.player.actions.skills.dungeoneering.DungeonConstants.SkillDoors;
import net.wheeloftime.game.player.actions.skills.dungeoneering.DungeonManager;
import net.wheeloftime.game.player.actions.skills.dungeoneering.DungeonResourceShop;
import net.wheeloftime.game.player.actions.skills.dungeoneering.DungeonUtils;
import net.wheeloftime.game.player.actions.skills.dungeoneering.Room;
import net.wheeloftime.game.player.actions.skills.dungeoneering.RoomReference;
import net.wheeloftime.game.player.actions.skills.dungeoneering.VisibleRoom;
import net.wheeloftime.game.player.actions.skills.dungeoneering.rooms.PuzzleRoom;
import net.wheeloftime.game.player.actions.skills.dungeoneering.rooms.puzzles.PoltergeistRoom;
import net.wheeloftime.game.player.actions.skills.dungeoneering.skills.DungeoneeringFarming;
import net.wheeloftime.game.player.actions.skills.dungeoneering.skills.DungeoneeringFarming.Harvest;
import net.wheeloftime.game.player.actions.skills.dungeoneering.skills.DungeoneeringFishing;
import net.wheeloftime.game.player.actions.skills.dungeoneering.skills.DungeoneeringMagicCrafting.CraftMageDung;
import net.wheeloftime.game.player.actions.skills.dungeoneering.skills.DungeoneeringMining;
import net.wheeloftime.game.player.actions.skills.dungeoneering.skills.DungeoneeringMining.DungeoneeringRocks;
import net.wheeloftime.game.player.actions.skills.dungeoneering.skills.DungeoneeringRunecrafting.ImbueAction;
import net.wheeloftime.game.player.actions.skills.dungeoneering.skills.DungeoneeringTraps;
import net.wheeloftime.game.player.actions.skills.herblore.WeaponPoison;
import net.wheeloftime.game.player.actions.skills.magic.Magic;
import net.wheeloftime.game.player.actions.skills.mining.MiningBase;
...
import net.wheeloftime.game.player.actions.skills.woodcutting.Woodcutting;
import net.wheeloftime.game.player.actions.skills.woodcutting.Woodcutting.HatchetDefinitions;
import net.wheeloftime.game.player.actions.skills.woodcutting.Woodcutting.TreeDefinitions;
import net.wheeloftime.game.player.dialogues.Dialogue;
import net.wheeloftime.game.tasks.WorldTask;
import net.wheeloftime.game.tasks.WorldTasksManager;
import net.wheeloftime.network.decoders.WorldPacketsDecoder;
...
public class DungeonController extends Controller {
	...
	public void showBar(boolean show, String name) {
		...
		showBar();
		if (show)
			player.getPackets().sendCSVarString(315, name);
	}
	...
	public boolean canMove(int dir) {
		...
		VisibleRoom vr = dungeon.getVisibleRoom(dungeon
				.getCurrentRoomReference(player));
		WorldTile to = new WorldTile(player.getX()
		...
		if (room != null) {
			if (room.getRoom() == DungeonUtils.getBossRoomWithChunk(
					DungeonConstants.FROZEN_FLOORS, 26, 624)) {
				if (!player.isCantWalk()
						&& World.getObjectWithType(new WorldTile(player.getX()
								+ Utils.DIRECTION_DELTA_X[dir], player.getY()
								+ Utils.DIRECTION_DELTA_Y[dir], 0), 22) == null) {
					player.getAppearence().setRenderEmote(1429);
					player.setRun(true);
					player.setCantWalk(true);
				}
				if (player.isCantWalk()) {
					WorldTile nextStep = new WorldTile(player.getX()
							+ Utils.DIRECTION_DELTA_X[dir] * 2, player.getY()
							+ Utils.DIRECTION_DELTA_Y[dir] * 2, 0);
					NPC boss = getNPC(player, 9929);
					boolean collides = boss != null
							&& Utils.colides(nextStep.getX(), nextStep.getY(),
									player.getSize(), boss.getX(), boss.getY(),
									boss.getSize());
					player.resetWalkSteps();
					WorldObject object = World.getObjectWithType(new WorldTile(
							nextStep.getX(), nextStep.getY(), 0), 22);
					if (collides
							|| ((object != null && (object.getId() == 49331 || object
									.getId() == 49333)) || !player
									.addWalkSteps(nextStep.getX(),
											nextStep.getY(), 1))) {
	...
	public boolean sendDeath() {
		player.lock(8);
		player.stopAll();
		player.getPackets().sendMusicEffectOld(418);
		if (player.getInventory().containsItem(
				DungeonConstants.GROUP_GATESTONE, 1)) {
			WorldTile tile = new WorldTile(player);
			dungeon.setGroupGatestone(new WorldTile(player));
			World.addGroundItem(new Item(DungeonConstants.GROUP_GATESTONE),
					tile);
			player.getInventory().deleteItem(DungeonConstants.GROUP_GATESTONE,
					1);
			player.getPackets().sendGameMessage(
					"Your group gatestone drops to the floor as you die.");
		}
		if (player.getInventory().containsItem(DungeonConstants.GATESTONE, 1)) {
			...
			player.getPackets().sendGameMessage(
					"Your gatestone drops to the floor as you die.");
		}
					...
					if (dungeon != null) {
						for (Player p2 : dungeon.getParty().getTeam()) {
							...
							p2.getPackets().sendGameMessage(
									player.getDisplayName() + " has died.");
						}
					}
				} else if (loop == 3) {
					Effect previousOvlEffect = player.getEffectsManager()
							.getEffectForType(EffectType.OVERLOAD);
					Effect previousRenewalEffect = player.getEffectsManager()
							.getEffectForType(EffectType.OVERLOAD);
					player.reset();
					if (previousOvlEffect != null)
						player.getEffectsManager().startEffect(
								new Effect(EffectType.OVERLOAD,
										previousOvlEffect.getCycle()));
					if (previousRenewalEffect != null)
						player.getEffectsManager().startEffect(
								new Effect(EffectType.OVERLOAD,
										previousRenewalEffect.getCycle()));
					player.resetReceivedHits();
					if (dungeon != null
							&& dungeon.getParty().getTeam().contains(player)) {
						...
						dungeon.playMusic(player,
								dungeon.getCurrentRoomReference(startRoom));
						increaseDeaths();
					}
	...
	public boolean processMagicTeleport(WorldTile toTile) {
		return !(dungeon == null
				...
				|| !dungeon.hasStarted() || dungeon.isAtRewardsScreen());
	}
	...
	public boolean canTakeItem(FloorItem item) {
		...
		if (item.getId() == DungeonConstants.GROUP_GATESTONE) {
			...
			if (!item.hasOwner()) {
				...
				player.getPackets().sendGameMessage(
						"This isn't your gatestone!");
				return false;
			}
			setGatestone(null);
			return true;
		} else if (!World.isFloorFree(item.getTile().getPlane(), item.getTile()
				.getX(), item.getTile().getY())) {
	...
	private void openSkillDoor(final SkillDoors s, final WorldObject object,
			final Room room, final int floorType) {
		...
		if (door.getLevel() > player.getSkills().getLevel(s.getSkillId())) {
			player.getPackets().sendGameMessage(
					"You need a " + Skills.SKILL_NAME[s.getSkillId()]
							+ " level of " + door.getLevel()
							+ " to remove this "
		...
		if (s.getSkillId() == Skills.FIREMAKING) {
			...
			PickAxeDefinitions defs = MiningBase.getPickAxeDefinitions(player,
					true);
			if (defs == null) {
				player.getPackets()
						.sendGameMessage(
		...
		player.lock(3);
		if (s.getOpenAnim() != -1)
			player.setNextAnimation(new Animation(openAnim != -1 ? openAnim
					: fail && s.getFailAnim() != -1 ? s.getFailAnim() : s
							.getOpenAnim()));
		if (s.getOpenGfx() != -1 || s.getFailGfx() != -1)
			player.setNextGraphics(new Graphics(fail ? s.getFailGfx() : s
					.getOpenGfx()));
		if (s.getOpenObjectAnim() != -1 && !fail)
			World.sendObjectAnimation(object,
					new Animation(s.getOpenObjectAnim()));
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (!fail) {
					if (room.getDoor(index) == null) // means someone else
						// opeenda t same time
						return;
					player.getSkills().addXp(s.getSkillId(),
							door.getLevel() * 5 + 10);
					...
					if (openId == -1)
						...
						dungeon.setDoor(
								dungeon.getCurrentRoomReference(object), -1,
								openId, object.getRotation());
				} else {
					player.getPackets().sendGameMessage(s.getFailMessage());
					player.applyHit(new Hit(player, door.getLevel() * 4,
					...
					if (s.getFailObjectAnim() != -1)
						World.sendObjectAnimation(object,
								new Animation(s.getFailObjectAnim()));
				}
	...
	public boolean processNPCClick1(final NPC npc) {
		if (dungeon == null || !dungeon.hasStarted()
			...
			return false;
		VisibleRoom vRoom = dungeon.getVisibleRoom(dungeon
				.getCurrentRoomReference(player));
		...
		if (npc.getId() == DungeonConstants.FISH_SPOT_NPC_ID) {
			player.getActionManager().setAction(
					new DungeoneeringFishing((DungeonFishSpot) npc));
			return false;
			...
			player.getDialogueManager().startDialogue("SmugglerD",
					dungeon.getParty().getComplexity());
			return false;
	...
	public boolean processNPCClick2(final NPC npc) {
		if (dungeon == null || !dungeon.hasStarted()
				|| dungeon.isAtRewardsScreen())
			return false;
		VisibleRoom room = dungeon.getVisibleRoom(dungeon
				.getCurrentRoomReference(player));
		if (room == null)
			return false;
		...
		if (npc instanceof Familiar) {
			...
			if (player.getFamiliar() != familiar) {
				player.getPackets()
						.sendGameMessage("That isn't your familiar.");
				return false;
			...
			if (!dungeon.getParty().isLeader(player)) {
				player.getPackets().sendGameMessage(
						"Only your party's leader can mark a target!");
				return false;
			}
			...
			DungeonResourceShop.openResourceShop(player, dungeon.getParty()
					.getComplexity());
			return false;
		}
		return true;
	...
	private static NPC getNPC(Entity entity, int id) {
		List<Integer> npcsIndexes = World.getRegion(entity.getRegionId())
				.getNPCsIndexes();
		if (npcsIndexes != null) {
	...
	public boolean processObjectClick1(final WorldObject object) {
		...
		if (vr == null || !vr.processObjectClick1(player, object))
			return false;
		int floorType = DungeonUtils
				.getFloorType(dungeon.getParty().getFloor());
		...
		if (object.getId() >= 54439 && object.getId() <= 54456
			...
			return !(boss == null || !(boss instanceof Gravecreeper))
					&& ((Gravecreeper) boss).cleanseTomb(player, object);
		}
		if (object.getId() == 49265) {
			NPC boss = getNPC(player, 9725);
			if (boss == null) {
				player.getPackets().sendGameMessage(
						"You don't need to light anymore.");
				return false;
			}
			...
			Integer value = (Integer) player.getTemporaryAttributtes().get(
			...
			if (value != null && value >= 6) {
				...
				if (boss != null) {
					...
					player.getTemporaryAttributtes().put(
							Key.UNHOLY_CURSEBEARER_ROT, 1);
					player.getSkills().restoreSkills();
					boss.heal(boss.getMaxHitpoints() / 10);
				}
			} else
				player.getPackets().sendGameMessage(
						"You can't restore your stats yet.");
			return false;
		} else if (object.getId() >= KeyDoors.getLowestObjectId()
				&& object.getId() <= KeyDoors.getMaxObjectId()) {
			...
			return false;
				...
				|| (object.getId() >= KeyDoors.getLowestDoorId(floorType) && object
						.getId() <= KeyDoors.getMaxDoorId(floorType))
				|| (object.getDefinitions().name.equals("Door") && object
						.getDefinitions().containsOption(0, "Enter")) // theres
		// many
		// ids
		// for
		// challenge
		// doors
			...
			if (object.getId() == DungeonConstants.DUNGEON_BOSS_DOORS[floorType]
					&& player.isUnderCombat()) {
				player.getPackets().sendGameMessage(
						"This door is too complex to unlock while in combat.");
				return false;
			}
			...
			else if (door.getType() == DungeonConstants.KEY_DOOR
					|| door.getType() == DungeonConstants.SKILL_DOOR)
				player.getPackets().sendGameMessage("The door is locked.");
			else if (door.getType() == DungeonConstants.CHALLENGE_DOOR)
				player.getPackets().sendGameMessage(
						((PuzzleRoom) vr).getLockMessage());
			return false;
			...
			if (level > player.getSkills().getLevel(Skills.THIEVING)) {
				player.getPackets()
						.sendGameMessage(
								"You need a "
										+ Skills.SKILL_NAME[Skills.THIEVING]
										+ " level of " + level
										+ " to open this chest.");
				return false;
			}
			...
			player.getInventory().addItemDrop(DungeonConstants.RUSTY_COINS,
					Utils.random((type + 1) * 10000) + 1);
			if (Utils.random(2) == 0)
				player.getInventory()
						.addItemDrop(
								DungeonConstants.CHARMS[Utils
										.random(DungeonConstants.CHARMS.length)],
								Utils.random(5) + 1);
			if (Utils.random(3) == 0)
				player.getInventory().addItemDrop(
						DungeoneeringFarming.getHerbForLevel(level),
						Utils.random(1) + 1);
			if (Utils.random(4) == 0)
				player.getInventory()
						.addItemDrop(DungeonUtils.getArrows(type + 1),
								Utils.random(100) + 1);
			if (Utils.random(5) == 0)
				player.getInventory().addItemDrop(
						DungeonUtils.getRandomWeapon(type + 1), 1);
			player.getSkills().addXp(Skills.THIEVING,
					DungeonConstants.THIEF_CHEST_XP[type]);
			return false;
		} else if (object.getId() == DungeonConstants.THIEF_CHEST_OPEN[floorType]) {
			player.getPackets().sendGameMessage(
					"You already looted this chest.");
			return false;
		} else if (DungeonUtils.isLadder(object.getId(), floorType)) {
			if (voteStage != 0) {
				player.getPackets().sendGameMessage(
						"You have already voted to move on.");
				return false;
			}
			player.getDialogueManager().startDialogue("DungeonClimbLadder",
					this);
			return false;
		} else if (object.getId() == 53977 || object.getId() == 53978
				|| object.getId() == 53979) {
			int type = object.getId() == 53977 ? 0
					: object.getId() == 53979 ? 1 : 2;
			NPC boss = getNPC(player);
			if (boss != null)
				((RuneboundBehemoth) boss).activateArtifact(player, object,
						type);
			return false;
		}
		...
		case "water trough":
			...
			return false;
		case "salve nettles":
			DungeoneeringFarming.initHarvest(player, Harvest.SALVE_NETTLES,
					object);
			return false;
		case "wildercress":
			DungeoneeringFarming.initHarvest(player, Harvest.WILDERCRESS,
					object);
			return false;
		case "blightleaf":
			DungeoneeringFarming
					.initHarvest(player, Harvest.BLIGHTLEAF, object);
			return false;
		case "roseblood":
		...
		case "runeleaf":
			...
			return false;
		case "spiritbloom":
			DungeoneeringFarming.initHarvest(player, Harvest.SPIRITBLOOM,
					object);
			return false;
		case "tangle gum tree":
			player.getActionManager().setAction(
					new Woodcutting(object, TreeDefinitions.TANGLE_GUM_VINE));
			return false;
		case "seeping elm tree":
			player.getActionManager().setAction(
					new Woodcutting(object, TreeDefinitions.SEEPING_ELM_TREE));
			return false;
		case "blood spindle tree":
			player.getActionManager()
					.setAction(
							new Woodcutting(object,
									TreeDefinitions.BLOOD_SPINDLE_TREE));
			return false;
		case "utuku tree":
			player.getActionManager().setAction(
					new Woodcutting(object, TreeDefinitions.UTUKU_TREE));
			return false;
		case "spinebeam tree":
			player.getActionManager().setAction(
					new Woodcutting(object, TreeDefinitions.SPINEBEAM_TREE));
			return false;
		case "bovistrangler tree":
			player.getActionManager()
					.setAction(
							new Woodcutting(object,
									TreeDefinitions.BOVISTRANGLER_TREE));
			return false;
		case "thigat tree":
			player.getActionManager().setAction(
					new Woodcutting(object, TreeDefinitions.THIGAT_TREE));
			return false;
		case "corpsethorn tree":
			player.getActionManager().setAction(
					new Woodcutting(object, TreeDefinitions.CORPESTHORN_TREE));
			return false;
		case "entgallow tree":
			player.getActionManager().setAction(
					new Woodcutting(object, TreeDefinitions.ENTGALLOW_TREE));
			return false;
		case "grave creeper tree":
			player.getActionManager()
					.setAction(
							new Woodcutting(object,
									TreeDefinitions.GRAVE_CREEPER_TREE));
			return false;
		case "novite ore":
			player.getActionManager().setAction(
					new DungeoneeringMining(object,
							DungeoneeringRocks.NOVITE_ORE));
			return false;
		case "bathus ore":
			player.getActionManager().setAction(
					new DungeoneeringMining(object,
							DungeoneeringRocks.BATHUS_ORE));
			return false;
		case "marmaros ore":
			player.getActionManager().setAction(
					new DungeoneeringMining(object,
							DungeoneeringRocks.MARMAROS_ORE));
			return false;
		case "kratonite ore":
			player.getActionManager().setAction(
					new DungeoneeringMining(object,
							DungeoneeringRocks.KRATONIUM_ORE));
			return false;
		case "fractite ore":
			player.getActionManager().setAction(
					new DungeoneeringMining(object,
							DungeoneeringRocks.FRACTITE_ORE));
			return false;
		case "zephyrium ore":
			player.getActionManager().setAction(
					new DungeoneeringMining(object,
							DungeoneeringRocks.ZEPHYRIUM_ORE));
			return false;
		case "argonite ore":
			player.getActionManager().setAction(
					new DungeoneeringMining(object,
							DungeoneeringRocks.AGRONITE_ORE));
			return false;
		case "katagon ore":
			player.getActionManager().setAction(
					new DungeoneeringMining(object,
							DungeoneeringRocks.KATAGON_ORE));
			return false;
		case "gorgonite ore":
			player.getActionManager().setAction(
					new DungeoneeringMining(object,
							DungeoneeringRocks.GORGONITE_ORE));
			return false;
		case "promethium ore":
			player.getActionManager().setAction(
					new DungeoneeringMining(object,
							DungeoneeringRocks.PROMETHIUM_ORE));
			return false;
		case "furnace":
			player.getDialogueManager().startDialogue("SingleSmithingD",
					object, DungeonConstants.SMELTING_BARS);
			return false;
		case "runecrafting altar":
			player.faceObject(object);
			player.getDialogueManager().startDialogue(
					"DungeoneeringRunecraftingD", ImbueAction.AIR_RUNE);
			return false;
		...
		case "sunken pillar":
			...
			if (boss == null) {
				...
				player.getPackets()
						.sendGameMessage(
								"The mechanism will not respond while the other pillar is raised.");
				return false;
			}
			for (Entity t : boss.getPossibleTargets()) {
				if (t.matches(object) || boss.matches(object)) {
					player.getPackets()
							.sendGameMessage(
									"The mechanism cannot be activated while someone is standing there.");
					return false;
				}
			}
	...
	public boolean processObjectClick4(final WorldObject object) {
		if (dungeon == null || !dungeon.hasStarted()
				|| dungeon.isAtRewardsScreen())
			return false;
		if (!dungeon.getVisibleRoom(dungeon.getCurrentRoomReference(player))
				.processObjectClick4(player, object)) {
			return false;
		}
		String name = object.getDefinitions().name.toLowerCase();
		switch (name) {
	...
	public boolean handleItemOnObject(WorldObject object, Item item) {
		...
		case "anvil":
			...
			if (bar != null) {
				player.getDialogueManager().startDialogue("SmithingD", object,
						bar);
			} else {
				player.getPackets().sendGameMessage(
	...
	public boolean canUseItemOnItem(Item itemUsed, Item usedWith) {
		if (dungeon == null || !dungeon.hasStarted()
				|| dungeon.isAtRewardsScreen())
			return false;
		if (WeaponPoison.poison(player, itemUsed, usedWith, true))
			return false;
		return true;
	}
	...
	public void magicTeleported(int type) {
		dungeon.playMusic(player,
				dungeon.getCurrentRoomReference(player.getNextWorldTile()));
		hideBar();
	}
			...
			for (int index = 0; index < DungeonConstants.SLAYER_CREATURES.length; index++) {
				if (npc.getId() == DungeonConstants.SLAYER_CREATURES[index]) {
					...
					if (player.getSkills().getLevel(Skills.SLAYER) < level) {
						player.getPackets().sendGameMessage(
								"You need a Slayer level of " + level
										+ " in order to attack this monster.");
						return false;
					}
				}
			}
	...
	public boolean processButtonClick(int interfaceId, int componentId,
		...
		if (interfaceId == 1465) {
			if (componentId == 51) {
				WorldTile startRoom = dungeon.getHomeTile();
				player.getActionManager()
						.setAction(new HomeTeleport(startRoom));
				return false;
			}
		}// REALLY LOsec. look.
			...
			if (interfaceId == 1465) {
				if (componentId == 44) {
					if (player.getInterfaceManager().containsScreenInterface()
						...
						player.getPackets()
								.sendGameMessage(
										"Please finish what you're doing before opening the dungeon map.");
						return false;
					}
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					if (item.getId() == DungeonConstants.GATESTONE
						...
						if (item.getId() == DungeonConstants.GROUP_GATESTONE)
							...
							World.addGroundItem(item, currentTile, player,
									true, -1, 2, -1);
							player.getPackets()
									.sendGameMessage(
											"You place the gatestone. You can teleport back to it at any time.");
							return false;
						}
						World.addGroundItem(item, currentTile);
						return false;
					}
					CraftMageDung mageDung = CraftMageDung.getBar(item.getId());
					if (item.getDefinitions().containsInventoryOption(0,
							"Craft")
							&& player.getDungManager().isInside()) {
						if (mageDung != null)
							player.getDialogueManager().startDialogue(
									"DungeoneeringMagicCraftingD", item,
									mageDung);
					}
					...
					for (int index = 0; index < DungeoneeringTraps.ITEM_TRAPS.length; index++) {
						if (item.getId() == DungeoneeringTraps.ITEM_TRAPS[index]) {
							DungeoneeringTraps
									.placeTrap(player, dungeon, index);
							return false;
						}
					}
					for (int i = 0; i < PoltergeistRoom.HERBS.length; i++) {
						if (item.getId() == PoltergeistRoom.HERBS[i]) {
							...
							((PoltergeistRoom) room).consecrateHerbs(player,
									item.getId());
							return false;
						}
					}
				...
				// Summoning.handleInfusionOptions(player, packetId,
				// componentId, slotId, slotId2, true);
				// return false;
				...
				if (componentId == 4) {
					...
					if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET)
						...
						quantity = 50;
					DungeonResourceShop.handlePurchaseOptions(player, slotId,
							quantity);
				}
				return false;
				...
				if (componentId == 0) {
					...
					if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET)
						...
						quantity = 50;
					DungeonResourceShop.handleSellOptions(player, slotId,
							slotId2, quantity);
				}
				return false;
			} else if (interfaceId == 1461) { // TODO dung spellbook
				/*
				 * if (componentId == 2)
				 * player.getCombatDefinitions().switchDefensiveCasting(); else
				 * if (componentId == 7)
				 * player.getCombatDefinitions().switchShowCombatSpells(); else
				 * if (componentId == 9)
				 * player.getCombatDefinitions().switchShowTeleportSkillSpells
				 * (); else if (componentId == 11)
				 * player.getCombatDefinitions().
				 * switchShowMiscallaneousSpells(); else if (componentId == 13)
				 * player.getCombatDefinitions().switchShowSkillSpells(); else
				 * if (componentId >= 15 & componentId <= 17)
				 * player.getCombatDefinitions().setSortSpellBook(componentId -
				 * 15);
				 */
				if (componentId == 1) {
					if (slotId == 158)
						...
						if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET) {
							player.getInventory().deleteItem(
									DungeonConstants.GATESTONE, 1);
							player.getInventory().addItem(
									DungeonConstants.GATESTONE, 1);
							player.getInventory().deleteItem(17789, 3);
							player.getSkills().addXp(Skills.MAGIC, 43.5);
							player.setNextAnimation(new Animation(713));
							player.setNextGraphics(new Graphics(113));
							if (gatestone != null) {
								FloorItem item = World.getRegion(
										gatestone.getRegionId()).getGroundItem(
										DungeonConstants.GATESTONE, gatestone,
										player);
								if (item == null)
									return false;
								World.removeGroundItem(player, item, false);
							...
							if (player.getInventory().containsItem(
								...
								player.getDialogueManager().startDialogue(
										new Dialogue() {
											...
											public void start() {
												this.sendDialogue(
														"You already have an active gatestone, ",
						...
						Magic.processDungSpell(player, slotId, slotId2);
				}
				return false;
			}
			return true;
	...
	private void stoneTeleport(boolean group) {
		...
		if (dungeon.isAtBossRoom(player, 26, 626, true)
				|| (dungeon.isAtBossRoom(player, 26, 672, true) && !YkLagorThunderous
						.isBehindPillar(player, dungeon, dungeon
								.getCurrentRoomReference(new WorldTile(player))))) {
			player.getPackets().sendGameMessage("You can't teleport here.");
			return;
		} else if (tile == null) { // Shouldn't happen for group gatestone
			player.getPackets().sendGameMessage(
					"You currently do not have an active gatestone.");
			return;
		}

		if (!group) {
			FloorItem item = World.getRegion(gatestone.getRegionId())
					.getGroundItem(DungeonConstants.GATESTONE, tile, player);
			if (item == null)
				return;
			World.removeGroundItem(player, item);
			player.getInventory().deleteItem(item);
		...
		if (!Magic.checkRunes(player, true, true,
				group ? DungeonConstants.GROUP_GATESTONE_RUNES : new int[] {}))
			return;
		Magic.sendTeleportSpell(player, 13288, 13285, 2516, 2517, group ? 64
				: 32, 0, tile, 3, false, Magic.MAGIC_TELEPORT);
		if (!group) {
	...
	private void portalGroupStoneTeleport() {
		...
		if (tile == null) // cant happen
			return;
		Magic.sendTeleportSpell(player, 14279, 13285, 2518, 2517, 1, 0, tile,
				1, false, Magic.OBJECT_TELEPORT);
	}
