...
import net.wheeloftime.cache.loaders.RenderAnimDefinitions;
...
public class CosmeticsManager implements Serializable {
	...
	// all(invisbile item) by stewie
	private static final int[] FREE_COSMETICS = { 14289, 14538, 5692, 19139,
			19140, 21095, 21096, 21097, 21103, 21102, 19019, 19022, 19025,
			19061, 21088, 21091, 20281, 23186, 24196, 24199, 14993, 21109,
			7013, 14992, 26532, 26590, 26659, 28165, 28168, 28163, 28321,
			28323, 28325, 28444, 28447, 28449, 28787, 28789, 28791, 28793,
			28795, 28797, 24353, 24356, 24359, 28957, 29338, 29158, 28962,
			29699, 29767, 29786, 29791, 29840, 29851, 29929, 29931, 29933,
			29935, 29937, 29939, 29941, 29943, 29945, 29947, 29949, 29951,
			29953, 29955, 29957, 29959, 29961, 29963, 29965, 29967, 29969,
			29971, 29973, 29975, 29977, 24285 };
	...
	public CosmeticsManager() {
		showingAllItems = true; // keepsakes does save
		keepSakeItems = new ArrayList<Item>(20);// try this
	}

	public void init() {
		if (keepSakeItems == null)
			keepSakeItems = new ArrayList<Item>(20);
		refreshVars();
		refreshShowingAllItems();
		player.getPackets().sendItems(675,
				keepSakeItems.toArray(new Item[keepSakeItems.size()]));
	}

	public void setPlayer(Player player) {
		this.player = player;
		if (unlockedCosmetics == null) // temporary
			unlockedCosmetics = new ArrayList<>(1000);
	}

	// save unlocked cosmetics and so on here.

	public void open(CosmeticType type) {
		for (int x = 153; x < 172; x++) {
			for (int x2 = 174; x < 176; x++) {
				player.getPackets().sendIComponentSettings(1311, x, 0, 500, 10);
				player.getPackets()
						.sendIComponentSettings(1311, x2, 0, 500, 10);
			}
		}

		player.getPackets().sendHideIComponent(1311, 448, false);
		player.getPackets().sendIComponentText(1311, 424, "");
		player.getPackets().sendCSVarInteger(2696, 24000);
		...
		for (int i = 0; i < MENUS_COMPONENT_IDS.length; i++) {
			for (int x = 0; x < Utils
					.getInterfaceDefinitionsComponentsSize(1311); x++) {

			}
			if (MENUS_COMPONENT_IDS[i] == 260)
				player.getPackets().sendUnlockIComponentOptionSlots(1311,
						MENUS_COMPONENT_IDS[i], 0, 1000, 0, 1, 2);
			else
				player.getPackets().sendUnlockIComponentOptionSlots(1311,
		...
		sendRenderAnimation();
		player.getPackets().sendIComponentSettings(1311, 173, 0, 500, 14);
	}
	...
	private void sendPreviewOptions(int id, boolean hide) {
		...
		player.getTemporaryAttributtes().remove("Keepsake");
		...
		if (!hide && canBuy)
			player.getPackets().sendIComponentText(1311, 681,
					unlockedCosmetics.contains(id) ? "Apply" : "Buy");
	}
	...
	private void setCosmetic(Item item, boolean preview) {
		...
		sendRenderAnimation();
		if (preview) {
			player.getPackets().sendHideIComponent(1311, 174, true);
			player.getPackets().sendHideIComponent(1311, 194, false);
			player.getPackets().sendIComponentText(1311, 681, "Apply");
			player.getTemporaryAttributtes().put("Keepsake", item);
		}
	}
	...
	private static final int[] MENUS_COMPONENT_IDS = { 153, 154, 155, 156, 158,
			159, 160, 161, 162, 163, 164, 165, 166, 167, 168, 169, 170, 171,
			172, 173, 174, 175 };

	// helm, cape, amulet, weapon, body, shield, shield+1?, legs, legs+1?,
	// gloves, boots, boots +1?, boots +2?
	// , boots +3?, effects, effects, +1, +2, +3, wings, keepsakes, full
	// outfits, clears
	...
	public void refreshVars() {
		for (int id : cosmeticsVars1.keySet()) {
			Integer varBit = cosmeticsVars1.get(id);
			if (varBit == null)
				continue;
			player.getVarsManager().sendVarBit(varBit, 1);
		}
		for (int id : cosmeticsVars2.keySet()) {
			Integer varBit = cosmeticsVars2.get(id);
			if (varBit == null)
				continue;
			player.getVarsManager().sendVarBit(varBit, 1);
		}
	...
	public void handleButtons(int componentId, int slotId, int slotId2,
		...
		else if (componentId == 463) {
			revert();
		} else if (componentId == 454) { // buy / apply
			Integer id = (Integer) player.getTemporaryAttributtes().remove(
					Key.CURRENT_COSMETIC);
			if (id == null) {
				Item item = (Item) player.getTemporaryAttributtes().remove(
						"Keepsake");
				if (item != null) {
					setCosmetic(item, false);
					revert();
				}
				return;
			}
			apply(id);
		} else if (componentId == 675) {// back/ok
			closeReclaimItem();
			if (componentId == 346)
				player.getInterfaceManager().closeMenu();
		} else if (componentId == 667) {// confirm
			player.getPackets().sendHideIComponent(1311, 648, true);// ok
			player.getPackets().sendHideIComponent(1311, 650, true);// confirm
			int index = (int) player.getTemporaryAttributtes().remove(
					"KeepSakeIndex");
			Item item = keepSakeItems.get(index);
			if (item == null)
				return;
			int equipSlot = item.getDefinitions().getEquipSlot();
			Item cosmetic = player.getEquipment().getCosmeticItems()
					.get(equipSlot);
			if (cosmetic != null && cosmetic.getId() == item.getId())
				clearCosmetic(false, equipSlot);
			keepSakeItems.set(index, null);
			player.getPackets().sendItems(675,
					keepSakeItems.toArray(new Item[keepSakeItems.size()]));
			if (!player.getInventory().addItem(item)) {
				player.getBank().addItem(item.getId(), 1, true);
				player.getPackets().sendIComponentText(1311, 653,
						"Success! Your item has been added to your bank.");
				return;
			}
			player.getPackets().sendIComponentText(1311, 653,
					"Success! Your item has been added to your inventory.");
			revert();
		} else {
				...
				if (MENUS_COMPONENT_IDS[menuId] == componentId) {
					...
					if (type == CosmeticType.WARDROBE) {
						player.getPackets().sendGameMessage(
								"" + slotId + " , menuid " + menuId);
						if (menuId == 19) {
							if (index > keepSakeItems.size() - 1)
								return;
							Item item = keepSakeItems.get(index);
							if (item == null)
								return;
						...
						GeneralRequirementMap data = GeneralRequirementMap
								.getMap(dataId);
						System.out.println("dataId=" + dataId + ", MenuId= "
								+ menuId + ", index = " + index);
						if (!(isAutoUnlocked(data.getId()) || hasCosmeticVar(
								data.getId(), data.getIntValue(2546) == 1)))
							player.getPackets().sendGameMessage(
						...
						if (menuId == 2 || menuId == 5) {
							...
							if (menuId != 2
								...
								player.getPackets()
										.sendGameMessage(
												"This cosmetic var is not currently added.");
						...
						GeneralRequirementMap data = getCosmeticData(type,
								listIds[index]);
						player.getPackets().sendGameMessage(
								"menu= " + menuId + " ,type= " + type
										+ " , index= " + index + " , emoteId= "
										+ data.getIntValue(2535));
						...
						if (type == CosmeticType.ANIMATION) {
							...
							// comps: 539
							if (index >= 3 && menuId == 1)
								index += index < 8 ? 1 : index == 8 ? 10 : 12;
							if (index >= 5 && menuId == 3)
								index++;

							data = getCosmeticData(type, listIds[index]);
							if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
								sendPreviewPet(data.getId());
							}
							return;
						}
	...
	private void spawnPet(int dataId) {
		int growth = 2;
		GeneralRequirementMap data = GeneralRequirementMap
				.getMap(ClientScriptMap.getMap(7225).getIntValue(dataId));
		int npcId = (growth == 2 || data.getIntValue(3150) == 0) ? data
				.getIntValue(3152) : growth == 1 ? data.getIntValue(3150)
				: data.getIntValue(3148);
		World.spawnNPC(npcId, new WorldTile(player.getX(), player.getY(),
				player.getPlane()), 0, false);
	}

	private void sendPreviewPet(int dataId) {
		int growth = 2;
		GeneralRequirementMap data = GeneralRequirementMap
				.getMap(ClientScriptMap.getMap(7225).getIntValue(dataId));
		int color = 0;
		int npcId = (growth == 2 || data.getIntValue(3150) == 0) ? data
				.getIntValue(3152) : growth == 1 ? data.getIntValue(3150)
				: data.getIntValue(3148);
		NPCDefinitions ndef = NPCDefinitions.getNPCDefinitions(npcId);
		RenderAnimDefinitions rDef = RenderAnimDefinitions
				.getRenderAnimDefinitions(NPCDefinitions
						.getNPCDefinitions(npcId).renderEmote);
		int emoteId = rDef.walkAnimation;
		int zoom = ndef.size * 100;
		int height1 = 100;
		player.getPackets().sendGameMessage(
				"" + emoteId + " "
						+ NPCDefinitions.getNPCDefinitions(npcId).renderEmote);
		player.getPackets().sendExecuteScript(2716, dataId);
		player.getVarsManager().sendVar(
				3472,
				(Integer) NPCDefinitions.getNPCDefinitions(data
						.getIntValue(3150) == 0 ? data.getIntValue(3152) : data
						.getIntValue(3148)).clientScriptData.get(75));
		player.getPackets().sendExecuteScript(7422, npcId, emoteId, color,
				zoom, height1);
		if (data.getIntValue(3150) == 0)
			player.getPackets().sendExecuteScript(7425,
					"<col=E5B051><shad>" + ndef.getName() + "</col></shad>");
		else {
			String[] name = ndef.getName().split(" ");
			String sufix = growth == 0 ? "Baby" : growth == 1 ? "Adolescent"
					: "Adult";
			player.getPackets()
					.sendExecuteScript(
							7424,
							sufix,
							name.length > 2 ? (name[0] + " " + name[1] + (dataId == 7013 ? (" " + name[2])
									: ""))
									: name[0]);
		}
	}
	...
	public void buy(int id, CosmeticType type) {
		...
		// player.getRights() >= 2 || player.isAMember()) {
		...
		refreshVars();
		this.open(CosmeticType.WARDROBE);// try this
		// player.getInterfaceManager().closeMenu();
		// player.getPackets().sendGameMessage("Bought cosmetic, please reopen
		// the interface again.");
		player.lock(5);
		return;
		// }
		// player.getDialogueManager().startDialogue("SimpleMessage", "You need
		// to be a member to use this feature.");
		// return;
	}
	...
	public void sendPreviewAnimation(int id) {
		if (player.getInterfaceManager().containsInterface(1311))
	...
	// var 3472 npc preview npcscripts 75
	...
	private static void setVars() {
		...
		cosmeticsVars1.put(19028, null); // TODO manual
		cosmeticsVars1.put(19029, null); // TODO manual
		...
		cosmeticsVars1.put(25689, null); // TODO manual
		cosmeticsVars1.put(25690, null); // TODO manual
		cosmeticsVars1.put(25691, null); // TODO manual
		...
		cosmeticsVars1.put(28382, null); // TODO manual
		cosmeticsVars1.put(28383, null); // TODO manual
		cosmeticsVars1.put(28391, null); // TODO manual
	...
	public boolean keepSakeItem(Item itemUsed, Item itemUsedWith) {
		if (itemUsed.getId() != ItemIdentifiers.KEEPSAKE_KEY
				&& itemUsedWith.getId() != ItemIdentifiers.KEEPSAKE_KEY)
			return false;
		if (itemUsed.getId() == ItemIdentifiers.KEEPSAKE_KEY
				&& itemUsedWith.getId() == ItemIdentifiers.KEEPSAKE_KEY)
			return false;
		Item keepSakeKey = itemUsed.getId() == ItemIdentifiers.KEEPSAKE_KEY ? itemUsed
				: itemUsedWith;
		Item keepSakeItem = itemUsed.getId() == ItemIdentifiers.KEEPSAKE_KEY ? itemUsedWith
				: itemUsed;
		if (keepSakeItem == null || keepSakeKey == null)
			return false;
		if (keepSakeItems.size() >= 20) {
			player.getPackets().sendGameMessage(
					"You can only keep sake 20 items.");
			return true;
		}
		int equipSlot = keepSakeItem.getDefinitions().getEquipSlot();
		if (equipSlot == Equipment.SLOT_ARROWS
				|| equipSlot == Equipment.SLOT_AURA
		...
		if (equipSlot == -1) {
			player.getPackets().sendGameMessage(
					"You can't keep sake this item as its not wearable.");
			return true;
		}
		if (!ButtonHandler.canKeepSakeItem(player, keepSakeItem)) {
			player.getPackets()
					.sendGameMessage(
							"You don't have enough requirments to keep sake this item.");
			return true;
		}
		if (keepSakeItem.getDefinitions().isBindItem()
				|| keepSakeItem.getDefinitions().isLended()
				|| keepSakeItem.getDefinitions().isStackable())
			return true;
		String name = keepSakeItem.getName().toLowerCase();
		if (name.contains("broken")) {
			player.getPackets().sendGameMessage(
					"You can't keep sake broken items.");
			return true;
		}
		for (Item item : keepSakeItems) {
			...
			if (item.getId() == keepSakeItem.getId()) {
				player.getPackets().sendGameMessage(
						"You already have that item in your keepsake box.");
				return true;
			}
		}
		player.stopAll();
		player.getDialogueManager().startDialogue(new Dialogue() {
			...
			@Override
			public void start() {
				sendDialogue("Are you sure you wish to place this item in your Dragon Keepsake Box? You"
						+ " will not be able to equip, trade or lend it while it is in storage.");
				stage = 1;
			}
			...
			public void run(int interfaceId, int componentId, int slotId)
				...
				switch (stage) {
				case 1:
					sendDialogue("You can reclaim it in a bank area by visiting the Customisation Menu."
							+ " Once reclaimed, you will no longer be to use it as a custom override."
							+ "Reclaiming a stored item does not give you a Dragon Keepsake key back, "
							+ "and storing another item would require a new key.");
					stage = 2;
					break;
				case 2:
					sendOptionsDialogue("Item to store: "
							+ "<col=116190><shad>" + keepSakeItem.getName()
							+ "</col></shad>",
							"Store this item in my Dragon Keepsake Box.",
							"Quit.");
					stage = 3;
					break;
				case 3:
					...
					case OPTION_1:
						...
						if (index == -1)
							keepSakeItems.add(keepSakeItem);
						else
							keepSakeItems.set(index, keepSakeItem);
						player.getPackets()
								.sendGameMessage(
										"Success! Your item is now stored in your Dragon Keepsake Box. You can now activate it as a custom ovveride in the Customisation Menu.");
						player.getPackets().sendItems(
								675,
								keepSakeItems.toArray(new Item[keepSakeItems
										.size()]));
						player.getInventory().deleteItem(
								ItemIdentifiers.KEEPSAKE_KEY, 1);
	...
	public int getNextIndex() {
		for (int i = 0; i < keepSakeItems.size(); i++) {
			Item keepSaked = keepSakeItems.get(i);
			if (keepSaked == null)
				return i;
		}
		return -1;
	...
	public void sendReclaimItem(int index) {
		player.getTemporaryAttributtes().put("KeepSakeIndex", index);
		player.getPackets().sendHideIComponent(1311, 648, false);
		player.getPackets().sendHideIComponent(1311, 650, false);
		player.getPackets()
				.sendIComponentText(
						1311,
						653,
						"Are you sure you want to reclaim this item? It will be placed in your inventory or bank.<br><br><br>You will not be able to reclaim your Dragon keepsake Key.");
	}

	public void closeReclaimItem() {
		player.getTemporaryAttributtes().remove("KeepSakeIndex");
		player.getPackets().sendHideIComponent(1311, 648, true);
		player.getPackets().sendHideIComponent(1311, 650, true);
	}
