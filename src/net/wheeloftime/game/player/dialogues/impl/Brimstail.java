	...
	public void run(int interfaceId, int componentId, int slotId) {
		if (stage == -1) {
			sendOptionsDialogue(
					...
					"I've been investigating another watcher in the Tree Gnome Village.",
					"I need to mine some rune essence.",
					"I'd like another crystal please.", "Nevermind.");
			stage = 0;
		} else if (stage == 0) {
			...
			sendPlayerDialogue(
					NORMAL,
					"I've been investigating another watcher in the Tree Gnome Village. And, as a consequence, I've found an underground cave complex full of warped terrorbirds and tortoises. It seems to be underneath Arposandra or, at least, a city with gnomes in that use talking terrorbirds as guards.");
			stage = 5;
		} else if (stage == 5) {
			sendNPCDialogue(npcId, NORMAL,
					"Wow! You've really found Arposandra.");
			stage = 6;
		} else if (stage == 6) {
			sendPlayerDialogue(
					NORMAL,
					"I think so, though I can't get in. It's all blocked up. But I'll find a way...eventually.");
			stage = 7;
		} else if (stage == 7) {
			...
			sendEntityDialogue(IS_ITEM, "", 9626, -1,
					"Brimstail hands you a small crystal.");
			...
			sendNPCDialogue(
					npcId,
					NORMAL,
					"Anything for a fellow magician, just dont lose it. I have a very limited supply of them that flows from our local mine.");
			stage = 15;
		} else if (stage == 15) {
			end();
		}
