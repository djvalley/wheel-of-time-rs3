...
public class PiratePete extends Dialogue {
	...
	public void start() {
		...
		sendNPCDialogue(npc.getId(), NORMAL, "Are you ready to travel to "
				+ (isBraindeadIsland ? "Port Phasmatys" : "Braindead Island")
				+ "?");
		stage = -1;
	}
	...
	public void run(int interfaceId, int componentId, int slotId) {
		if (stage == -1) {
			sendOptionsDialogue("Would you like to travel?",
			...
			stage = 8;
			sendNPCDialogue(npc.getId(), NORMAL,
					"Alright then, just come again if you would like to travel.");
		} else if (stage == 3) {
			sendPlayerDialogue(WORRIED, "Oh no! Get it out. Get it out!");
			stage = 4;
		} else if (stage == 4) {
			sendNPCDialogue(npc.getId(), NORMAL,
					"Sure. But quickly, close your eyes!");
			stage = 5;
			player.lock();
			...
			final WorldTile tile = isBraindeadIsland ? new WorldTile(3680,
					3536, 0) : new WorldTile(2162, 5115, 1);
			...
			sendNPCDialogue(
					npc.getId(),
					NORMAL,
					"I hit you in the hea... I mean.. *err* you slipped and fell on the dock. Its slippery ya know!");
			stage = 7;
		} else if (stage == 7) {
			sendNPCDialogue(npc.getId(), NORMAL,
					"Anyways, be off now. You seem to have recovered from the fall.");
			stage = 8;
		} else if (stage == 8) {
			sendPlayerDialogue(NORMAL,
					"Oh... alright then. Thank you for the lift.");
			stage = 9;
		} else if (stage == 9) {
			end();
		}
