...
public class RiseOfTheSixD extends Dialogue {
			...
			case OPTION_3:
				player.getPackets()
						.sendInputLongTextScript(
								"Please enter the name of the player whose party you wish to join:");
		...
		case 2:
			...
			case OPTION_2:
				if (player.getInventory().containsItem(30004, 1)) {
					player.clearROSPartyMembers();
					player.addROSPartyMember(player);
					if (player.getROSPartyMembers().isEmpty()
							|| player.getROSPartyMembers() == null)
						player.getPackets().sendGameMessage(
								"You must create a party first.");
					else if (!player.withinDistance(
							new WorldTile(2326, 5910, 0), 50))
						player.getPackets()
								.sendGameMessage(
										"You must be in the Shadow Realm in order to start the game.");
					else {
						for (Player member : player.getROSPartyMembers()) {
							if (!member.withinDistance(new WorldTile(2326,
									5910, 0), 50)) {
								member.getPackets()
										.sendGameMessage(
												"You must be in the Shadow Realm in order to start the game.");
						...
						WorldTasksManager.schedule(new WorldTask() {
							...
							public void run() {
								if (stage == 3) {
									for (Player member : player
											.getROSPartyMembers())
										member.getControllerManager()
												.startController(
														"RiseOfTheSix", false,
														player);
									this.stop();
	...
	public static void joinParty(Player player, String username) {
		...
		if (player.getUsername().equalsIgnoreCase(username))
			...
			player.getPackets().sendGameMessage(
					"The player you have entered is busy.");
		else if (host.getROSPartyMembers().size() >= 4)
			player.getPackets()
					.sendGameMessage(
							"The player you have entered already has a full party of 4.");
		else {
			host.addROSPartyMember(player);
			player.getPackets().sendGameMessage(
					"You have joined " + Utils.fixChatMessage(username)
							+ "'s party.");
			host.getPackets().sendGameMessage(
					player.getDisplayName() + " has joined your party.");
		}
		player.setNextAnimation(new Animation(21922));
	}
