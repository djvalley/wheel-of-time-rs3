...
import net.wheeloftime.game.player.actions.skills.construction.House.ObjectReference;
...
public class PortalChamberD extends Dialogue {
	...
	private static final int[][] RUNES = { { 563, 100, 556, 300, 554, 100 },
			{ 563, 100, 556, 300, 557, 100 }, { 563, 100, 556, 300, 555, 100 },
			{ 563, 100, 556, 500 }, { 563, 200, 555, 200 },
			{ 563, 200, 557, 200 }, { 563, 200, 565, 200 } };

	private static final int[] LEVELS = { 25, 31, 37, 45, 51, 58, 66 };
	private static final double[] EXPERIENCE = { 19, 41, 48, 55.5, 61, 68, 76 };
	private static final String[] OPTION_NAMES = { "Nowhere", "Varrock",
			"Lumbridge", "Falador", "Camelot", "Ardougne", "Yanille", "Kharyll" };
	...
	public void run(int interfaceId, int componentId, int slotId) {
		if (stage == -1) {
			...
			sendOptionsDialogue("Redirect which portal?",
					existingPortals[0] ? OPTION_NAMES[primaryFrame]
							: "No portal frame",
					existingPortals[1] ? OPTION_NAMES[secondaryFrame]
							: "No portal frame",
					existingPortals[2] ? OPTION_NAMES[tertiaryFrame]
							: "No portal frame", "Cancel");
			stage = 0;
		} else if (stage == 0) {
			if (componentId >= OPTION_1 && componentId <= OPTION_3) {
				...
				sendOptionsDialogue("Select a desination", "Varrock",
						"Lumbridge", "Falador", "Camelot", "More");
				stage = 1;
			...
			if (componentId >= OPTION_1 && componentId <= OPTION_4) {
				...
				sendOptionsDialogue("Select a desination", "Ardougne",
						"Yanille", "Kharyll", "Previous");
				stage = 2;
			}
		} else if (stage == 2) {
			if (componentId >= OPTION_1 && componentId <= OPTION_3) {
	...
	private void placePortal(int index) {
		...
		if (player.getSkills().getLevel(Skills.MAGIC) < requriedLevel) {
			sendDialogue("You need a magic level of at least " + requriedLevel
					+ " in order to focus to this location");
			return;
		}
		int runesCount = 0;
		...
		player.getPackets().sendGameMessage(
				"You redirected the portal. Turn build mode off to see it.");
