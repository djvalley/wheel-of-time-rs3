...
public class CreateRoomStairsD extends Dialogue {
	...
	public void start() {
		...
		sendOptionsDialogue((dungeonEntrance ? "This entrance does"
				: "These stairs do")
				+ " not lead anywhere. Do you want to build a room at the "
		...
		if (stage == -1) {
			if (componentId == OPTION_1) {
				...
				if (room.getPlane() == 1 && !up)
					sendOptionsDialogue(DEFAULT_OPTIONS_TITLE, "Skill hall",
							"Quest hall", "Dungeon stairs room");
				else
					sendOptionsDialogue(DEFAULT_OPTIONS_TITLE, "Skill hall",
							"Quest hall");
				return;
			...
			Room r = room.getPlane() == 1 && !up && componentId == OPTION_3 ? Room.DUNGEON_STAIRS
					: componentId == OPTION_2 ? up ? Room.HALL_QUEST_DOWN
							...
							: Room.HALL_SKILL;
			Builds stair = (room.getPlane() == 1 && !up && componentId == OPTION_3) ? Builds.STAIRCASE
