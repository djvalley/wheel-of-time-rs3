...
public class ItemOnServantD extends Dialogue {
	...
	public void start() {
		...
		if (definition.isNoted()) {
			sendOptionsDialogue("Un-cert this item?", "Un-cert " + name + ".",
					"Fetch another " + name + ".", "Bank", "Cancel");
			stage = 0;
		} else if ((boolean) this.parameters[2] && plank != null) {
			sendOptionsDialogue("Take this to the sawmill?",
					"Take it to the sawmill.", "Bank", "Cancel");
			stage = 2;
		} else {
			sendOptionsDialogue("Take this item to the bank?", "Fetch another "
					+ name + ".", "Bank", "Cancel");
			stage = 1;
		}
	}
