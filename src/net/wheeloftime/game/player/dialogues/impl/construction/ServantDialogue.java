...
public class ServantDialogue extends Dialogue {
	...
	private static final String[] BEGINNING_MESSAGE = {
			...
			"Ah, I visited with her Royal Highness for the Diamond Jubillee,"
					...
					+ " an income. Care to hire me, for only 375 coins?",
	...
	public void run(int interfaceId, int componentId, int slotId) {
		int slot = getSlot();
		if (stage == -1) {
			sendOptionsDialogue("Select an option", "What can you do?",
					"Tell me about your previous jobs.", "You're hired!");
			stage = 0;
		} else if (stage == 0) {
			...
			if (player.getInventory().getCoinsAmount() < servant.getCost()) {
				...
				stage = 10;
				return;
			} else if (player.getSkills().getLevel(Skills.CONSTRUCTION) < servant
					.getLevel()) {
				sendDialogue("You need a Construction level of at least "
						+ servant.getLevel() + ".");
				stage = 10;
				return;
			}
