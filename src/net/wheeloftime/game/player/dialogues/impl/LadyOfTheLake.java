...
public class LadyOfTheLake extends Dialogue {
	...
	@Override
	public void start() {
		sendNPCDialogue(npcId, NORMAL, "Good day to you, "
	...
	public void run(int interfaceId, int componentId, int slotId) {
		if (stage == -1) {
			sendOptionsDialogue("What would you like to say?", "Who are you?",
					"I seek the sword Excalibur.", "Goodbye.");
			stage = 0;
		} else if (stage == 0) {
			...
			if (IGNORE_QUEST) { // player.getQuestManager().completedQuest(Quests.MERLINS_CRYSTAL)
				sendNPCDialogue(
						npcId,
						NORMAL,
						"I recall you have already proven yourself to be worthy of wielding it. I shall return it to you if you can prove yourself to still be worthy.");
				stage = 5;
			} else {
				sendNPCDialogue(npcId, NORMAL,
						"You are not worthy of wielding this magnificent weapon.");
				stage = 14;
			}
		} else if (stage == 5) {
			...
			sendNPCDialogue(npcId, NORMAL,
					"Why, by proving yourself to be above material goods.");
			stage = 7;
		} else if (stage == 7) {
			sendPlayerDialogue(NORMAL, "...And how can I do that?");
			stage = 8;
			...
			sendNPCDialogue(npcId, NORMAL,
					"Thanks. I'm saving up for a new dress!");
			stage = 12;
		} else if (stage == 11) {
			...
			int itemId = player.getSkills().hasRequiriments(Skills.COOKING, 80,
					Skills.FLETCHING, 80, Skills.FISHING, 76,
					Skills.FIREMAKING, 75, Skills.DEFENCE, 70, Skills.PRAYER,
					70, Skills.WOODCUTTING, 60, Skills.MAGIC, 56,
					Skills.RANGED, 39, Skills.AGILITY, 36, Skills.STRENGTH, 22) ? 14632
					: 35;
			player.getInventory().addItemDrop(itemId, 1);
			sendEntityDialogue(IS_ITEM, "", itemId, -1,
					"The Lady of the Lake hands you an "
							+ (itemId == 35 ? "Excalibur"
									: "Enhanced Excalibur") + ".");
			stage = 14;
		} else if (stage == 14) {
