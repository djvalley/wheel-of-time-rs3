...
public class Scribe extends Dialogue {
	...
	public void start() {
		...
		stage = 50;
		sendOptionsDialogue(DEFAULT_OPTIONS_TITLE, "I'd like a "
				+ ItemDefinitions.getItemDefinitions(20708).getName()
						.toLowerCase() + ".", "I'd like a "
				+ ItemDefinitions.getItemDefinitions(20709).getName()
						.toLowerCase() + ".", "Nevermind.");
	}
	...
	public void run(int interfaceId, int componentId, int slotId) {
		...
		case 50:
			...
			switch (componentId) {
			case OPTION_1:
				if (!player.getInventory().containsItem(20708, 1)) {
					sendNPCDialogue(npcId, HAPPY, "Enjoy your "
							+ ItemDefinitions.getItemDefinitions(20708)
									.getName().toLowerCase() + "!");
					player.getInventory().addItem(new Item(20708));
				} else {
					sendNPCDialogue(npcId, MAD, "You already have a "
							+ ItemDefinitions.getItemDefinitions(20708)
									.getName().toLowerCase() + "!");
				}
				break;
			case OPTION_2:
				if (!player.getInventory().containsItem(20709, 1)) {
					sendNPCDialogue(npcId, HAPPY, "Enjoy your "
							+ ItemDefinitions.getItemDefinitions(20709)
									.getName().toLowerCase() + "!");
					player.getInventory().addItem(new Item(20709));
				} else {
					sendNPCDialogue(npcId, MAD, "You already have a "
							+ ItemDefinitions.getItemDefinitions(20709)
									.getName().toLowerCase() + "!");
