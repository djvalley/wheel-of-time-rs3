package net.wheeloftime.game.player.dialogues.impl.rottenpotato;
...
public class OPGear extends Dialogue {
	...
	public void run(int interfaceId, int componentId, int slotId) {
		...
		case 1:
			...
			case OPTION_4:
				stage = 5;
				sendOptionsDialogue("Tank", "Nothing.", "Full Tank.",
						"Nevermind.");
				break;
			case OPTION_5:
				stage = 6;
				sendOptionsDialogue("Custom setup", "Load setup.",
						"Save this setup.", "J-Mod outfit.", "Nevermind.");
				break;
			}
			break;
		case 2:
			...
			case OPTION_1:
				...
				setEquipment(2581, 6107, 24379, 25991, 18349, 3105, 24376,
						15220, 22298, 23659, 19335, 0);
				setFinish("melee/pure");
				break;
			case OPTION_2:
				setClear(2, true);
				setStats(99, 99, 99, 99, 99, 99, 99, 99);
				setEquipment(30005, 30008, 30011, 0, 31725, 21787, 30213,
						15220, 22298, 20771, 19335, 0);
				setFinish("melee/max");
				break;
			case OPTION_3:
				end();
		...
		case 3:
			...
			case OPTION_1:
				...
				setEquipment(2581, 6107, 24379, 28441, 28437, 2577, 24376,
						15019, 22298, 23659, 19335, 28465);
				setFinish("range/pure");
				break;
			case OPTION_2:
				setClear(0, false);
				setStats(99, 99, 99, 99, 99, 99, 99, 99);
				setEquipment(29854, 29857, 29860, 28441, 28437, 20452, 31203,
						15019, 22298, 20771, 19335, 28465);
				setFinish("range/max");
				break;
			case OPTION_3:
				end();
		...
		case 4:
			...
			case OPTION_1:
				...
				setEquipment(25825, 25827, 25831, 25802, 28617, 25833, 25829,
						15018, 22298, 23659, 19335, 0);
				setFinish("mage/pure");
				break;
			case OPTION_2:
				setClear(1, true);
				setStats(99, 99, 99, 99, 99, 99, 99, 99);
				setEquipment(28608, 28611, 28614, 30022, 25654, 20452, 31189,
						15018, 22298, 20771, 19335, 0);
				setFinish("mage/max");
				break;
			case OPTION_3:
				end();
		...
		case 5:
			...
			case OPTION_2:
				setClear(2, true);
				setStats(99, 99, 99, 99, 99, 99, 99, 99);
				setEquipment(30005, 30008, 30011, 30014, 31725, 21787, 30213,
						15220, 22298, 20771, 19335, 0);
				setFinish("tank/full");
				break;
			case OPTION_3:
		...
		case 6:
			...
			case OPTION_1:
				setClear(0, false);
				setStats(player.saveattack, player.savedefence,
						player.savestrength, player.savehitpoints,
						player.saverange, player.saveprayer, player.savemagic,
						player.savesummoning);
			...
			case OPTION_2:
				...
				player.getPackets().sendGameMessage(
						"Successfully saved your outfit.");
				end();
				break;
			case OPTION_3:
				setClear(0, false);
				setEquipment(25867, 25868, 25869, 0, 0, 25870, 25871, 0, 0,
						25872, 0, 0);
	...
	public void start() {
		stage = 1;
		sendOptionsDialogue("Rotten Potato - Option: Gear Setup", "Melee.",
				"Range.", "Mage.", "Tank.", "Load/save custom setup.");
	}

	private void setClear(int spellbook, boolean curses) {
	...
	private void setStats(int attack, int defence, int strength, int hitpoints,
			int range, int prayer, int magic, int summoning) {
	...
	private void setEquipment(int head, int chest, int legs, int shield,
			int weapon, int boots, int gloves, int ring, int aura, int cape,
			int amulet, int arrow) {
		if (head != 0)
			player.getEquipment().getItems()
					.set(Equipment.SLOT_HAT, new Item(head, 1));
		if (chest != 0)
			player.getEquipment().getItems()
					.set(Equipment.SLOT_CHEST, new Item(chest, 1));
		if (legs != 0)
			player.getEquipment().getItems()
					.set(Equipment.SLOT_LEGS, new Item(legs, 1));
		if (boots != 0)
			player.getEquipment().getItems()
					.set(Equipment.SLOT_FEET, new Item(boots, 1));
		if (ring != 0)
			player.getEquipment().getItems()
					.set(Equipment.SLOT_RING, new Item(ring, 1));
		if (gloves != 0)
			player.getEquipment().getItems()
					.set(Equipment.SLOT_HANDS, new Item(gloves, 1));
		if (aura != 0)
			player.getEquipment().getItems()
					.set(Equipment.SLOT_AURA, new Item(aura, 1));
		if (cape != 0)
			player.getEquipment().getItems()
					.set(Equipment.SLOT_CAPE, new Item(cape, 1));
		if (shield != 0)
			player.getEquipment().getItems()
					.set(Equipment.SLOT_SHIELD, new Item(shield, 1));
		if (amulet != 0)
			player.getEquipment().getItems()
					.set(Equipment.SLOT_AMULET, new Item(amulet, 1));
		if (weapon != 0)
			player.getEquipment().getItems()
					.set(Equipment.SLOT_WEAPON, new Item(weapon, 1));
		if (arrow != 0)
			player.getEquipment()
					.getItems()
					.set(Equipment.SLOT_ARROWS,
							new Item(arrow, Integer.MAX_VALUE));
	}
	...
	public void setFinish(String equipmentName) {
		...
		player.getInventory().addItem(5733, 1);
		player.getPackets().sendGameMessage(
				"equipment set to: " + equipmentName + ".");
		player.reset();
		end();
	}
}