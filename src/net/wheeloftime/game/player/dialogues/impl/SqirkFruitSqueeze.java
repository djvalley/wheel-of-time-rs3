...
public class SqirkFruitSqueeze extends Dialogue {
	...
	public static enum SqirkFruit {
		WINTER(10847, 5, 10851), SPRING(10844, 4, 10848), AUTUMM(10846, 3,
				10850), SUMMER(10845, 2, 10849);
	...
	public void start() {
		...
		if (!player.getInventory().containsItem(BEER_GLASS, 1)) {
			sendItemDialogue(
					BEER_GLASS,
					"I should get an empty beer glass to hold the juice before I squeeze the fruit.");
			stage = -2;
		} else if (!player.getInventory().containsItemToolBelt(233)) {
			sendItemDialogue(BEER_GLASS,
					"I should get a pestle and mortal before I squeeze the fruit.");
			stage = -2;
		} else if (!player.getInventory().containsItem(fruit.fruitId,
				fruit.amtRequired))
			sendPlayerDialogue(9827,
					"I think I should wait until I have enough fruit to make a full glass.");
		else {
			...
			sendItemDialogue(BEER_GLASS, "You squeeze " + fruit.amtRequired
					+ " sq'irks into an empty glass.");
			stage = -2;
		}
	}
	...
	public void run(int interfaceId, int componentId, int slotId) {
		...
		case -1:
			stage = -2;
			sendItemDialogue(fruit.fruitId, "You need " + fruit.amtRequired
					+ " sq'irks to fill a glass of juice.");
			break;
		default:
			end();
			break;
