...
public class AmuletAttaching extends Dialogue {
	...
	private static final int[] BASE = { 1673, 1675, 1677, 1679, 1681, 1683,
			6579 };
	private static final int[] PRODUCTS = { 1692, 1694, 1696, 1698, 1700, 1702,
			6581 };
	...
	public void start() {
		 ...
		 * LEVELS[count++]; if (player.getSkills().getLevel(Skills.CRAFTING) <
	...
	public void run(int interfaceId, int componentId, int slotId) {
		...
		player.getActionManager().setAction(new Action() {
			...
			public boolean start(final Player player) {
				if (!checkAll(player))
					return false;
				int amuletAmount = player.getInventory().getAmountOf(
						BASE[index]);
				int requestedAmount = result.getQuantity();
			...
			public boolean checkAll(Player player) {
				if (player.getSkills().getLevel(Skills.CRAFTING) < LEVELS[index]) {
					player.getPackets().sendGameMessage(
							"You need a Crafting level of " + LEVELS[index]
									+ ".");
					return false;
	...
	public static boolean isAttaching(int used, int usedWith) {
		for (int item : BASE) {
			if (item == used && usedWith == STRING || item == usedWith
					&& used == STRING)
				return true;
		}
		return false;
