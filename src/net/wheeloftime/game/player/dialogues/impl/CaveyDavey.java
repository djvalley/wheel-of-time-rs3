...
public class CaveyDavey extends Dialogue {
	...
	public void start() {
		sendNPCDialogue(npcId, NORMAL,
				"Be ye mad? There be Horrors in this cave!");
	...
	public void run(int interfaceId, int componentId, int slotId) {
		if (stage == -1) {
			...
			sendNPCDialogue(
					npcId,
					NORMAL,
					"Have ye ever head of the sort of evil, flesh-eatin' horros that dwell in the darkest pits of the world?");
			stage = 1;
		} else if (stage == 1) {
			sendNPCDialogue(
					npcId,
					NORMAL,
					"The sort of dark, sanity-breaking' THINGS that cause the livin' to dorp to their knees and weep for the fate of all creation,");
			stage = 2;
		} else if (stage == 2) {
			...
			sendPlayerDialogue(NORMAL,
					"Yes, I think i've killed a few of them as well.");
			stage = 4;
		} else if (stage == 4) {
			...
			sendNPCDialogue(
					npcId,
					NORMAL,
					"Wearin' earmuffs or a helmet won't work, cos them masks they wear make the sound magical. Only thing that works is wearin' a Witchwood Icon.");
			stage = 10;
		} else if (stage == 10) {
			sendNPCDialogue(
					npcId,
					NORMAL,
					"That is, o' course, if ye can see them cos if ye don't have any light down there then yer likely te be picked te bones by the insects before the horrors get ye.");
			stage = 11;
		} else if (stage == 11) {
			...
			sendOptionsDialogue("Do you still want to proceed?",
					"Yes, I am not afraid to be killed in nasty ways.",
					"No, I think I need to go get one of these Icon things.");
			stage = 14;
		} else if (stage == 14) {
			...
			sendOptionsDialogue("Do you still want to proceed?",
					"Yes, I think I am fully prepared.",
					"No, I think I need to go get food, weapons and a light source.");
			stage = 14;
		}
	}
