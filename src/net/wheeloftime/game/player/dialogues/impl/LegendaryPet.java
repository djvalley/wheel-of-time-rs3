...
public class LegendaryPet extends Dialogue {
	...
	public void start() {
		...
		options("What would you like to do with "
				+ item.getName().toLowerCase() + "?", "Alch The Item",
				"Note The Item");
	}
	...
	public void run(int interfaceId, int componentId, int slotId) {
		if (componentId == OPTION_1) {
			int itemId = (item.getDefinitions().isNoted() ? (item.getId() - 1)
					: item.getId());
			int alchPrice = GrandExchange.getPrice(itemId);
			alchPrice = (alchPrice / (int) 2);
			alchPrice = (alchPrice * player.getInventory().getAmountOf(
					item.getId()));
			if (alchPrice == 0)
				return;
			...
			player.getPackets().sendGameMessage(
					"<col=E86100>" + player.getPet().getName() + " has alched "
							+ item.getName().toLowerCase() + " for "
							+ Utils.formatNumber(alchPrice) + "gp.");
		} else {
			int notedItem = item.getId() + 1;
			ItemDefinitions defs = new ItemDefinitions(notedItem);
			if (defs.isNoted()) {
				for (int i = 0; i < player.getInventory()
						.getItemsContainerSize(); i++) {
				...
				player.getPackets().sendGameMessage(
						"<col=E86100>" + player.getPet().getName()
								+ " notes the " + item.getName().toLowerCase()
								+ "...");
			} else
				player.getPackets().sendGameMessage(
						"You are unable to note this item.");
		}
		finish();
	}
