...
public class NeemDrupeSqueeze extends Dialogue {
	...
	public void start() {
		...
		if (!hasOil && !player.getInventory().containsItem(1935, 1)) {
			sendItemDialogue(1935,
					"I should get an empty jug to hold the juice before I squeeze the neem drupe.");
		} else if (!player.getInventory().containsItemToolBelt(233))
			sendItemDialogue(233,
					"I should get a pestle and mortal before I squeeze the neem drupe.");
		else {
			if (!hasOil) {
	...
	public static void removeCharge(Player player) {
		...
		if (player.getCharges().getCharges(22444) == 0) {
			...
			player.getPackets().sendGameMessage(
					"There is no more oil in the jug.");
		}
	}
