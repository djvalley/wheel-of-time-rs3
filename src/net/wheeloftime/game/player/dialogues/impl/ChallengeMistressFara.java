...
public class ChallengeMistressFara extends Dialogue {
	...
	public void run(int interfaceId, int componentId, int slotId) {
		...
		case 2:
			...
			sendOptionsDialogue(
					...
					player.getInventory().containsItem(23030, 1)
							|| player.getBank().containsItem(23030) ? "Who are you?"
							: "Can I have my baby troll?",
					"Tell me about challenges.",
					"Tell me about Daily Challenges.", "Goodbye.");
			break;
		case 3:
			switch (componentId) {
			case OPTION_1:
				...
				break;
			case OPTION_2:
				if (player.getInventory().containsItem(23030, 1)
						|| player.getBank().containsItem(23030)) {
					stage = 18;
					sendNPCDialogue(npcId, HAPPY,
							"The name's Fara, proud gnome and collector extraordinaire.");
				} else {
					stage = 4;
					sendNPCDialogue(
							npcId,
		...
		case 4:
			stage = 5;
			sendOptionsDialogue(DEFAULT_OPTIONS_TITLE,
					"I could look after him.", "Bad luck!",
		...
		case 15:
			stage = 16;
			sendNPCDialogue(npcId, HAPPY,
					"I'll give you one of these lamps for every Daily Challenge you complete.");
			break;
		case 16:
			stage = 17;
			sendDialogue("If you complete a Combat Challenge you'll get an XP lamp that can be used in any combat skill. If you complete any other challenge, you'll get XP in that skill directly.");
			break;
		case 17:
			stage = 2;
			sendDialogue("Every day you will receive a new Daily Challenge. <col=FF0000>If you already have five Daily Challenges, they oldest will disappear!");
			break;
		case 18:
			stage = 2;
			sendNPCDialogue(
		...
		case 19:
			...
			sendNPCDialogue(
					...
					HAPPY,
					"I shall reward you "
							+ Utils.format(exp * Settings.XP_RATE)
							+ " xp in "
							+ player.getSkills().getSkillName(skill)
							+ " and x"
							+ Utils.format(amount)
							+ " "
							+ ItemDefinitions.getItemDefinitions(reward)
									.getName() + ".");
	...
	private void handleReward(int reward, int amount, int exp, int skill,
			int coins) {
