...
public class SmugglerD extends Dialogue {
	...
	public void start() {
		sendNPCDialogue(DungeonConstants.SMUGGLER, NORMAL,
				"Hail, " + player.getDisplayName() + ". Need something?");
		stage = -1;
	}
	...
	public void run(int interfaceId, int componentId, int slotId) {
		if (stage == -1) {
			sendOptionsDialogue("Select an Option",
					"What can you tell me about this place?", "Who are you?",
					"Do I have any rewards to claim?", "I'm here to trade.");
			stage = 0;
		} else if (stage == 0) {
			if (componentId == OPTION_1) {
				...
				sendNPCDialogue(DungeonConstants.SMUGGLER, NORMAL,
						"I have no rewards for you at the moment.");
				stage = 100;
			...
			sendNPCDialogue(DungeonConstants.SMUGGLER, NORMAL,
					"Yes, I'm a trader. But I'm not supposed to be trading here.");
			stage = 10;
		} else if (stage == 10) {
			sendNPCDialogue(DungeonConstants.SMUGGLER, NORMAL,
			...
			sendNPCDialogue(DungeonConstants.SMUGGLER, NORMAL,
					"Well, there's the job I'm supposed to be doing down here.");
			stage = 14;
		} else if (stage == 14) {
			...
			sendPlayerDialogue(NORMAL,
					"I'll be sure to come back if I find a particularly strong opponent, then.");
			stage = 19;
		} else if (stage == 19) {
			sendNPCDialogue(DungeonConstants.SMUGGLER, NORMAL,
					"You'd be wise to " + player.getDisplayName() + ".");
			stage = 20;
		} else if (stage == 20) {
			...
			sendNPCDialogue(DungeonConstants.SMUGGLER, NORMAL,
					"Nothing gets in or out of Daemonhiem wihout me knowing about it.");
			stage = 22;
		} else if (stage == 22) {
			...
			// DungeonResourceShop.openResourceShop(player, (int)
			// this.parameters[0]);
			end();
		} else if (stage == 100) {
			end();
