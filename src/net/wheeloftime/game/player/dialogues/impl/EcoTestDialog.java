...
public class EcoTestDialog extends Dialogue {
	...
	private void runStep() {
		if (step == 0) {
			...
			this.sendEntityDialogue(Dialogue.IS_NPC, "Suspicious merchant",
					1282, 9770, "Psst! Be quiet!",
					"I can sell you forbidden goods, want some?");
		} else if (step == 2) {
			this.sendOptionsDialogue("Do you want to buy some goods?", "Yes",
					"No");
		} else if (step == 3) {
			this.sendOptionsDialogue("Select shop", "Melee", "Ranged", "Magic",
					"Food & Potions", "Next");
		} else if (step == 4) {
			this.sendOptionsDialogue("Select shop", "Rings & Amulets & Gloves",
					"Capes", "Skilling items 1", "Skilling items 2",
					"Vote items");
		}
	}
