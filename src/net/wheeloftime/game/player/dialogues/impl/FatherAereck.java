	...
	public void run(int interfaceId, int componentId, int slotId) {
		if (stage == -1) {
			...
			sendOptionsDialogue(DEFAULT_OPTIONS_TITLE,
					"Can I have a different gravestone?",
					"Can you restore my prayer?", "Nothing, nevermind.");
		} else if (stage == 0) {
			...
			sendNPCDialogue(npcId, 9827,
					"Of course you can. Have a look at this selection of gravestones.");
		} else if (stage == 2) {
			stage = 3;
			sendNPCDialogue(
					npcId,
					9827,
					"I think the Gods prefer it if you pray<br>to them at an altar dedicated to their name.");
		} else if (stage == 4) {
			end();
