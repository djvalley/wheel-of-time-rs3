...
public class BoatingDialouge extends Dialogue {
	...
	public void start() {
		...
		sendNPCDialogue(npcId, 9827,
				"Hello adventurer, how can I help you today?");
	}
	...
	public void run(int interfaceId, int componentId, int slotId) {
		if (stage == -1) {
			...
			sendOptionsDialogue(DEFAULT_OPTIONS_TITLE,
					"Where does this boat take me?", "Nothing, nevermind.");
		} else if (stage == 0) {
			...
			if (cost == -1)
				sendNPCDialogue(
						npcId,
						9827,
						"This boat? Why this boat takes you to "
				...
				sendNPCDialogue(
						...
						"This boat? Why this boat takes you to "
								+ ship.getFixedName(returning)
								+ ", for a small fee of " + cost + " coins.");
		} else if (stage == 2) {
			if (cost != -1)
				sendOptionsDialogue("Pay the price of " + cost + " coins?",
						"Yes, board the ship.", "I can't affored that!");
			else
				sendOptionsDialogue("Board the ship?", "Yes, board the ship.",
						"No, sometime later.");
			stage = 3;
		} else if (stage == 3) {
			if (componentId == OPTION_1)
			...
			sendNPCDialogue(npcId, 9827,
					"Oh alright then, have a splended day.");
			stage = 5;
		} else if (stage == 5) {
			end();
		}
