		...
		case 21:
			if (player.getInventory().getCoinsAmount() < 5000000) {
				...
				if (player.getInventory().getFreeSlots() < 2) {
					...
					int type = player.getCompCapeManager().hasTrimmedRequirements() ? 2 : 1;
					if (type == 2) {
					...
					if (!player.getCompCapeManager().receivedCape(1)) {
						if (player.isAnIronMan())
							World.sendNews(
									...
									player.getIronmanTitle(false)
											...
											+ (type == 2 ? "trimmed " : "")
											+ "Completionist Cape!", 0, true);
						else
							World.sendNews(player, player.getDisplayName()
									+ " has been awarded the "
									+ (type == 2 ? "trimmed " : "")
									+ "Completionist Cape!", 0, true);
						player.getCompCapeManager().setReceived(type);
