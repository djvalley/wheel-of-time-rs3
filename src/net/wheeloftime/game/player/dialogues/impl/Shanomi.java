...
public class Shanomi extends Dialogue {
	...
	@Override
	public void start() {
		sendNPCDialogue(npcId, NORMAL, "Greetings " + player.getDisplayName()
				+ " Welcome you are in the test of combat.");
	}

	@Override
	public void run(int interfaceId, int componentId, int slotId) {
		if (stage == -1) {
			sendOptionsDialogue(DEFAULT_OPTIONS_TITLE, "What do I do here?",
					"Where do the machines come from?", "Bye!");
			stage = 0;
		} else if (stage == 0) {
