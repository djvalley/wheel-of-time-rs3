package net.wheeloftime.game.player.dialogues;
...
public abstract class Dialogue {
	...
	public abstract void run(int interfaceId, int componentId, int slotId) throws ClassNotFoundException;
	...
	public static final String[] RANDOM_GREETINGS = { "Hello there.", "Howdy.", "Good day.", "Salutations!",
			"Greetings.", "Nice to meet you." };

	public static final String[] RANDOM_RESPONSES = { "Good day to you, adventurer.", "Well met, adventurer.",
			"Well, hello there." };

	public static final int NORMAL = 9827, QUESTIONS = 9827, MAD = 9789, MOCK = 9878, LAUGHING = 9851, WORRIED = 9775,
			HAPPY = 9850, CONFUSED = 9830, DRUNK = 9835, ANGERY = 9790, SAD = 9775, SCARED = 9780;
	...
	public static final int ANGRY = 9790, ASKING = 9829, BLANK = 9772, UPSET = 9776, MILDLY_ANGRY = 9784,
			VERY_ANGRY = 9792, MANIAC = 9800, NOT_TALKING_JUST_LISTENING = 9804, PLAIN_TALKING = 9808, UNSURE = 9836,
			LISTENS_THEN_LAUGHS = 9840, GOOFY_LAUGH = 9851, THINKING_THEN_TALKING = 9859, NONONO = 9844,
			SAD_SHOCKED = 9776;
	...
	public static final int OPTION_1 = 5, OPTION_2 = 10, OPTION_3 = 15, OPTION_4 = 20, OPTION_5 = 25;
	...
	public boolean sendOptionsDialogue(String title, String... options) {
		...
		player.getPackets().sendIComponentText(1188, 31, title);
	...
	public boolean sendEntityDialogues(short interId, String[] talkDefinitions, byte type, int entityId,
			int animationId) {
		if (type == IS_PLAYER || type == IS_NPC) {
			String[] texts = new String[talkDefinitions.length - 1];
			System.arraycopy(talkDefinitions, 1, texts, 0, texts.length);
			sendEntityDialogue(type, talkDefinitions[0], entityId, animationId, texts);
			return true;
		}
		...
		if (talkDefinitions.length != componentOptions.length)
			return false;
	...
	public boolean sendEntityDialogue(int type, String title, int entityId, int animationId, String... texts) {
		...
		if (type == IS_NPC) {
			...
			player.getPackets().sendIComponentText(1184, 11, title);
			player.getPackets().sendIComponentText(1184, 9, text);
			...
			player.getPackets().sendPlayerHeadOnIComponent(1191, 10);
			if (animationId != -1)
				player.getPackets().sendIComponentAnimation(animationId, 1191, 10);
		} else if (type == IS_ITEM) {
			player.getInterfaceManager().sendDialogueInterface(1189);
			player.getPackets().sendIComponentText(1189, 3, title);
			player.getPackets().sendIComponentText(1189, 2, text);
			player.getPackets().sendItemOnIComponent(1189, 8, entityId, 1); // there
			// is
			// a
			// config
			// for
	...
	public boolean sendEntityDialogue(short interId, String[] talkDefinitions, byte type, int entityId,
			int animationId) {
