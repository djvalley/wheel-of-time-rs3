...
import net.wheeloftime.game.player.GamePointManager.GPR;
...
public final class Combat {
	...
	private static final double WEAKNESS_MULTIPLIER_STYLE = 0.7,
			WEAKNESS_MULTIPLIER_WEAK = 0.65,
			WEAKNESS_MULTIPLIER_NEUTRAL = 0.55,
			WEAKNESS_MULTIPLIER_STRONG = 0.45;

	public static final int NONE_STYLE = 0, ARROW_STYLE = 8, BOLT_STYLE = 9,
			THROWN_STYLE = 10;

	public static final int TYPE_AIR = 1, TYPE_WATER = 2, TYPE_EARTH = 3,
			TYPE_FIRE = 4;

	public static double getWeaknessMultiplier(double totalArmor,
			double weakArmor, double neutralArmor, double strongArmor) {
		return totalArmor == 0 ? WEAKNESS_MULTIPLIER_NEUTRAL : ((weakArmor
				* WEAKNESS_MULTIPLIER_WEAK + neutralArmor
				* WEAKNESS_MULTIPLIER_NEUTRAL + strongArmor
				* WEAKNESS_MULTIPLIER_STRONG) / totalArmor);
	}

	public static int[] COMBAT_SKILLS = { Skills.ATTACK, Skills.DEFENCE,
			Skills.STRENGTH, Skills.RANGED, Skills.MAGIC };
	...
	public static void giveXP(Entity from, double totalXP) {
		...
		for (Entity source : dmgReceived.keySet()) {
			...
			int combatStyle = player.getCombatDefinitions().getType(
					Equipment.SLOT_WEAPON);
			if (combatStyle == Combat.MELEE_TYPE
					|| combatStyle == Combat.ALL_TYPE) {
				switch (player.getCombatDefinitions()
						.getMeleeCombatExperience()) {
				case 0:
				...
				case 2:
					...
					break;
				case 3:
					player.getSkills().addXp(Skills.ATTACK, receivedXP / 2);
					player.getSkills().addXp(Skills.STRENGTH, receivedXP / 2);
					break;
				case 4:
				...
				switch (player.getCombatDefinitions()
						.getRangedCombatExperience()) {
				case 0:
					player.getSkills().addXp(Skills.RANGED, receivedXP / 2);
					player.getSkills().addXp(Skills.DEFENCE, receivedXP / 2);
					break;
				case 1:
					player.getSkills().addXp(Skills.RANGED, receivedXP);
					break;
				case 2:
					player.getSkills().addXp(Skills.DEFENCE, receivedXP);
					break;
			...
			if (Settings.DEBUG)
				System.out.println(from.getName() + " gave: " + totalXP
						* Settings.XP_RATE + "xp");
		}

	}

	public static double getHitChance(Entity from, Entity target,
			int attackStyle, boolean mainHand) {
		double accuracy;
		int attackType = from instanceof Player ? Combat
				.getStyleType(attackStyle) : attackStyle;
		if (from instanceof Player) {
			// This handles PVP
			Player player = (Player) from;
			accuracy = player.getCombatDefinitions()
					.getSkillAccuracy(!mainHand)
					+ (player.getCombatDefinitions().getStats()[mainHand ? CombatDefinitions.MAINHAND_ACCURACY
							: CombatDefinitions.OFFHAND_ACCURACY] * 10);
			accuracy -= player.getCombatDefinitions().getStats()[attackType == MAGIC_TYPE ? CombatDefinitions.MAGE_ACCURACY_PENALTY
					: attackType == RANGE_TYPE ? CombatDefinitions.RANGE_ACCURACY_PENALTY
							: CombatDefinitions.MELEE_ACCURACY_PENALTY];
			accuracy *= player.getPrayer()
					.getCombatRatingMultiplier(attackType);

			if (attackType == RANGE_TYPE) {
				int rangeLevel = player.getSkills().getLevel(4);
				accuracy *= Utils.random(1.0, rangeLevel > 10 ? (1.125 * 15)
						/ (rangeLevel / 10) : 1.5);
			}
		...
		if (target instanceof Player)
			accuracy *= ((Player) target).getPrayer()
					.getEnemyCombatRatingMultiplier(attackType);
		...
		if (target instanceof Player) {
			...
			if (playerTarget.getEffectsManager().hasActiveEffect(
					EffectType.BERSERK))
				defence *= 0.5;

			double multiplier = playerTarget.getCombatDefinitions().getStats()[attackType == MELEE_TYPE ? CombatDefinitions.ARMOR_MELEE_WEAKNESS
			...
			double prayerMultiplier = playerTarget.getPrayer()
					.getDefenceRatingMultiplier();
		...
		if (from instanceof Player) {
			boolean ranging = (attackType == RANGE_TYPE);
			defence *= ((Player) from).getPrayer()
					.getEnemyDefenceRatingMultiplier();
			if ((ranging && ((Player) from).getGamePointManager()
					.hasGamePointsReward(GPR.RANGE_WEAKNESS))
					|| (!ranging && ((Player) from).getGamePointManager()
							.hasGamePointsReward(GPR.MELEE_WEAKNESS))
					|| ((Player) from).getGamePointManager()
							.hasGamePointsReward(GPR.WEAKNESS))
				defence *= 0.8;
			if (((Player) from).getGamePointManager().hasGamePointsReward(
					GPR.MAGIC_WEAKNESS)
					|| ((Player) from).getGamePointManager()
							.hasGamePointsReward(GPR.WEAKNESS))
				defence *= 0.6;
			if (((Player) from).getEquipment().containsOneItem(
					ItemIdentifiers.KALPHITE_DEFENDER,
				...
				if (((Player) from).getTemporaryAttributtes().get(
						"KalphitePassive") != null
						&& (boolean) ((Player) from).getTemporaryAttributtes()
								.get("KalphitePassive")) {
					accuracy *= 1.2;
					((Player) from).getTemporaryAttributtes().put(
							"KalphitePassive", false);
				} else
					accuracy *= 1.03;
			}
		}
		...
		if (from.getEffectsManager().hasActiveEffect(EffectType.SMOKE_EFFECT))
			accuracy *= (double) from.getEffectsManager()
					.getEffectForType(EffectType.SMOKE_EFFECT).getArguments()[0];
		if (from.getEffectsManager().hasActiveEffect(EffectType.CONFUSE_EFFECT))
			accuracy *= (double) from.getEffectsManager()
					.getEffectForType(EffectType.CONFUSE_EFFECT).getArguments()[0];
		if (from.getEffectsManager().hasActiveEffect(EffectType.STAGGER_EFFECT))
			accuracy *= (double) from.getEffectsManager()
					.getEffectForType(EffectType.STAGGER_EFFECT).getArguments()[0];
			...
			if (playerIndexes != null) {
				for (int playerIndex : playerIndexes) {
					...
					if (player == null
							...
							|| player.hasFinished()
							|| !player.isCanPvp()
							|| (from.getAttackedBy() != player && from
									.getAttackedByDelay() > Utils
									.currentTimeMillis())
							|| player.getAttackedBy() != from
							|| !from.withinDistance(player, 16))
						continue;
					return player;
				}
			}
		...
		if (npcsIndexes != null) {
			for (int npcIndex : npcsIndexes) {
				...
				if (npc == null
						|| npc.isDead()
						|| npc.hasFinished()
						|| (from.getAttackedBy() != npc && from
								.getAttackedByDelay() > Utils
								.currentTimeMillis())
						|| npc.getAttackedBy() != from
						|| !from.withinDistance(npc, 16))
					continue;
				return npc;
			}
		}
	...
	public static int getDefenceEmote(Entity target) {
		if (target instanceof NPC) {
			...
			if (weapon == null)
				return legacy ? 424 : 18346;
			int emote = weapon.getDefinitions().getCombatOpcode(
					legacy ? 4387 : 2917 /* 4371 */);
