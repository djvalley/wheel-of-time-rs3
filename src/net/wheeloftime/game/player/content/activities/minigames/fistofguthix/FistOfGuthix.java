...
public class FistOfGuthix {
	...
	private LinkedList<Player> caveMembers = new LinkedList<>();
	private LinkedList<Player> lobbyMembers = new LinkedList<>();
	private LinkedList<Participant> gameMembers = new LinkedList<>();
			...
			if (lobbyMembers().size() >= MINIMUM_NUMBER_OF_PLAYERS_NEEDED
				...
				for (Player player : lobbyMembers()) {
					...
					for (Player p2 : lobbyMembers()) {
						if (p2 == null || !lobbyMembers().contains(p2)
								|| !p2.hasStarted())
							continue;
	...
	public void enterPassageway(Player player) {
		if (caveMembers().contains(player)) {
			...
			if (!(player.getControllerManager().getController() instanceof FistOfGuthixController))
