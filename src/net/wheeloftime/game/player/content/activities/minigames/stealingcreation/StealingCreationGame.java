...
public class StealingCreationGame {
	...
	public StealingCreationGame(int areaSize, List<Player> blue,
			List<Player> red) {
		blueTeam = blue;
		redTeam = red;
		lockPeople(-1);
	...
	public void remove(Player player) {
		...
		synchronized (logicLock) {
			...
			player.getTemporaryAttributtes().put("SC_PENALTY",
					Utils.currentTimeMillis() + Helper.PENALTY_DURATION);
	...
	public boolean repairBarrier(int x, int y) {
		synchronized (logicLock) {
			...
			displayWall(area.getWallTeam(x, y) == 2 ? true : false, tier, x, y,
					newDamage);
			return true;
		}
	}
		...
		synchronized (logicLock) {
			...
			if (newHealth > 1) {
				...
				if (oldDamage != newDamage)
					displayWall(area.getWallTeam(x, y) == 2 ? true : false,
							tier, x, y, newDamage);
			} else {
	...
	public boolean useSkillPlot(int x, int y) {
		synchronized (logicLock) {
			...
			int newLook = newDegradation > 30 ? 0
					: (newDegradation > 0 ? 1 : 2);
	...
	public void updateSkillPlot(int x, int y, int look) {
		synchronized (logicLock) {
			...
			if (type < 9 || type > 12 || tier < 1 || tier > 5)
				return;
			int[] objects = (type == 9 ? Helper.ROCK_SPOTS
					: (type == 10 ? Helper.TREE_SPOTS
							: (type == 11 ? Helper.POOL_SPOTS
									: Helper.SWARM_SPOTS)))[tier - 1];
			...
			for (int testX = startX - 1; testX < (startX + 2); testX++) {
				for (int testY = startY - 1; testY < (startY + 2); testY++) {
					for (int a = 0; a < objects.length; a++) {
						WorldObject obj = World.getObjectWithId(new WorldTile(
								testX, testY, 0), objects[a]);
						if (obj != null) {
							if (a != look)
								World.spawnObject(new WorldObject(
										objects[look], obj.getType(), obj
												.getRotation(), obj));
							return;
						}
					}
	...
	public void displayWall(boolean team, int tier, int x, int y, int damage) {
		synchronized (logicLock) {
			...
			if (tier == 0) {
				World.spawnObject(new WorldObject(Helper.EMPTY_BARRIER1, 1, 1,
						new WorldTile(minX, minY, 0)));
				World.spawnObject(new WorldObject(Helper.EMPTY_BARRIER1, 1, 2,
						new WorldTile(minX, maxY, 0)));
				World.spawnObject(new WorldObject(Helper.EMPTY_BARRIER1, 1, 3,
						new WorldTile(maxX, maxY, 0)));
				World.spawnObject(new WorldObject(Helper.EMPTY_BARRIER1, 1, 0,
						new WorldTile(maxX, minY, 0)));
				for (x = minX + 1; x <= maxX - 1; x++) {
					World.spawnObject(new WorldObject(Helper.EMPTY_BARRIER1, 0,
							1, new WorldTile(x, minY, 0)));
					World.spawnObject(new WorldObject(Helper.EMPTY_BARRIER1, 0,
							3, new WorldTile(x, maxY, 0)));
				}
				for (y = minY + 1; y <= maxY - 1; y++) {
					World.spawnObject(new WorldObject(Helper.EMPTY_BARRIER1, 0,
							2, new WorldTile(minX, y, 0)));
					World.spawnObject(new WorldObject(Helper.EMPTY_BARRIER1, 0,
							0, new WorldTile(maxX, y, 0)));
				}
			} else {
				int wallID = team ? Helper.RED_BARRIER_WALLS[tier - 1][damage]
						: Helper.BLUE_BARRIER_WALLS[tier - 1][damage];
				int gate1ID = team ? Helper.RED_BARRIER_GATES[tier - 1][0]
						: Helper.BLUE_BARRIER_GATES[tier - 1][0];
				int gate2ID = team ? Helper.RED_BARRIER_GATES[tier - 1][1]
						: Helper.BLUE_BARRIER_GATES[tier - 1][1];

				World.spawnObject(new WorldObject(wallID, 0, 2, new WorldTile(
						minX + 0, minY + 1, 0)));
				World.spawnObject(new WorldObject(wallID, 0, 2, new WorldTile(
						minX + 0, minY + 4, 0)));

				World.spawnObject(new WorldObject(wallID, 0, 0, new WorldTile(
						maxX + 0, minY + 1, 0)));
				World.spawnObject(new WorldObject(wallID, 0, 0, new WorldTile(
						maxX + 0, minY + 4, 0)));

				World.spawnObject(new WorldObject(wallID, 0, 1, new WorldTile(
						minX + 1, minY + 0, 0)));
				World.spawnObject(new WorldObject(wallID, 0, 1, new WorldTile(
						minX + 4, minY + 0, 0)));

				World.spawnObject(new WorldObject(wallID, 0, 3, new WorldTile(
						minX + 1, maxY + 0, 0)));
				World.spawnObject(new WorldObject(wallID, 0, 3, new WorldTile(
						minX + 4, maxY + 0, 0)));

				// corners
				World.spawnObject(new WorldObject(wallID, 1, 1, new WorldTile(
						minX + 0, minY + 0, 0)));
				World.spawnObject(new WorldObject(wallID, 1, 2, new WorldTile(
						minX + 0, maxY + 0, 0)));
				World.spawnObject(new WorldObject(wallID, 1, 0, new WorldTile(
						maxX + 0, minY + 0, 0)));
				World.spawnObject(new WorldObject(wallID, 1, 3, new WorldTile(
						maxX + 0, maxY + 0, 0)));
				// --------
				...
				// west
				World.spawnObject(new WorldObject(gate1ID, 0, 2, new WorldTile(
						minX + 0, minY + 3, 0)));
				World.spawnObject(new WorldObject(gate2ID, 0, 2, new WorldTile(
						minX + 0, minY + 2, 0)));

				// east
				World.spawnObject(new WorldObject(gate2ID, 0, 0, new WorldTile(
						maxX + 0, minY + 3, 0)));
				World.spawnObject(new WorldObject(gate1ID, 0, 0, new WorldTile(
						maxX + 0, minY + 2, 0)));

				// south
				World.spawnObject(new WorldObject(gate1ID, 0, 1, new WorldTile(
						minX + 2, minY + 0, 0)));
				World.spawnObject(new WorldObject(gate2ID, 0, 1, new WorldTile(
						minX + 3, minY + 0, 0)));

				// north
				World.spawnObject(new WorldObject(gate2ID, 0, 3, new WorldTile(
						minX + 2, maxY + 0, 0)));
				World.spawnObject(new WorldObject(gate1ID, 0, 3, new WorldTile(
						minX + 3, maxY + 0, 0)));
	 ...
	 */
	public void updateScore(Player player, int dGathering, int dProcessing,
			int dDepositing, int dWithdrawing, int dDamaging, int dKills,
			int dDeaths) {
		synchronized (logicLock) {
	...
	public void sendItemToBase(Player player, Item item, boolean team,
			boolean BOB, boolean clicked) {
		...
		if ((base[0] == 0 && base[1] == 0)
			...
			entrance = team ? Helper.BASE_ENTRANCE_RED
					: Helper.BASE_ENTRANCE_BLUE;
		else
			entrance = Helper.PLOT_ENTRANCES[Utils
					.random(Helper.PLOT_ENTRANCES.length)];
		String name = item.getName().toLowerCase();
		int index = Helper.getItemIndex(name);
		if (clicked && index == 2 || index == -1)
			return;
		WorldTile tile = new WorldTile(getArea().getMinX() + (base[0] * 8)
				+ entrance[0], getArea().getMinY() + (base[1] * 8)
				+ entrance[1], 0);
		objectLoop: for (WorldObject object : World.getRegion(
				tile.getRegionId()).getAllObjects()) {
			if (object == null || object.getId() != 39534 + index
					|| !object.withinDistance(tile, 30))
				continue;
			int nameIndex = name.indexOf("(class");
			int clayQuality = 1;
			if (nameIndex != -1)
				clayQuality = index == 9 ? 1 : Integer.parseInt(name
						.substring(nameIndex).replace("(class ", "")
						.replace(")", ""));
			score.updateDepositing(((item.getDefinitions().isStackable() ? 1
					: 30) * clayQuality) * item.getAmount());
			if (BOB)
		...
		synchronized (logicLock) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						player.getControllerManager().startController(
								"StealingCreationGame", game, false);
			...
			for (final Player player : redTeam) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						player.getControllerManager().startController(
								"StealingCreationGame", game, true);
	...
	public void gameShutdown() {
		synchronized (logicLock) {
			final ArrayList<Score> allScores = new ArrayList<Score>(
					scores.values());
			int totalBlue = Score.totalXP(allScores, false, false);
			int totalRed = Score.totalXP(allScores, true, false);
			final int winner = totalBlue > totalRed ? 1
					: (totalBlue == totalRed ? 0 : 2);
			lockPeople(7);
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					for (Player player : blueTeam) {
						...
						Helper.sendHome(player);
						Helper.displayScores(player, false, personal,
								allScores, winner);
					...
					for (Player player : redTeam) {
						Score personal = scores.get(player);
						Helper.sendHome(player);
						Helper.displayScores(player, true, personal, allScores,
								winner);
						Helper.awardPoints(player, personal, winner);
					}
