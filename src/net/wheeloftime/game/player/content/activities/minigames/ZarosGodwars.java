...
import net.wheeloftime.game.npc.godwars.zaros.Nex.NexPhase;
...
public class ZarosGodwars {
	...
	private static final Animation START = new Animation(17412),
			SECOND_START = new Animation(17413), THIRD_START = new Animation(
					17414);
	...
	private static NexMinion[] MINIONS = new NexMinion[4];
	...
	public static void start() {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (players.isEmpty()
						|| (nex != null && nex.getTaskUID() != TASK_UID)) {
					stop();
					return;
				}
				if (stage == 0) {
					World.spawnNPC(13447, new WorldTile(2925, 5203, 0), -1,
							true, true);
					sendBeginningAction(NexPhase.values()[stage]);
				} else if (stage < 5) {
					if (MINIONS[stage - 1] == null) {
						MINIONS[stage - 1] = (NexMinion) World.spawnNPC(
								13450 + stage, MINION_SPAWNS[stage - 1], 0,
								true, true);
						NexMinion nexMinion = MINIONS[stage - 1];
						if (nex != null)
	...
	public static void incrementStage(final NexPhase lastPhase) {
		...
		if (phase == NexPhase.ZAROS)
			nex.sendFinalStage();
		if (phase != NexPhase.ZAROS)
			World.sendProjectile(nex.getMinion(phase.getPhaseValue() - 1), nex,
					2244, 18, 18, 60, 30, 0, 0);
	}

	public static void sendBeginningAction(NexPhase phase) {
		nex.setNextForceTalk(new ForceTalk(phase.getMinionName()));
		nex.setNextAnimation((phase == NexPhase.SHADOW || phase == NexPhase.ICE) ? THIRD_START
				: phase == NexPhase.START ? START : SECOND_START);
