...
import net.wheeloftime.game.npc.others.AnimatedArmor;
...
public class WarriorsGuild extends Controller {
		...
		public void run() {
			try {
				...
				if (ticks % 14 == 0) {
					...
					World.sendProjectile(CATAPULT_PROJECTILE_BASE,
							CATAPULT_TARGET, 679 + projectileType, 85, 0, 15,
							16, 15, 0);
				}
			} catch (Throwable e) {
				Logger.handle(e);
	...
	public static final int[][] ARMOUR_SET = { { 1155, 1117, 1075 },
	...
	public static boolean canEnter(Player player) {
		...
		if (player.getPrayer().isAncientCurses()
			...
			player.getPackets().sendGameMessage(
					"You cannot use curses inside the warrior's guild.");
		}
	...
	public void sendInterfaces() {
		...
		player.getInterfaceManager().sendMinigameInterface(1057);
	...
	public boolean processObjectClick1(WorldObject object) {
		if (object.getId() >= 15624 && object.getId() <= 15630) {
			...
			player.getInterfaceManager().sendCentralInterface(412);
			return false;
		} else if (object.getId() == 66604) {
			player.getInterfaceManager().sendCentralInterface(410);
			return false;
			...
			new WorldObject(object.getId(), object.getType(),
					object.getRotation() - 1, object.getX(), object.getY(),
					object.getPlane());
			// TODO: properly fix
			...
			player.addWalkSteps(object.getX(), inLobby ? object.getY()
					+ (object.getId() == 15647 ? 1 : -1) : object.getY(), 1,
					false);
			return false;
			...
			player.addWalkSteps(player.getX() == object.getX() ? object.getX()
					+ (object.getId() == 66758 ? -1 : 1) : object.getX(),
					object.getY(), 1, false);
			return false;
			...
			if (!withinArea) {
				...
				// World.spawnObjectTemporary(opened, 600);
				player.addWalkSteps(
						withinArea ? object.getX() + 1 : object.getX(),
						object.getY(), 1, false);
				inCyclopse = false;
			} else
	...
	public boolean handleItemOnObject(final WorldObject object, Item item) {
		if (object.getId() == 15621) {
			...
			int realIndex = getIndex(player, item.getId());
			if (realIndex == -1)
				return false;
			for (int armor : ARMOUR_SET[realIndex])
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					if (ticks == 0)
						...
						player.getHintIconsManager().addHintIcon(npc, 1, -1,
								false);
						player.getTemporaryAttributtes().put(
								"animator_spawned", true);
					} else if (ticks == 6) {
						player.unlock();
						stop();
						return;
				...
				if (ARMOUR_SET[primaryLoop][secondaryLoop] == checkedId) {// if
					...
					for (int equipmentId : ARMOUR_SET[primaryLoop]) {// re-loops
						...
						if (!player.getInventory().containsItem(equipmentId, 1)) {// if
																					...
																					// it
							player.getPackets().sendGameMessage(
									"You need a full set of "
											+ ARMOUR_TYPE[primaryLoop]
											+ " to use the animator.");
	...
	private void submitDummyHit(final WorldObject object) {
		player.setNextAnimation(new Animation(PlayerCombat.getAttackAnimation(
				player, true)));
		WorldTasksManager.schedule(new WorldTask() {
	...
	public boolean processButtonClick(int interfaceId, int componentId,
		...
		if (interfaceId == 411) {
			...
			if (componentId == 22) {
				...
				player.getVarsManager().sendVarBit(15404, 5);
			} else if (componentId == 23) {
				cyclopseOption = STRENGTH;
				player.getVarsManager().sendVarBit(15404, 3);
			} else if (componentId == 24) {
				cyclopseOption = COMBAT;
				player.getVarsManager().sendVarBit(15404, 4);
			} else if (componentId == 26) {
				cyclopseOption = DEFENCE;
				player.getVarsManager().sendVarBit(15404, 2);
			} else if (componentId == 25) {
				cyclopseOption = ATTACK;
				player.getVarsManager().sendVarBit(15404, 1);
			} else if (componentId == 3) {
				cyclopseOption = ALL;
			} else if (componentId == 2) {
				player.getVarsManager().sendVarBit(15404, 0);
				cyclopseOption = -1;
			}
			if (componentId == 44) {
				...
				if (cyclopseOption == -1) {
					player.getPackets()
							.sendGameMessage(
									"You must select an option before proceeding to the cyclopes room.");
					return false;
				...
				if (failure) {
					player.getPackets()
							.sendGameMessage(
									"You don't have enough points to complete this option.");
					return false;
				}
				for (int i = 0; i < 2; i++) {
					WorldObject object = new WorldObject(
							i == 1 ? 66599 : 66601, 0, 2, 2846, 3535, 2);
					if (World.isSpawnedObject(object))
						return false;
					player.lock(2);
	...
	private void sendShieldInterfaces() {
		player.getInterfaceManager().setWindowInterface(
				player.getInterfaceManager().hasRezizableScreen() ? 116 : 176,
				411);
	...
	public void prepareShotput(final byte stage, final boolean is18LB) {
		...
		if ((player.getSkills().getLevel(Skills.STRENGTH) / 100.0) > Math
				.random()) {
			player.getPackets().sendGameMessage(
