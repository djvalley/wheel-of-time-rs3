...
import net.wheeloftime.game.player.dialogues.impl.CastleWarsScoreboard;
...
public final class CastleWars {
	...
	public static final WorldTile[] ORB_TELEPORTS = {
			new WorldTile(2399, 3104, 0), new WorldTile(2411, 3125, 0),
			new WorldTile(2426, 3078, 0), new WorldTile(2384, 3084, 0),
			new WorldTile(2373, 3129, 0) };
	...
	public static void viewScoreBoard(Player player) {
		player.getInterfaceManager().sendDialogueInterface(55);
	...
	public static void joinPortal(Player player, int team) {
		...
		if (player.containsOneItem(4037, 4039)) {
			player.getPackets().sendGameMessage(
					"You cannot take flags in the arena.");
			return;
		}
		if (Consumables.getNextFoodSlot(player) != -1) {
			player.getPackets().sendGameMessage(
	...
	public static void createPlayingGame() {
		...
		GameExecutorManager.fastExecutor.scheduleAtFixedRate(playingGame,
				60000, 60000);
	...
	public static void joinPlayingGame(Player player, int team) {
		...
		player.getControllerManager()
				.startController("CastleWarsPlaying", team);
	...
	public static void handleInterfaces(Player player, int interfaceId,
		...
		if (interfaceId == 60)
			if (componentId != 1 && componentId != 5 && componentId != 9)
				interactCastleWarsTicketExchange(player, componentId,
						packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET);
	}

	public static void openCastleWarsTicketExchange(Player player) {
		player.stopAll();
		player.getInterfaceManager().sendCentralInterface(60);
	}

	public static void interactCastleWarsTicketExchange(Player player,
			int componentId, boolean buy) {
		...
		if (buy) {
			...
			if (!player.getInventory().containsItem(CW_TICKET, cost)) {
				player.getPackets().sendGameMessage(
						"You don't have enough tickets to buy this item.");
				return;
			}
			...
			player.getPackets().sendGameMessage(
					ClientScriptMap.getMap(3061).getStringValue(id));
		} else
			player.getPackets().sendGameMessage(
