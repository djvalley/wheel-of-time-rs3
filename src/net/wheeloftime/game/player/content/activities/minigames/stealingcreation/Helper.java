...
public class Helper {
	...
	public static final int[][] PLOT_ENTRANCES = new int[][] { { 2, 2 },
	...
	public static final int BLUE_DOOR_1 = 42013;
	public static final int BLUE_DOOR_2 = 42013;
	public static final int RED_DOOR_1 = 42016;
	public static final int RED_DOOR_2 = 42016;
	...
	public static final int[] BARRIER_ITEMS = new int[] { 14172, 14174, 14176,
			14178, 14180 };
	...
	public static final int[][] BLUE_BARRIER_WALLS = new int[][] {
			{ 39770, 39772, 39774, 39776 }, { 39782, 39784, 39786, 39788 },
			{ 39794, 39796, 39798, 39800 }, { 39806, 39808, 39810, 39812 },
			{ 39818, 39820, 39822, 39824 } };
	public static final int[][] RED_BARRIER_WALLS = new int[][] {
			{ 39771, 39773, 39775, 39777 }, { 39783, 39785, 39787, 39789 },
			{ 39795, 39797, 39799, 39801 }, { 39807, 39809, 39811, 39813 },
			{ 39819, 39821, 39823, 39825 } };
	public static final int[][] BLUE_BARRIER_GATES = new int[][] {
			{ 39778, 39780 }, { 3990, 39792 }, { 39802, 39804 },
			{ 39814, 39816 }, { 39826, 39828 } };
	public static final int[][] RED_BARRIER_GATES = new int[][] {
			{ 39779, 39781 }, { 3991, 39793 }, { 39803, 39805 },
			{ 39815, 39817 }, { 39827, 39829 } };
	public static final int[][] ROCK_SPOTS = new int[][] {
			{ 39548, 39548, 39549 }, { 39550, 39554, 39558 },
			{ 39551, 39555, 39559 }, { 39552, 39556, 39560 },
			{ 39553, 39557, 39561 } };
	public static final int[][] POOL_SPOTS = new int[][] {
			{ 39548, 39548, 39549 }, { 39565, 39569, 39573 },
			{ 39564, 39568, 39572 }, { 39563, 39567, 39571 },
			{ 39562, 39566, 39570 } };
	public static final int[][] SWARM_SPOTS = new int[][] {
			{ 39548, 39548, 39549 }, { 39577, 39581, 39585 },
			{ 39576, 39580, 39584 }, { 39575, 39579, 39583 },
			{ 39574, 39578, 39582 } };
	public static final int[][] TREE_SPOTS = new int[][] {
			{ 39548, 39548, 39549 }, { 39589, 39593, 39597 },
			{ 39588, 39592, 39596 }, { 39587, 39591, 39595 },
			{ 39586, 39590, 39594 } };
	...
	public static void reset(Player player) {
		...
		player.reset();
		StealingCreationController.resetFOG(player);
	}

	public static void spawn(StealingCreationGame game, Player player,
			boolean team) {
		int size = game.getArea().getSize();
		int[] base = findNearestBase(game.getArea(), player, team);
		int[] entrance;
		if ((base[0] == 0 && base[1] == 0)
				|| (base[0] == (size - 1) && base[1] == (size - 1)))
			entrance = team ? BASE_ENTRANCE_RED : BASE_ENTRANCE_BLUE;
		else
			entrance = PLOT_ENTRANCES[Utils.random(PLOT_ENTRANCES.length)];
		WorldTile tile = new WorldTile(game.getArea().getMinX() + (base[0] * 8)
				+ entrance[0], game.getArea().getMinY() + (base[1] * 8)
				+ entrance[1], 0);
	...
	public static int[] findNearestBase(GameArea area, Player player,
			boolean team) {
		int[] base = new int[2];
	...
	public static boolean withinSafeArea(Player player, GameArea area,
			boolean team) {
		...
		if (area.getType(flagX, flagY) == 1) {
			if (team) {
				if (Helper.withinArea(player, area, flagX, flagY, new int[] {
					...
					1, 1 }, 5))
				return true;
		} else if (Helper.withinArea(player, area, flagX, flagY, new int[] { 2,
				2 }))
			return true;
		return false;
	}

	public static WorldTile getNearestRespawnPoint(Player player,
			GameArea area, boolean team) {
		List<WorldObject> o = World.getRegion(player.getRegionId())
				.getAllObjects();
		if (o != null) {
			for (int[] gateIDS : (team ? Helper.RED_BARRIER_GATES
		...
		if ((base[0] == 0 && base[1] == 0)
			...
			entrance = PLOT_ENTRANCES[Utils.random(PLOT_ENTRANCES.length)];
		WorldTile tile = new WorldTile(area.getMinX() + (base[0] * 8)
				+ entrance[0], area.getMinY() + (base[1] * 8) + entrance[1], 0);
		return tile;
	}
	...
	public static int getItemIndex(String name) {
		if (name.contains("rune") || name.contains("staff"))
			...
			return 1;
		else if (name.contains("hatchet") || name.contains("pickaxe")
				|| name.contains("butterfly net") || name.contains("harpoon"))
			return 2;
		else if (name.contains("robe") || name.contains("hat"))
			return 8;
		else if (name.contains("leather body") || name.contains("coif")
				|| name.contains("chaps"))
			return 6;
		else if (name.contains("dagger") || name.contains("warhammer")
	...
	public static boolean proccessKilnItems(Player player, int componentId,
		...
		if (amount > 0 && player.getInventory().containsItem(clayId, amount)) {
			...
			player.getInventory()
					.addItem(
							new Item(
									CLASS_ITEMS_BASE[componentId - 37]
											+ ((componentId == 57
			...
			player.getPackets()
					.sendGameMessage("You have no clay to proccess.");
			return false;
		}
	}
	...
	public static void giveCape(Player player, boolean team) {
		player.getEquipment()
				.getItems()
				.set(Equipment.SLOT_CAPE, new Item(team ? RED_CAPE : BLUE_CAPE));
		player.getEquipment().refresh(Equipment.SLOT_CAPE);
		player.getAppearence().generateAppearenceData();
	}

	public static boolean withinArea(Player player, GameArea area, int flagX,
			int flagY, int[] range, int distance) {
		int minX = area.getMinX() + (flagX << 3) + Helper.BARRIER_MIN[0]
				+ range[0];
		int minY = area.getMinY() + (flagY << 3) + Helper.BARRIER_MIN[1]
				+ range[1];
		return player.withinDistance(
				new WorldTile(minX, minY, player.getPlane()), distance);
	}
	...
	public static boolean withinArea2(Player player, GameArea area, int flagX,
			int flagY, int[] range) {
		...
		if (player.withinDistance(new WorldTile(minX, minY, player.getPlane()),
				3))
			return player.withinArea(minX, minY, maxX, maxY)
	...
	public static boolean setWalkToGate(WorldObject gate, Player player) {
		...
		if (gate.getRotation() == 0)
			if (player.getX() == (gate.getX() - 1)
					&& player.getY() == gate.getY())
				return true;
			else
				return player.addWalkSteps(gate.getX(), gate.getY())
						|| player.addWalkSteps(gate.getX() - 1, gate.getY());
		else if (gate.getRotation() == 1)
			if (player.getX() == gate.getX()
					&& player.getY() == (gate.getY() + 1))
				return true;
			else
				return player.addWalkSteps(gate.getX(), gate.getY())
						|| player.addWalkSteps(gate.getX(), gate.getY() + 1);
	...
	public static boolean isAtGate(WorldObject gate, Player player) {
		if (player.getX() == gate.getX() && player.getY() == gate.getY())
			return true;
		if (gate.getRotation() == 0)
			return player.getX() == (gate.getX() - 1)
					&& player.getY() == gate.getY();
		else if (gate.getRotation() == 1)
			return player.getX() == gate.getX()
					&& player.getY() == (gate.getY() + 1);
		else if (gate.getRotation() == 2)
			return player.getX() == (gate.getX() + 1)
					&& player.getY() == gate.getY();
	...
	public static void displayClayStatus(GameArea area, Player player) {
		player.getInterfaceManager().sendCentralInterface(806);
		int basic = 0;
		...
		for (int x = 0; x < size; x++) {
			for (int y = 0; y < size; y++) {
				...
				if (degradation > 0 && type >= 9 && type <= 12 && tier > 0
						&& tier < 6) {
					if (tier == 1)
						basic++;
					else if (type == 9) {
		...
		player.getPackets().sendCSVarInteger(564, basic);
		for (int i = 0; i < fishing.length; i++)
			player.getPackets().sendCSVarInteger(565 + i, fishing[i]);
		for (int i = 0; i < mining.length; i++)
			player.getPackets().sendCSVarInteger(569 + i, mining[i]);
		for (int i = 0; i < woodcuting.length; i++)
			player.getPackets().sendCSVarInteger(573 + i, woodcuting[i]);
		for (int i = 0; i < hunter.length; i++)
			player.getPackets().sendCSVarInteger(577 + i, hunter[i]);
	}
	...
	public static void awardPoints(Player player, Score personal, int winner) {
		...
		if (points > 0) {
			player.getCompCapeManager().increaseRequirement(
					Requirement.STEALING_CREATIONS, 1);
			player.increaseStealingCreationPoints(points);
		}
		player.getPackets().sendGameMessage(
				"You earn: " + points + " reward point"
						+ (points != 1 ? "s" : "") + ".");
	}

	public static void displayScores(Player player, boolean redTeam,
			Score personal, List<Score> total, int winner) {
		player.getInterfaceManager().sendCentralInterface(810);
		...
		player.getPackets().sendCSVarInteger(598,
				redTeam ? redTotal : blueTotal); // your
													// team
		player.getPackets().sendCSVarInteger(597,
				redTeam ? blueTotal : redTotal); // enemy
													// team
		...
		player.getPackets().sendCSVarInteger(592,
				(personal.getDepositing() - personal.getWithdrawing()) * 2);
		player.getPackets().sendCSVarInteger(593, personal.getDamaging());
		player.getPackets().sendCSVarInteger(594, personal.getKilled());
		player.getPackets().sendCSVarInteger(595, personal.getDied());
		player.getPackets().sendCSVarInteger(596,
				personal.total((personal.getTeam() ? 2 : 1) == winner));
		// --------
		...
		if (mostProcessed == null || mostGathered == null
				|| mostDeposited == null || mostDamage == null
				|| mostKills == null || mostDeaths == null || highestP == null
				|| lowestP == null)
			return; // client is prepared for this, and will show required
		// messages.

		player.getPackets().sendCSVarString(45, mostGathered.getName());
		player.getPackets().sendCSVarInteger(608,
				mostGathered.getTeam() ? 2 : 1);
		player.getPackets().sendCSVarInteger(600, mostGathered.getGathering());

		player.getPackets().sendCSVarString(44, mostProcessed.getName());
		player.getPackets().sendCSVarInteger(607,
				mostProcessed.getTeam() ? 2 : 1);
		player.getPackets()
				.sendCSVarInteger(599, mostProcessed.getProcessing());

		player.getPackets().sendCSVarString(49, mostDeposited.getName());
		player.getPackets().sendCSVarInteger(612,
				mostDeposited.getTeam() ? 2 : 1);
		player.getPackets()
				.sendCSVarInteger(
						604,
						(mostDeposited.getDepositing() - mostDeposited
								.getWithdrawing()) * 2);

		player.getPackets().sendCSVarString(48, mostDamage.getName());
		player.getPackets().sendCSVarInteger(611, mostDamage.getTeam() ? 2 : 1);
		player.getPackets().sendCSVarInteger(603, mostDamage.getDamaging());

		player.getPackets().sendCSVarString(46, mostKills.getName());
		player.getPackets().sendCSVarInteger(609, mostKills.getTeam() ? 2 : 1);
		player.getPackets().sendCSVarInteger(601, mostKills.getKilled());

		player.getPackets().sendCSVarString(47, mostDeaths.getName());
		player.getPackets().sendCSVarInteger(610, mostDeaths.getTeam() ? 2 : 1);
		player.getPackets().sendCSVarInteger(602, mostDeaths.getDied());

		player.getPackets().sendCSVarString(50, highestP.getName());
		player.getPackets().sendCSVarInteger(613, highestP.getTeam() ? 2 : 1);
		player.getPackets().sendCSVarInteger(605,
				highestP.total((highestP.getTeam() ? 2 : 1) == winner));

		player.getPackets().sendCSVarString(51, lowestP.getName());
		player.getPackets().sendCSVarInteger(614, lowestP.getTeam() ? 2 : 1);
		player.getPackets().sendCSVarInteger(606,
				lowestP.total((lowestP.getTeam() ? 2 : 1) == winner));
		// --------

		player.getPackets().sendGameMessage(
				"Team 1's score(blue team): " + blueTotal
						+ " - Team 2's score(red team): " + redTotal);
		if (winner != 0 && winner == (personal.getTeam() ? 2 : 1)) {
			player.getPackets()
					.sendGameMessage(
							"Your team, Team "
									+ (personal.getTeam() ? 2 : 1)
									+ ", is victorious! You gain a 10% bonus to your score!");
		} else if (winner != 0 && winner != (personal.getTeam() ? 2 : 1)) {
			player.getPackets().sendGameMessage(
					"Your team, Team " + (personal.getTeam() ? 2 : 1)
							+ ", is defeated! Team "
							+ (personal.getTeam() ? 1 : 2) + " is victorious!");
		} else if (winner == 0) {
			player.getPackets().sendGameMessage(
					"Your team, Team " + (personal.getTeam() ? 2 : 1)
							+ ", ties with Team "
							+ (personal.getTeam() ? 1 : 2) + "!"); // TODO
			// get
			// original
			// rs
			// message
	...
	public static void displayKiln(final Player player) {
		...
		player.getTemporaryAttributtes().put("in_kiln",
	...
	public static void refreshKiln(Player player) {
		int quality = player.getTemporaryAttributtes().get("sc_kiln_quality") != null ? (int) player
				.getTemporaryAttributtes().get("sc_kiln_quality") : 4;
		for (int i = 0; i < 5; i++) {
			player.getPackets().sendCSVarInteger(KILN_CONFIG_BASE + i,
					player.getInventory().getAmountOf(SACRED_CLAY[i]));
			player.getPackets().sendHideIComponent(813,
					Helper.KILN_CLAY_COMP_BASE + (i * 2), quality != i);
		}
	}
}
