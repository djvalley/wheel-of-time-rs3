...
import net.wheeloftime.game.npc.fightpits.FightPitsNPC;
import net.wheeloftime.game.npc.fightpits.TzKekPits;
...
public final class FightPits {
				...
				if (player.hasSkull() && player.getSkullId() == 1) {// if has
					...
					if (!player.getInventory().addItem(6529, tokkul)
						...
						World.addGroundItem(new Item(6529, tokkul),
								new WorldTile(4585, 5076, 0), player, true, 180);
				}
				if (type == 1) {
					player.lock(5);
					player.addWalkSteps(4585, 5076, 5, false);
				} else if (type == 2)
					player.useStairs(-1, new WorldTile(new WorldTile(4592,
							5073, 0), 2), 0, 1);
			}
			refreshFoes();
			checkPlayersAmmount();
	...
	public static void startGame(boolean end) {
		...
		GameExecutorManager.fastExecutor.scheduleAtFixedRate(gameTask,
				end ? 60000 : 10000, 60000);
