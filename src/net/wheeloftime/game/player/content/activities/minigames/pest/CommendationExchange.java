package net.wheeloftime.game.player.content.activities.minigames.pest;
...
public class CommendationExchange {
	...
	enum CommendationStock {
		...
		ATTACK_XP(1, Skills.ATTACK | EXP_TYPE << 24, new int[] { 66, 84, 86 }), STRENGTH_XP(
				...
				new int[] { 130, 132, 134 }), RANGE_XP(1, Skills.RANGED
		...
		if (type == EXP_TYPE) {
			...
			if (stock != CommendationStock.VOID_KNIGHT_SEAL) {
				if (!player.getSkills().hasRequiriments(Skills.ATTACK, 42,
						...
						Skills.HITPOINTS, 42, Skills.RANGED, 42, Skills.MAGIC,
						42, Skills.PRAYER, 22)) {
					player.getPackets()
							.sendGameMessage(
	...
	private static void completePurchase(Player player) {
		...
		if (type == EXP_TYPE) {
			...
			if (key == 0) {
				/*
				 * if (stock == CommendationStock.HERBLORE_PACK) { packs[0] =
				 * new Item( Herbs.values()[Utils.random(5)].getHerbId() + 1,
				 * Utils.random(1, 4)); packs[1] = new
				 * Item(Herbs.values()[Utils.random(Herbs .values().length -
				 * 1)].getHerbId() + 1, Utils.random(1, 3)); } else if (stock ==
				 * CommendationStock.MINERAL_PACK) { packs[0] = new Item(441,
				 * Utils.random(1, 30)); packs[1] = new Item(454,
				 * Utils.random(1, 20)); } else if (stock ==
				 * CommendationStock.SEED_PACK) { packs[0] = new Item(
				 * Nest.SEEDS[0][Utils.random(Nest.SEEDS[0].length)],
				 * Utils.random(1, 5)); packs[1] = new Item(
				 * Nest.SEEDS[0][Utils.random(Nest.SEEDS[0].length)],
				 * Utils.random(1, 3)); packs[2] = new Item(
	...
	private static double getExperience(Player player, int skill, int rate) {
		...
		double experience = Math.pow(level, 2)
				/ ((skill == Skills.MAGIC || skill == Skills.RANGED) ? 5.195
