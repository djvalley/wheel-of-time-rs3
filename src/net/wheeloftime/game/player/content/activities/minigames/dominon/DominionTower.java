package net.wheeloftime.game.player.content.activities.minigames.dominon;
...
public final class DominionTower implements Serializable {
	...
	private static final Item[] REWARD_CHEST = new Item[] { new Item(3024, 1),
			...
			new Item(6729, 1), new Item(22346, 1), new Item(22347, 1),
	...
	public boolean hasRequiriments() {
		return player.getSkills().getCombatLevel() >= 110;
	}

	public void openSpectate() {
		player.getInterfaceManager().sendCentralInterface(1157);
	}

	public void growFace() {
		player.getPackets().sendSoundEffect(7913);
	...
	public void openModes() {
		...
		if (progress == 256) {
			player.getDialogueManager().startDialogue(
					"SimpleMessage",
					"You have some dominion fa"
							+ "ctor which you must exchange before",
		...
		player.getInterfaceManager().sendCentralInterface(1164);
	...
	public GeneralRequirementMap getMap() {
		return GeneralRequirementMap.getMap(ClientScriptMap.getMap(5213)
				.getIntValue(getNextBossIndex()));
	}
			...
			public void run() {
				try {
					...
					if (a == 1 || a == 2)
						...
						arena = PODIUM_ARENA;
					MapBuilder.copyAllPlanesMap(arena[0], arena[1],
	...
	private void teleportToArena(int mode) {
		player.setNextFaceWorldTile(new WorldTile(getBaseX() + 11,
				getBaseY() + 29, 0));
		player.getControllerManager().startController("DTController", mode);
		...
		player.getMusicsManager().playMusic(
				MUSICS[Utils.random(MUSICS.length - 1)]);
	}
			...
			public void run() {
				if (count == 0) {
					...
					player.setNextForceTalk(new ForceTalk(
							getStartFightText(Utils.random(1))));
				} else if (count == 3) {
					...
					player.getInterfaceManager().sendCentralInterface(1172);
					player.getPackets().sendHideIComponent(1172, 2, true);
					player.getPackets().sendHideIComponent(1172, 5, true);
					...
					if (nextBossIndex == 47) {
						World.addGroundItem(new Item(2402), new WorldTile(
	...
	public NPC[] createBosses() {
		...
		NPC[] bosses = new NPC[npc5 != 0 ? 5 : npc4 != 0 ? 4 : npc3 != 0 ? 3
				: npc2 != 0 ? 2 : 1];
		for (int i = 0; i < bosses.length; i++) {
			...
			bosses[i] = World.spawnNPC(id, new WorldTile(getBaseX() + 37
					+ (i * 2), getBaseY() + 31, 2), -1, true, true);
	...
	public void selectBoss() {
		if (nextBossIndex < 0) {
			nextBossIndex = (int) (long) ClientScriptMap.getMap(5213)
					.getValues().keySet().toArray()[Utils
					.random(ClientScriptMap.getMap(5213).getSize())];// Utils.random(BOSSES.length);
			if (nextBossIndex < 0)
				nextBossIndex = 1;
			player.getPackets()
					.sendExecuteScriptReverse(5435, 0, nextBossIndex);
		} else
			player.getPackets()
					.sendExecuteScriptReverse(5435, 1, nextBossIndex);
	}

	public void openClimberMode() {
		player.getPackets().sendGameMessage(
				"Only freestyle is currently working..");
	...
	public void openEnduranceMode() {
		player.getPackets().sendGameMessage(
				"Only freestyle is currently working..");
		/*
		 * selectBoss(); player.getInterfaceManager().setScreenInterface(96,
		 * 1173); // player.getPackets().sendIComponentText(1173, 25, //
		 ...
		 * player.getPackets().sendIComponentText( 1173, 31, dominionFactor ==
		 * MAX_FACTOR ? "" : String .valueOf(getBossesTotalLevel() * 10)); // /
		 * // on // win player.getPackets().sendIComponentText(1173, 33,
	...
	public int getBossesTotalLevel() {
		...
		int npc4 = map.getIntValue(2171);
		int npc5 = map.getIntValue(2172);
		int count = npc5 != 0 ? 5 : npc4 != 0 ? 4 : npc3 != 0 ? 3
				: npc2 != 0 ? 2 : 1;
		int level = 0;
		for (int i = 0; i < count; i++)
			level += NPCDefinitions
					.getNPCDefinitions(map.getIntValue(2168 + i)).combatLevel;
		return level;
	}

	public void openSpecialMode() {
		player.getPackets().sendGameMessage(
				"Only Rumble/Freestyle is currently working..");
	...
	public void talkToFace(boolean fromDialogue) {
		...
		if (!talkedWithFace)
			...
			player.getInterfaceManager().sendCentralInterface(1160);
		}
	}
	...
	public void openRewards() {
		...
		player.getPackets().sendVoice(7893);
		player.getInterfaceManager().sendCentralInterface(1156);
	}
	...
	public void openRewardsChest() {
		...
		rewards = new ItemsContainer<Item>(dominionFactor > 100000 ? 20
				: dominionFactor / 5000, false);
		for (int i = 0; i < rewards.getSize(); i++) {
			Item item = new Item(
					REWARD_CHEST[Utils.random(REWARD_CHEST.length)]);
		...
		progress = 0;
		dominionFactor = 0;
		player.getInterfaceManager().sendCentralInterface(1171);
	...
	public void viewScoreBoard() {
		player.getPackets().sendGameMessage(
				"This feature has been disabled due to rework.");
	}
