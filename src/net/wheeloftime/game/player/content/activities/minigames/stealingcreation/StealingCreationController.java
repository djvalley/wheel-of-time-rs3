...
import net.wheeloftime.game.player.*;
...
public class StealingCreationController extends Controller {
	...
	public boolean login() {
		getPlayer().getPackets().sendGameMessage(
				"How did you manage to remain here eh? REPORTED!!");
		return true;
	}

	@Override
	public void sendInterfaces() {
		player.getInterfaceManager().sendMinigameInterface(809);
		player.getPackets().sendCSVarInteger(558,
	...
	public static void resetFOG(final Player player) {
		if (player.getAppearence().isNPC()) {
	...
	public boolean canAttack(Entity target) {
		if (target instanceof Player) {
			...
			if (playerTarget.getEquipment().getCapeId() == player
					.getEquipment().getCapeId()) {
				player.getPackets().sendGameMessage(
						"You cannot attack player's on the same team!");
				return false;
			} else if (target.getTemporaryAttributtes().get("in_kiln") != null
					&& (Long) target.getTemporaryAttributtes().get("in_kiln") >= Utils
							.currentTimeMillis()) {
				player.getPackets()
						.sendGameMessage(
								"The power of the creation kiln is protecting the player.");
				return false;
			}
		} else if (target instanceof Familiar) {
			Familiar familiar = (Familiar) target;
	...
	public boolean processItemOnPlayer(Player target, Item item, int slot) {
		if (player.withinDistance(target, 3)) {
			if (target.isDead() || player.isDead()) {
				...
				return false;
			} else if (!target.getAcceptAid().isAcceptingAid(
					AcceptAid.BENEFICIAL_BOOSTS, player)) {
				player.getPackets().sendGameMessage(
		...
		for (int i = 0; i < Helper.MANAGER_NPCS.length; i++) {
			if (n.getId() == Helper.MANAGER_NPCS[i]) {
				...
				player.getDialogueManager().startDialogue(
						"StealingCreationManagerD", n, getGame());
				return false;
			}
		}
		return true;
	...
	public boolean canPlayerOption3(final Player target) {
		...
		Long PICKPOCK_DELAY = (Long) player.getTemporaryAttributtes().get(
				"PICKPOCK_DELAY");
		if (PICKPOCK_DELAY != null
				&& PICKPOCK_DELAY > Utils.currentTimeMillis()) {
			return false;
		} else if (Helper.withinSafeArea(target, game.getArea(), !getTeam())
			...
			player.getPackets()
					.sendGameMessage(
							"You cannot seem to access "
									+ target.getDisplayName()
									+ "'s pockets. He seems to be doing an action that prevents you reaching them.");
			return false;
		...
		double ratio = level
				/ (Utils.random(target.getSkills().getLevel(Skills.THIEVING)) + 3);
		if (Math.round(ratio * thievingLevel) > target.getSkills().getLevel(
			...
			target.getPackets().sendGameMessage(
					player.getDisplayName() + " has stolen a item from you.");
			player.setNextAnimation(new Animation(881));
			player.getPackets().sendGameMessage(
					"You have successfully pickpocketed an item from "
							+ Utils.formatPlayerNameForDisplay(target
	...
	public boolean keepCombating(Entity target) {
		...
		if (player.getAppearence().isNPC()) {
			player.getPackets().sendGameMessage(
					"You cannot attack while you are hidden.");
			return false;
		}
		return true;
	}
	...
	public boolean canTakeItem(FloorItem item) {
		...
		int clayQuality = 1;
		if (nameIndex != -1)
			clayQuality = name.contains("potion") ? 1 : Integer.parseInt(name
					.substring(nameIndex).replace("(class ", "")
					.replace(")", ""));
		int pointsSubtracted = 2 * (((item.getDefinitions().isStackable() ? 1
				: 15) * clayQuality) * item.getAmount());
		if (score.getWithdrawing() - pointsSubtracted <= -3000) {
			player.getPackets()
					.sendGameMessage(
							"You cannot take this amount of items as your score is too low.");
			return false;
		...
		if (interfaceId == 813) {
			if (componentId >= 37 && componentId <= 71)
				...
				if (player.getInventory().containsItem(
						Helper.SACRED_CLAY[index], 1)) {
					player.getTemporaryAttributtes().put("sc_kiln_quality",
							index);
			...
			if (item != null) {
				...
				if (itemName.contains("food (class")) {
					...
					for (String name : Skills.SKILL_NAME) {
						int doses = Integer.parseInt(itemName
								.substring(itemName.indexOf("("))
								.replace("(", "").replace(")", ""));
						String skill = superPotion ? item.getName()
	...
	private void doFoodEffect(Item item, int itemTier) {
		if (player.getFoodDelay() > Utils.currentTimeMillis())
			return;
		player.setNextAnimation(new Animation(829));
		player.addFoodDelay(1100);
		player.getPackets().sendGameMessage("You eat the food.", true);
	...
	public boolean processObjectClick1(final WorldObject object) {
		...
		gateLoop: for (int[] wallIDS : (getTeam() ? Helper.BLUE_BARRIER_GATES
		...
		wallLoop: for (int[] wallIDS : (getTeam() ? Helper.BLUE_BARRIER_WALLS
		...
		if (object.getId() == Helper.KILN) {
			...
			/*
			 * final int combatDelay =
			 * player.getCombatDefinitions().getAttackSpeed(); if
			 * (player.getActionManager().getAction() != null ||
			 * player.getActionManager().getActionDelay() > 0) return false;
			 * player.getActionManager().addActionDelay(combatDelay);
			 * player.setNextAnimation(new
			 * Animation(PlayerCombatNew.getAttackAnimation(player, true));
			 * player.setNextAnimation(new
			 * Animation(PlayerCombat.getWeaponAttackEmote(weaponId,
			 * attackStyle)));
			 */
			WorldTasksManager.schedule(new WorldTask() {
				@Override
			...
			boolean runEnergy = Utils.random(2) == 0;
			if (runEnergy)
			...
			int xExtra = 0, yExtra = 0, direction = 0, totalDistance = isWall ? 2
					: 3;
			...
			final int clayQuality = Integer.parseInt(name
					.substring(name.indexOf("(class")).replace("(class ", "")
					.replace(")", "")) - 1;
			final CreationChunks skill = CreationChunks.valueOf(name.replace(
					" (class " + (clayQuality + 1) + ")", "").toUpperCase());
			...
			player.getActionManager().setAction(new Action() {
					...
					itemLoop: for (int index = 4; index >= 0; index--) {
						...
						if (player.getEquipment().getWeaponId() == bestItem
								...
								|| player.getInventory().containsItem(
										bestItem.getId(), bestItem.getAmount())) {
				...
				public int processWithDelay(Player player) {
					...
					if (score == null)
						return -1;
					if (Utils.random(clayQuality + 1) == 0)
	...
	public boolean processObjectClick2(WorldObject object) {
		...
		gateLoop: for (int[] gateIDS : (getTeam() ? Helper.RED_BARRIER_GATES
		...
		if (object.getId() == Helper.EMPTY_BARRIER1
			...
			final int x = object.getChunkX() - (game.getArea().getMinX() >> 3);
			final int y = object.getChunkY() - (game.getArea().getMinY() >> 3);
			for (Player otherPlayer : redTeam ? game.getBlueTeam() : game
					.getRedTeam()) {
				if (otherPlayer == null
	...
	public boolean processObjectClick3(WorldObject object) {
		...
		wallLoop: for (int[] gateIDS : (getTeam() ? Helper.RED_BARRIER_WALLS
	...
	public static void passWall(Player player, WorldObject object,
			final boolean red) {
		if (player.isStunned())
			return;
		if (player.isBound()) {
			player.getPackets().sendGameMessage(
					"A mysterious force prevents you from moving.");
			return;
		}
		player.lock(3);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (step == 0) {
					WorldTile fromTile = new WorldTile(p.getX(), p.getY(), p
							.getPlane());
					...
					p.setNextForceMovement(new ForceMovement(fromTile, 0,
							faceTile, 1, Helper.getFaceDirection(faceTile, p)));
				...
				if (loop == 0) {
					...
					if (score != null && killer != null) {
						...
						killer.getPackets().sendGameMessage(
								"You have killed " + player.getDisplayName()
										+ ", you now have "
										+ killerScore.getKilled() + " kills.");
						player.getPackets().sendGameMessage(
