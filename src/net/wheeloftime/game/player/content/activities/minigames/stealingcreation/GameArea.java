...
public class GameArea {
	...
	public void calculate() {
		...
		while (skillPlots-- > 0) {
			setRandom(100, 0, 0, flags.length, flags.length,
					Utils.random(4) + 9, skillPlots == 0 ? 5 : Utils.random(5),
					Utils.random(4), 60);
		}
		...
		while (obstacles-- > 0) {
			...
			if (!setRandom(100, 0, 0, flags.length, flags.length, type, 0,
					rotation))
		...
		while (kilnsBlue-- > 0) {
			setRandom(100, 0, 0, flags.length / 2, flags.length / 2, 8, 0,
					Utils.random(4));
		}

		while (kilnsRed-- > 0) {
			setRandom(100, flags.length / 2, flags.length / 2, flags.length,
					flags.length, 8, 0, Utils.random(4));
		}
	}
		...
		for (int x = 0; x < flags.length; x++) {
			for (int y = 0; y < flags.length; y++) {
				...
				if (type >= 9 && type <= 12 && tier > 0) {
					...
					copy = EMPTY;
				MapBuilder.copyChunk(copy[0], copy[1], 0, chunkX, chunkY, 0,
						rot);
			}
		}

		World.spawnObject(new WorldObject(Helper.BLUE_DOOR_1, 0, 1, getMinX()
				+ Helper.BLUE_DOOR_P1[0], getMinY() + Helper.BLUE_DOOR_P1[1], 0));
		World.spawnObject(new WorldObject(Helper.BLUE_DOOR_2, 0, 1, getMinX()
				+ Helper.BLUE_DOOR_P2[0], getMinY() + Helper.BLUE_DOOR_P2[1], 0));
		World.spawnObject(new WorldObject(Helper.BLUE_DOOR_1, 0, 2, getMinX()
				+ Helper.BLUE_DOOR_P3[0], getMinY() + Helper.BLUE_DOOR_P3[1], 0));
		World.spawnObject(new WorldObject(Helper.BLUE_DOOR_2, 0, 2, getMinX()
				+ Helper.BLUE_DOOR_P4[0], getMinY() + Helper.BLUE_DOOR_P4[1], 0));

		World.spawnObject(new WorldObject(Helper.RED_DOOR_1, 0, 3, getMinX()
				+ ((flags.length - 1) * 8) + Helper.RED_DOOR_P1[0], getMinY()
				+ ((flags.length - 1) * 8) + Helper.RED_DOOR_P1[1], 0));
		World.spawnObject(new WorldObject(Helper.RED_DOOR_2, 0, 3, getMinX()
				+ ((flags.length - 1) * 8) + Helper.RED_DOOR_P2[0], getMinY()
				+ ((flags.length - 1) * 8) + Helper.RED_DOOR_P2[1], 0));
		World.spawnObject(new WorldObject(Helper.RED_DOOR_1, 0, 0, getMinX()
				+ ((flags.length - 1) * 8) + Helper.RED_DOOR_P3[0], getMinY()
				+ ((flags.length - 1) * 8) + Helper.RED_DOOR_P3[1], 0));
		World.spawnObject(new WorldObject(Helper.RED_DOOR_2, 0, 0, getMinX()
				+ ((flags.length - 1) * 8) + Helper.RED_DOOR_P4[0], getMinY()
				+ ((flags.length - 1) * 8) + Helper.RED_DOOR_P4[1], 0));

		int managerBlue = Helper.MANAGER_NPCS[Utils
				.random(Helper.MANAGER_NPCS.length)];
		int managerRed = Helper.MANAGER_NPCS[Utils
				.random(Helper.MANAGER_NPCS.length)];

		World.spawnNPC(
				managerBlue,
				new WorldTile(getMinX() + Helper.BLUE_MANAGER_P[0], getMinY()
						+ Helper.BLUE_MANAGER_P[1], 0), -1, false, true)
				.setRandomWalk(0);
		World.spawnNPC(
				managerRed,
				new WorldTile(getMinX() + ((flags.length - 1) * 8)
						+ Helper.RED_MANAGER_P[0], getMinY()
						+ ((flags.length - 1) * 8) + Helper.RED_MANAGER_P[1], 0),
				-1, false, true).setRandomWalk(0);
	}
	...
	public void destroy() {
		...
		MapBuilder.destroyMap(base[0] >> 3, base[1] >> 3, flags.length,
				flags.length);
		base = null;

	}

	private boolean setRandom(int attempts, int minX, int minY, int maxX,
			int maxY, int type, int tier, int rotation) {
		return setRandom(attempts, minX, minY, maxX, maxY, type, tier,
				rotation, 0);
	}

	private boolean setRandom(int attempts, int minX, int minY, int maxX,
			int maxY, int type, int tier, int rotation, int degradation) {
		while (attempts-- > 1) {
