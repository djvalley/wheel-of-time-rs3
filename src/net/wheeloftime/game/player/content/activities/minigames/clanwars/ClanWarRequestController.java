package net.wheeloftime.game.player.content.activities.minigames.clanwars;
 ...
 * 
 */
public final class ClanWarRequestController extends Controller {
		...
		player.setRouteEvent(new RouteEvent(target, new Runnable() {
			...
			public void run() {
				...
				if (target == null
						...
						|| !(target.getControllerManager().getController() instanceof ClanWarRequestController)) {
	...
	public boolean processObjectClick1(WorldObject object) {
		...
		case 38698: // Safe FFA portal.
			player.getDialogueManager().startDialogue("WarningD",
					DoomsayerManager.CLAN_WARS_WARNING_MESSAGE,
					DoomsayerManager.CLAN_WARS_SAFE_WARNING, null, null,
					new Runnable() {

						@Override
						public void run() {
							player.useStairs(-1, new WorldTile(2815, 5511, 0),
									0, 1);
		...
		case 38699: // Risk FFA portal.
			...
			player.getDialogueManager().startDialogue("WarningD",
					DoomsayerManager.CLAN_WARS_WARNING_MESSAGE,
					DoomsayerManager.CLAN_WARS_DANGEROUS_WARNING, null, null,
					new Runnable() {

						@Override
						public void run() {
							player.useStairs(-1, new WorldTile(3007, 5511, 0),
									0, 1);
							player.getControllerManager().startController(
									"clan_wars_ffa", true);
						}
	 ...
	 */
	public void stop(boolean needRemove) {
		if (needRemove)
			removeController();
	...
	private static boolean canRequest(Player player, Player target,
		...
		} else if (target.getCurrentFriendsChat() == null) {
			text = "This player is not in a friends chat.";
		} else if (player.getLastFriendsChatRank() < 3) {
			text = "You need to be a higher rank to start a war.";
		} else if (target.getLastFriendsChatRank() < 3) {
			text = "This player does not have a high enough rank to accept your challenge.";
		} else if (player.getCurrentFriendsChat() == target
				.getCurrentFriendsChat()) {
			text = "You can't challenge players in the same friends chat as you are in.";
		} else if (player.getCurrentFriendsChat().getClanWars() != null) {
			text = "Your clan is already in a war.";
		} else if (target.getCurrentFriendsChat().getClanWars() != null) {
