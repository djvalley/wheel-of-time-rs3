package net.wheeloftime.game.player.content.grandExchange;
...
public class Offer extends Item {
	...
	public boolean cancel() {
		...
		canceled = true;
		if (buying)
			receivedItems.add(new Item(995, (getAmount() - totalAmountSoFar)
					* price));
		else
			receivedItems
					.add(new Item(getId(), getAmount() - totalAmountSoFar));
		update();
		return true;
	}
	...
	public void sendUpdateWarning() {
		...
		owner.getMusicsManager().playMusicEffect(520);
		update();
	}
	...
	public boolean collectItems(int slot, int option) {
		...
		if (!defs.isStackable() && option == (item.getAmount() == 1 ? 0 : 1)) {
			...
			owner.getInventory().addItemMoneyPouch(
