...
public final class Inventory implements Serializable {
	...
	public void unlockInventory(boolean menu) {
		for (int i = 0; i < Utils
				.getInterfaceDefinitionsComponentsSize(menu ? INVENTORY_INTERFACE_2
						: INVENTORY_INTERFACE); i++) {
			player.getPackets().sendIComponentSettings(
					menu ? INVENTORY_INTERFACE_2 : INVENTORY_INTERFACE,
					menu ? 15 : 8, -1, -1, 2097152);
			player.getPackets().sendIComponentSettings(
					menu ? INVENTORY_INTERFACE_2 : INVENTORY_INTERFACE,
					menu ? 15 : 8, 0, 27, 15302030);
	...
	public void addItem(int itemId, int amount, boolean dropIfInvFull) {
		...
		if (!items.add(new Item(itemId, amount))) {
			...
			player.getPackets().sendGameMessage(
					"Not enough space in your inventory.");
			World.addGroundItem(new Item(itemId, numberToDrop), player, player,
					true, 60);
			refreshItems(itemsBefore);
			return;
		}
	...
	public boolean addItemMoneyPouch(int itemId, int amount) {
		return addItemMoneyPouch(new Item(itemId, amount));
	}
	...
	public boolean removeItemMoneyPouch(int itemId, int amount) {
		return removeItemMoneyPouch(new Item(itemId, amount));
	}
