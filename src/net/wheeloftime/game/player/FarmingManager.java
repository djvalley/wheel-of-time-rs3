...
public class FarmingManager implements Serializable {
	...
	private final static double GROW_TIME_MODIFIER = 0.3;
	...
	private static final int ALLOTMENT = 0, TREES = 1, HOPS = 2, FLOWERS = 3,
			FRUIT_TREES = 4, BUSHES = 5, HERBS = 6, COMPOST = 7, MUSHROOMS = 8,
			BELLADONNA = 9, PILLAR = 10, CALQUAT = 11, CACTUS = 12;
	private static final int RAKE = 5341, EMPTY_BUCKET = 1925;
	private static final String[] PATCH_NAMES = { "allotment", "tree", "hops",
			"flower", "fruit tree", "bush", "herb", "compost", "mushroom",
			"belladonna", "pillar", "calquat", "cactus", "null", "null", "null" };
	private static final int[][] HARVEST_AMOUNTS = { { 3, 53 }, { 1, 1 },
			{ 3, 41 }, { 1, 3 }, { 3, 5 }, { 3, 5 }, { 3, 18 }, { 0, 0 },
			{ 6, 9 }, { 1, 1 } };
		...
		if (!spot.isCleared()) {
					...
					if (info.type == COMPOST) {
						if (spot.getHarvestAmount() == 15) {
							...
							player.getPackets().sendGameMessage(
									"You close the compost bin.");
							player.getPackets().sendGameMessage(
									"The vegetation begins to decompose.");
						}
					} else
			...
			if (item != null) {
				...
				case 1:
					...
					} else if (info.type == FRUIT_TREES || info.type == CALQUAT) {
						if (spot.reachedMaxStage() && !spot.hasChecked())
							checkHealth(spot);
					...
					if (spot.isDiseased() || spot.reachedMaxStage()) {
						if (info.type == TREES) {
							if (spot.isEmpty()) // stump
								...
								player.getPackets()
										.sendGameMessage(
												"You need to chop the tree down before removing it.");
								return;
							}
						} else
							clearFarmingPatch(spot);
		...
		player.getActionManager().setAction(new Action() {
			...
			public boolean start(Player player) {
				if (!player.getInventory().containsItemToolBelt(952)) {
					player.getPackets().sendGameMessage(
							"You need a spade to harvest your crops.");
					return false;
				}
				if (spot.hasEmptyHarvestAmount() && !spot.hasGivenAmount()) {
					...
					player.getPackets()
							.sendGameMessage(
									"You have successfully harvested this patch for new crops.");
					player.setNextAnimation(new Animation(-1));
					spot.setIdle();
					return false;
				}
				player.getPackets().sendGameMessage(
						"You begin to harvest the " + patchName + " patch.");
			...
			public int processWithDelay(Player player) {
				...
				int amount = 1;
				if (spot.productInfo.type == ALLOTMENT
						|| spot.productInfo.type == HOPS
						|| spot.productInfo.type == HERBS
						&& player.getEquipment().containsOneItem(
								ItemIdentifiers.MAGIC_SECATEURS)
						&& Utils.random(10) == 1) {
					player.getPackets().sendGameMessage(
							"Your magic secateurs gain you an extra produce.");
					amount++;
				}
				player.getGamePointManager().addGamePointItem(
						new Item(spot.productInfo.productId, amount));
				return 2;
			}
			...
			public boolean process(Player player) {
				if (spot.harvestAmount > 0)
					...
					player.getPackets()
							.sendGameMessage(
									"You pick all of the "
											+ ((spot.productInfo.type == FRUIT_TREES || spot.productInfo.type == CALQUAT) ? "fruits"
													: "berries")
											+ " from the "
											+ getPatchName(spot.productInfo.type)
											+ " patch.");
					player.setNextAnimation(new Animation(-1));
					return false;
				}
			...
			public int processWithDelay(Player player) {
				player.getPackets().sendGameMessage(
						"You pick a "
								+ ItemDefinitions
										.getItemDefinitions(
												spot.productInfo.productId)
										.getName().toLowerCase() + ".");
				player.setNextAnimation(getHarvestAnimation(spot.productInfo.type));
				player.getSkills().addXp(Skills.FARMING,
						spot.productInfo.experience);
				player.getInventory()
						.addItemDrop(spot.productInfo.productId, 1);
	...
	public void clearFarmingPatch(final FarmingSpot spot) {
		...
		player.getActionManager().setAction(new Action() {
			...
			public boolean start(Player player) {
				if (!player.getInventory().containsItemToolBelt(952)) {
					player.getPackets().sendGameMessage(
							"You need a spade to clear this farming patch.");
					return false;
				}
				player.getPackets().sendGameMessage(
						"You start digging the " + patchName + " patch...");
				return true;
			}
			...
			public boolean process(Player player) {
				if (stage != 2)
					...
					player.getPackets()
							.sendGameMessage(
									"You have successfully cleared this patch for new crops.");
					player.setNextAnimation(new Animation(-1));
					spot.setIdle();
					return false;
				}
		...
		if (!player.getInventory().containsItemToolBelt(isTree ? 952 : 5343)) {
			...
			player.getPackets().sendGameMessage(
					"You don't have enough "
							...
							+ " to plant "
							+ (patchName.startsWith("(?i)[^aeiou]") ? "an"
									: "a") + " " + patchName + " patch.");
			return true;
		} else if (!player.getSkills().hasLevel(Skills.FARMING, level))
			return true;
		player.getPackets().sendGameMessage(
				"You plant the " + itemName + " in the " + patchName
						+ " patch.");
		player.setNextAnimation(isTree ? SPADE_ANIMATION
				: SEED_DIPPING_ANIMATION);
		player.getSkills().addXp(
				Skills.FARMING,
				isTree ? productInfo.experience
						: productInfo.plantingExperience);
		player.getInventory()
				.deleteItem(new Item(item.getId(), requiredAmount));
		if (player.getDailyTask() != null)
			player.getDailyTask().incrementTask(player, 3, item.getId(),
					Skills.FARMING);
	...
	public boolean startWateringAction(final FarmingSpot spot, Item item) {
		...
		if (item.getName().toLowerCase().startsWith("watering can(")) {
			...
			player.getPackets().sendGameMessage(
					"This crop is diseased and needs cure, not water!");
			return true;
		} else if (spot.isDead()) {
			player.getPackets().sendGameMessage(
					"This crop is dead and needs to be removed, not watered!");
			return true;
		}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				player.getPackets().sendGameMessage(
						"... and the patch becomes moist with water.");
				spot.refresh();
			}
		}, 2);
		return true;
	...
	public boolean startCureAction(final FarmingSpot spot, final Item item) {
		...
		if (!spot.isDiseased()) {
			player.getPackets().sendGameMessage(
					"This patch doesn't need to be cured.");
			return true;
		} else if (isTree || isBush) {
			if (!(player.getInventory().containsItemToolBelt(5329) || player
					.getInventory().containsItemToolBelt(7409))) {
				player.getPackets().sendGameMessage(
						"You need a pair of secateurs to prune the tree.");
			}
		}
		player.getPackets().sendGameMessage(
				isTree ? "You prune the "
						+ spot.productInfo.name().toLowerCase()
						+ " tree's diseased branches."
						: isBush ? "You prune the "
								+ spot.productInfo.name().toLowerCase()
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (!isTree && !isBush) {
					...
					player.getInventory().addItemDrop(new Item(229, 1));
				} else
					player.setNextAnimation(new Animation(-1));
				player.getPackets()
						.sendGameMessage("It is restored to health.");
				spot.refresh();
			}
		}, 2);
		return true;
	...
	public boolean startCompostAction(final FarmingSpot spot, final Item item,
		...
		if (spot.hasCompost()) {
			player.getPackets().sendGameMessage(
					"This patch is already saturated with a compost.");
			return true;
		} else if (!spot.isCleared()) {
			player.getPackets()
					.sendGameMessage(
							"The patch needs to be cleared in order to saturate it with compost.");
			return true;
		}
		player.getPackets().sendGameMessage(
				"You dump a bucket of "
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				player.getInventory().deleteItem(item);
				player.getInventory().addItemDrop(EMPTY_BUCKET, 1);
				player.getSkills().addXp(Skills.FARMING, 8);
				player.getPackets().sendGameMessage(
						"... and the patch becomes saturated with nutrients.");
				spot.refresh();
			}
		}, 2);
		return true;
		...
		player.getActionManager().setAction(new Action() {
			...
			public boolean start(Player player) {
				if (!checkAll(player))
					return false;
				player.getPackets().sendGameMessage(
						"You swing your hatchet at the tree...", true);
			...
			private int getWoodcuttingDelay(Player player) {
				...
				wcTimer /= player.getAuraManager()
						.getWoodcuttingAccuracyMultiplier();
				return wcTimer;
			}
		...
		player.getActionManager().setAction(new Action() {
			...
			public boolean start(Player player) {
				if (item == null
					...
					player.getPackets().sendGameMessage(
							"You cannot use this item to make compost.");
					return false;
				}
				return true;
			}
	...
	private void checkHealth(final FarmingSpot spot) {
		player.getPackets()
				.sendGameMessage(
						"You examine the "
								+ ((spot.productInfo.type == TREES
										|| spot.productInfo.type == FRUIT_TREES || spot.productInfo.type == CALQUAT) ? "tree"
										: "bush")
								+ " for signs of disease and find that it is in perfect health.");
		player.getSkills().addXp(Skills.FARMING,
				spot.productInfo.plantingExperience);
	...
	private Animation getHarvestAnimation(int type) {
		if (type == ALLOTMENT || type == HOPS || type == TREES
				|| type == MUSHROOMS || type == BELLADONNA)
			return SPADE_ANIMATION;
		else if (type == HERBS || type == FLOWERS) {
			if (player.getEquipment().getWeaponId() == 7409)
				return MAGIC_PICKING_ANIMATION;
			return type == HERBS ? HERB_PICKING_ANIMATION
					: FLOWER_PICKING_ANIMATION;
		} else if (type == FRUIT_TREES || type == CALQUAT) {
	...
	private void sendNeedsWeeding(boolean cleared) {
		player.getPackets().sendGameMessage(
				cleared ? "The patch is ready for planting."
	...
	public void resetTreeTrunks() {
		...
		spots.stream()
				.filter(spot -> spot.spotInfo.type == TREES
						|| spot.spotInfo.type == FRUIT_TREES)
				.filter(FarmingSpot::isEmpty).forEach(spot -> {
					spot.setEmpty(false);
					spot.refresh();
				});
	}
	...
	public enum ProductInfo {
		 ...
		 * Trees
		 */
		Oak(5370, 15, 6043, 1, 467.3, 14, 40, TREES), Willow(5371, 30, 6045, 6,
				1456.5, 25, 40, 6, TREES), Maple(5372, 45, 6047, 17, 3403.4,
				45, 40, 8, TREES), Yew(5373, 60, 6049, 26, 7069.9, 81, 40, 10,
				TREES), Magic(5374, 75, 6051, 41, 13768.3, 145.5, 40, 12, TREES), ELDER(
				32294, 90, 6051, 65, 23473.5, 180.5, 40, 14, TREES),

		/*
		 * int seedId, int level, int productId, int configIndex, double
		 * plantingExperience, double experience, int cycleTime, int maxStage,
		 * int stageSkip, int type
		 */

		/**
		 * Trees of the fruits :)
		 */
		Apple(5496, 27, 1955, 1, 1199.5, 22, 160, 6, FRUIT_TREES), Banana(5497,
				33, 1963, 26, 1841.5, 28, 160, 6, FRUIT_TREES), Orange(5498,
				39, 2108, 65, 2470.2, 35.5, 160, 6, FRUIT_TREES), Curry(5499,
				42, 5970, 90, 2906.9, 40, 160, 6, FRUIT_TREES), Pineapple(5500,
				51, 2114, 129, 4605.7, 57, 160, 6, FRUIT_TREES), Papaya(5501,
				57, 5972, 154, 6146.4, 72, 160, 6, FRUIT_TREES), Palm(5502, 68,
		...
		Bittercap(17825, 53, 17821, 0, 61.5, 57.7, 40, 6, 0, MUSHROOMS), Morchella(
				21620, 74, 21622, 1, 22, 77.7, 25, 6, 0, MUSHROOMS),

		Calquat(5503, 72, 5980, 0, 129.6, 48.5, 160, 8, CALQUAT),
		...
		ProductInfo(int seedId, int level, int productId, int configIndex,
			...
			this.cycleTime = new Double(cycleTime * GROW_TIME_MODIFIER)
					.intValue();
	...
	public enum SpotInfo {
		Talvery_Tree(8388, TREES), Falador_Garden_Tree(8389, TREES), Varrock_Tree(
				8390, TREES), Lumbridge_Tree(8391, TREES), Gnome_Tree(19147,
				TREES), Prif_Tree(93288, TREES), Port_Sarim_Spirit_Tree(8338,
				TREES),

		Gnome_Strong_Fruit_Tree(7962, FRUIT_TREES), Gnome_Fruit_Tree(7963,
				FRUIT_TREES), Brimhaven_Fruit_Tree(7964, FRUIT_TREES), Catherby_Fruit_Tree(
				7965, FRUIT_TREES), Lletya_Fruit_Tree(28919, FRUIT_TREES), Priff_Fruit_Tree(
				94323, FRUIT_TREES),

		Falador_Allotment_North(8550, ALLOTMENT), Falador_Allotment_South(8551,
				ALLOTMENT), Catherby_Allotment_North(8552, ALLOTMENT), Catherby_Allotment_South(
				8553, ALLOTMENT), Ardougne_Allotment_North(8554, ALLOTMENT), Ardougne_Allotment_South(
				8555, ALLOTMENT), Canfis_Allotment_North(8556, ALLOTMENT), Canfis_Allotment_South(
				8557, ALLOTMENT),

		Yannile_Hops_Patch(8173, HOPS), Talvery_Hops_Patch(8174, HOPS), Lumbridge_Hops_Patch(
				8175, HOPS), McGrubor_Hops_Patch(8176, HOPS),

		Falador_Flower(7847, FLOWERS), Catherby_Flower(7848, FLOWERS), Ardougne_Flower(
				7849, FLOWERS), Canfis_Flower(7850, FLOWERS),

		Champions_Bush(7577, BUSHES), Rimmington_Bush(7578, BUSHES), Etceteria_Bush(
				7579, BUSHES), South_Arddougne_Bush(7580, BUSHES), Priff_Bush(
				93285, BUSHES),

		Falador_Herb_Patch(8150, HERBS), Catherby_Herb_Patch(8151, HERBS), Ardougne_Herb_Patch(
				8152, HERBS), Canfis_Herb_Patch(8153, HERBS), Priff_Herb_Patch(
				93287, HERBS),

		Falador_Compost_Bin(7836, COMPOST), Catherby_Bin(7837, COMPOST), Port_Phasymatis_Bin(
				7838, COMPOST), Ardougn_Bin(7839, COMPOST), Taverly_Bin(66577,
				COMPOST),

		Priff_Pillar(94326, PILLAR), Priff_Pillar2(94325, PILLAR), Priff_Pillar3(
				94324, PILLAR), Priff_Pillar4(94327, PILLAR),
	...
	public class FarmingSpot implements Serializable {
		...
		public void process() {
			...
			while (cycleTime < Utils.currentTimeMillis()) {
				if (productInfo != null) {
					if (hasChecked()
							&& (isEmpty() || !hasMaximumRegeneration())) {
						if (isEmpty()) {
							setEmpty(false);
		...
		public void renewCycle() {
			...
			if (productInfo != null)
				cycleTime += (stage == 0 || Settings.DEBUG) ? 5000 : constant
		...
		public boolean canBeDiseased() {
			...
			return !(stage == 0 && productInfo.type != BUSHES
					|| reachedMaxStage() || isDiseased()
					|| productInfo == ProductInfo.White_lily
					|| productInfo == ProductInfo.Poison_ivy || productInfo.type == COMPOST);
		}
