...
public class Trade {
	...
	public void sendFlash(int slot) {
		player.getPackets().sendInterFlashScript(335, 0, 4, 7, slot);
		target.getPackets().sendInterFlashScript(335, 0, 4, 7, slot);
	}
	...
	public void sendOptions() {
		...
		player.getPackets().sendInterSetItemsOptionsScript(335, 26, 90, 4, 7,
				"Remove", "Remove-5", "Remove-10", "Remove-All", "Remove-X",
				"Value");
		player.getPackets().sendInterSetItemsOptionsScript(335, 29, 90, true,
				4, 7, "Value");
		player.getPackets().sendIComponentSettings(335, 26, 0, 27, 1150);
		player.getPackets().sendIComponentSettings(335, 29, 0, 27, 1026);
	...
	public void closeTrade(CloseTradeStage stage) {
		synchronized (this) {
			...
			target.getInventory().unlockInventory(true);
			if (isTrading() && target.getTrade().isTrading()) {
				...
				if (CloseTradeStage.DONE != stage) {
					...
					log(player, oldTarget, items.getItems());
					oldTarget.getTrade().items.clear();
					items.clear();
					player.getPackets().sendGameMessage(
