...
public class BarbarianOutpostAgility {
	...
	public static void climbOverCrumblingWall(final Player player,
		...
		if (player.getX() >= object.getX()) {
			player.getPackets().sendGameMessage(
					"You cannot climb the wall from this side.");
			return;
		}
			...
			public void run() {
				...
				if (stage == 3)
					...
					player.getSkillTasks().handleTask(
							AgilityCourses.BARBARIAN_OUTPOST, 1);
				}
			}
			...
			public void run() {
				if (stage == 0) {
					...
					if (getStage(player) == 1) {
						...
						player.getSkillTasks().handleTask(
								AgilityCourses.BARBARIAN_OUTPOST_ADVANCED, 1);
					}
					stop();
				}
			}
	...
	public static void WheatWalkEAST(final Player player,
			final WorldObject object) {
		if (!Agility.hasLevel(player, 1))
			return;
		player.lock(2);
		player.setNextAnimation(new Animation(6595));
		final WorldTile toTile = new WorldTile((player.getX() + 2),
				(player.getY()), player.getPlane());
		player.setNextForceMovement(new ForceMovement(player, 0, toTile, 9,
				ForceMovement.EAST));
		WorldTasksManager.schedule(new WorldTask() {
			int stage;
	...
	public static void WheatWalkNORTH(final Player player,
			final WorldObject object) {
		if (!Agility.hasLevel(player, 1))
			return;
		player.lock(2);
	...
	public static void WheatWalkSOUTH(final Player player,
			final WorldObject object) {
		if (!Agility.hasLevel(player, 1))
			return;
		player.lock(2);
	...
	public static void WheatWalkWEST(final Player player,
			final WorldObject object) {
		if (!Agility.hasLevel(player, 1))
			return;
		player.lock(2);
