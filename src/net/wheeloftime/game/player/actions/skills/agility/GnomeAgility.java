...
public class GnomeAgility {
			...
			public void run() {
				...
				if (!secondLoop) {
					...
					player.getSkillTasks().handleTask(
							AgilityCourses.GNOME_AGILITY_ADVANCED, 1);
					player.getSkills().addXp(Skills.AGILITY, 760);
					stop();
				}
	...
	public static void runGnomeBoard(final Player player,
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				player.setNextWorldTile(toTile);
				player.getSkills().addXp(Skills.AGILITY, 52);
			}
			...
			public void run() {
				...
				if (ticks == 2) {
					...
					if (getGnomeStage(player) == 5) {
						removeGnomeStage(player);
						player.getSkillTasks().handleTask(
								AgilityCourses.GNOME_AGILITY, 1);
