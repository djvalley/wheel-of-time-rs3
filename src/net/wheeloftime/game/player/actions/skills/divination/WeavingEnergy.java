...
public class WeavingEnergy extends Action {
	...
	public enum Energy {
		...
		Energy(int levelRequired, double experience,
	...
	public boolean start(Player player) {
		...
		if (!player.getInventory().containsItemToolBelt(
			...
			player.getPackets().sendGameMessage("You need "	+ energy.getItemsRequired()[0].getDefinitions().getName()+ " to create a "+ energy.getProduceEnergy().getDefinitions().getName() + ".");
			return false;
		}
		if (energy.getItemsRequired().length > 1) {
			if (!player.getInventory().containsItemToolBelt(
				...
				player.getPackets().sendGameMessage("You need "+ energy.getItemsRequired()[1].getDefinitions().getName()+ " to create a "+ energy.getProduceEnergy().getDefinitions().getName() + ".");
				return false;
			}
		}
		if (player.getSkills().getLevel(Skills.DIVINATION) < energy
				.getLevelRequired()) {
			player.getPackets().sendGameMessage("You need a divination level of at least "+ energy.getLevelRequired()+ " to weave "+ energy.getProduceEnergy().getDefinitions().getName());
			return false;
		}
		player.getPackets().sendGameMessage("You being to weave the energy and attempt to create a "+ energy.getProduceEnergy().getDefinitions().getName().toLowerCase().replace(" bar", "") + ".", true);
		return true;
	}
	...
	public boolean process(Player player) {
		...
		if (player.getSkills().getLevel(Skills.DIVINATION) < energy.getLevelRequired()) {
			player.getPackets().sendGameMessage("You need a Divination level of at least "+ energy.getLevelRequired()+ " to smelt "	+ energy.getProduceEnergy().getDefinitions().getName() +".");
			player.getInterfaceManager().removeInterface(1251);
			return false;
		}
		return true;
	...
	public int processWithDelay(Player player) {
		...
		if (maxQuantity > 1){
		SkillsDialogue.sendProgressBar(player, energy.getProduceEnergy().getId(), maxQuantity, (int) xp);
		...
		player.getPackets().sendGameMessage("You successfully weave the energy into a "	+ energy.getProduceEnergy().getDefinitions().getName().toLowerCase() + ".", true);
		if (ticks > 0) {
			return 1;
		}
		return -1;
