...
public class PrayerBooks {
	...
	private static final int[] PAGE_BASE = { 3827, 3831, 3835, 19600, 19604,
			19608 };
	...
	public static void bindPages(Player player, int bookId) {
		...
		for (int pageIndex = 0; pageIndex < 4; pageIndex++) {
			if (!player.getInventory().containsItem(PAGE_BASE[god] + pageIndex,
					1)) {
		...
		if (containsGodPages) {
			...
			player.getPackets().sendGameMessage(
					"You bind all four pages into the book.");
			player.getPrayerBook()[god] = true;
		}

	}

	public static void sermonize(Player player, final int bookId) {
		final int god = (bookId - (bookId > 5000 ? BOOKS[3] - 6 : BOOKS[0])) / 2;
		player.getDialogueManager().startDialogue(new Dialogue() {

			@Override
			public void start() {
				sendOptionsDialogue("Select a relevant passage.",
						"Wedding Ceremony", "Last Rites", "Blessings", "Preach");
			}
			...
			public void run(int interfaceId, int componentId, int slotId) {
				...
				if (ordinal == 3) {
					if (passages.length > 3)
						message = passages[3 + Utils
								.random(passages.length - 3)];
				}
