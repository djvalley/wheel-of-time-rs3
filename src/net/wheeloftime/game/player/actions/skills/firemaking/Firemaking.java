package net.wheeloftime.game.player.actions.skills.firemaking;
...
public class Firemaking extends Action {
	...
	public enum Fire {
		...
		BLISTER_WOOD(26832, 76, 900, 70766, 303.8, 1),

		ELDER(29556, 91, 1100, 70765, 403.5, 1),
	...
	public boolean start(Player player) {
		...
		if (!onGround) {
			...
			World.addGroundItem(new Item(fire.getLogId(), 1), new WorldTile(
					player), player, true, 180);
		}
	...
	public static boolean checkAll(Player player, Fire fire, boolean usingPyre) {
		...
		if (!World.isTileFree(player.getPlane(), player.getX(), player.getY(),
				1) // cliped
				|| World.getObjectWithSlot(usingPyre ? player.getFamiliar()
						: player, Region.OBJECT_SLOT_FLOOR) != null // fix
				// for
				// updated
				// api
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				player.getSkills().addXp(Skills.FIREMAKING,
						increasedExperience(player, fire.getExperience() / 2));
				player.setNextFaceWorldTile(tile);
				player.getSkillTasks().handleTask(fire, 1);
				if (player.getDailyTask() != null)
					player.getDailyTask().incrementTask(player, 3,
							fire.getLogId(), Skills.FIREMAKING);
