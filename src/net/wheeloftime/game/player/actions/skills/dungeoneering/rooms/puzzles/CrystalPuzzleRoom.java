...
public class CrystalPuzzleRoom extends PuzzleRoom {
	...
	private static final int[][] POS_DELTA = { { 1, 0 }, { 0, -1 }, { -1, 0 },
			{ 0, 1 }, };

	// Pressure plate location for each color
	private static final int[][] POS_PLATE = { { 2, 8 }, { 7, 13 }, { 12, 8 },
			{ 7, 3 }, };

	// Large center crystal lighting
	private static final int[] LARGE_CRYSTAL_OFF = { 49507, 49508, 49509,
			54275, 34866 };
	private static final int[] LARGE_CRYSTAL_FLASH = { 49510, 49511, 49512,
			54276, 35070 };
	private static final int[] LARGE_CRYSTAL_ON = { 49513, 49514, 49515, 54277,
			35231 };

	// Tile lighting
	private static final int[] TILE_INACTIVE = { 49465, 49466, 49467, 54261,
			34317 };
		...
		for (int color = 0; color < 4; color++) {
			if (Math.random() > 0.66 && activeCount++ != 3) {
				...
				World.spawnObject(new WorldObject(
						LODESTONE_ACTIVE[color][type], 10, 0, manager.getTile(
								reference, POS_BASE_LODESTONE[0] + color,
								POS_BASE_LODESTONE[1] + color)));
			} else {
				World.spawnObject(new WorldObject(
						LODESTONE_INACTIVE[color][type], 10, 0, manager
								.getTile(reference, POS_BASE_LODESTONE[0]
										+ color, POS_BASE_LODESTONE[1] + color)));
			}
		}
		task = new TileTask();
	...
	public boolean processObjectClick1(Player p, WorldObject object) {
		for (int color = 0; color < 4; color++) {
			if (object.getId() == LODESTONE_INACTIVE[color][type]) {
				if (!hasRequirement(p, Skills.MAGIC)) {
					p.getPackets().sendGameMessage(
							"You need a magic level of "
									+ getRequirement(Skills.MAGIC)
									+ " to power this lodestone.");
					return false;
				}
				...
				p.getPackets()
						.sendGameMessage(
								"You reach out and find the lodestone's power source. You spark it into life.");
				return false;
			}
		}
		return true;
	...
	private void resetPosition(int color) {
		...
		World.spawnObject(new WorldObject(TILE_ACTIVE[color][type], 22, 0,
				manager.getTile(reference, energyTile[color][0],
						energyTile[color][1])));
	}
			...
			synchronized (manager) {
				...
				outer: for (int color = 0; color < 4; color++) {
					...
					for (Player player : manager.getParty().getTeam()) {
						...
						WorldTile tile = manager.getTile(reference,
								POS_PLATE[color][0], POS_PLATE[color][1]);
						if (tile == null)
							return;
					...
					// Remove current tile
					World.spawnObject(new WorldObject(TILE_INACTIVE[type], 22,
							0, manager.getTile(reference, energyTile[color][0],
									energyTile[color][1])));

					// Rewind position if it's in the center
					if (energyTile[color][0] == POS_CENTER[0]
							&& energyTile[color][1] == POS_CENTER[1]) {
						resetPosition(color);
					} else {
						// Actually move it
						...
						World.spawnObject(new WorldObject(
								TILE_ACTIVE[color][type], 22, 0, manager
										.getTile(reference,
												energyTile[color][0],
												energyTile[color][1])));
					}
				...
				if (complete) {
					World.spawnObject(new WorldObject(LARGE_CRYSTAL_ON[type],
							10, 0, manager.getTile(reference, POS_CENTER[0],
									POS_CENTER[1])));
					setComplete();
					stop();
					task = null;
					...
					World.spawnObject(new WorldObject(
							LARGE_CRYSTAL_FLASH[type], 10, 0, manager.getTile(
									reference, POS_CENTER[0], POS_CENTER[1])));
				} else {
					World.spawnObject(new WorldObject(LARGE_CRYSTAL_OFF[type],
							10, 0, manager.getTile(reference, POS_CENTER[0],
									POS_CENTER[1])));
				}
			}
