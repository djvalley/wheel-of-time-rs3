package net.wheeloftime.game.player.actions.skills.dungeoneering.rooms.puzzles;
...
import net.wheeloftime.game.player.actions.skills.dungeoneering.rooms.puzzles.ReturnTheFlowRoom.FlowBuilder.FlowPiece;
...
public class ReturnTheFlowRoom extends PuzzleRoom {
	...
	private static final int[] PEDESTAL_FLOW = { 54114, 54115, 54116, 54117,
			37203 };

	// private static final int[] PILLAR =
	// { 54118, 54119, 54120, 54121, 37204 };
	private static final int[] PILLAR_BROKEN = { 54122, 54123, 54124, 54125,
			37207 };
	private static final int[] PILLAR_REPAIRED = { 54126, 54127, 54128, 54129,
			37219 };

	// Rubble pieces receiving no flow (each rooms seems to have a few of these
	// prespawned)
	private static final int[] RUBBLE_PIECE = { 54130, 54131, 54132, 54133,
			37220 };
	private static final int[] RUBBLE_PIECE_CLEARED = { 54134, 54135, 54136,
			54137, 37229 };
	// Cleared rubble pieces receiving flow (they still have remnants on them)
	private static final int[] RUBBLE_PIECE_FLOW = { 54138, 54139, 54140,
			54141, 37232 };
	private static final int[] RUBBLE_PIECE_CLEARED_FLOW = { 54142, 54143,
			54144, 54145, 37249 };

	// Clean straight pieces
	private static final int[] STRAIGHT_PIECE_PATH =
	// This must also be used as a base for the rubble pieces to make a groove
	// in the ground
	{ 54146, 54147, 54148, 54149, 37250 };
	private static final int[] STRAIGHT_PIECE_PATH_FLOW = { 54150, 54151,
			54152, 54153, 37251 };

	// Clean corner pieces
	private static final int[] CORNER_PIECE_PATH = { 54154, 54155, 54156,
			54157, 37252 };
	private static final int[] CORNER_PIECE_PATH_FLOW = { 54158, 54159, 54160,
			54161, 37253 };

	private static final int[][] pillars = new int[][] { { 5, 5 }, { 5, 10 },
			{ 10, 10 }, { 10, 5 } };
	...
	public void openRoom() {
		...
		for (int x = 5; x <= 10; x++) {
			for (int y = 5; y <= 10; y++) {
				WorldObject object = manager.getObjectWithType(reference, 10,
						x, y);
		...
		for (int i = 0; i < 4; i++) {
			...
			WorldObject pillar = new WorldObject(manager.getObjectWithType(
					reference, 10, pillarX, pillarY));
			if (counterClockwise) {
			...
			while (node.next != null) {
				...
				if (!broken && !node.blocked) {
					manager.spawnObject(reference,
							node.corner ? CORNER_PIECE_PATH_FLOW[type]
									: STRAIGHT_PIECE_PATH_FLOW[type], 22,
							node.rotation, 5 + node.x, 6 + node.y);
				} else {
					manager.spawnObject(reference,
							node.corner ? CORNER_PIECE_PATH[type]
									: STRAIGHT_PIECE_PATH[type], 22,
							node.rotation, 5 + node.x, 6 + node.y);
				}
				if (node.blocked) {
					if (!broken) {
						manager.spawnObject(reference, RUBBLE_PIECE_FLOW[type],
								10, node.rotation, 5 + node.x, 6 + node.y);
					} else {
						manager.spawnObject(reference, RUBBLE_PIECE[type], 10,
								node.rotation, 5 + node.x, 6 + node.y);
					}
					broken = true;
				}
	...
	@Override
	public boolean processObjectClick1(final Player player,
			final WorldObject object) {
		if (object.getId() == RUBBLE_PIECE[type]
				|| object.getId() == RUBBLE_PIECE_FLOW[type]) {
			if (!hasRequirement(player, Skills.MINING)) {
				player.getPackets().sendGameMessage(
						"You need a mining level of "
								+ getRequirement(Skills.MINING)
								+ " to min this rock.");
				return false;
			}
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					if (node.blocked) { // players might click in same tick
						...
						rubble.setId(object.getId() == RUBBLE_PIECE[type] ? RUBBLE_PIECE_CLEARED[type]
								: RUBBLE_PIECE_CLEARED_FLOW[type]);
	...
	private void startFlow(FlowPiece node) {
		while (node.next != null) {
			if (node.corner) {
				manager.spawnObject(reference, CORNER_PIECE_PATH_FLOW[type],
						22, node.rotation, 5 + node.x, 6 + node.y);
			} else if (!node.blocked) {
				manager.spawnObject(reference, STRAIGHT_PIECE_PATH_FLOW[type],
						22, node.rotation, 5 + node.x, 6 + node.y);
			} else {
				manager.spawnObject(reference, RUBBLE_PIECE_FLOW[type], 10,
						node.rotation, 5 + node.x, 6 + node.y);
				return;
			}
			node = node.next;
	...
	public class FlowBuilder {
		...
		public class FlowPiece {
			...
			public FlowPiece(int x, int y, boolean corner, int rotation,
					int dir, boolean blocked) {
				this.x = x;
				this.y = y;
				this.fromDir = dir;
				if (curr != null) {
