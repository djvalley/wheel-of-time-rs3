...
public abstract class PuzzleRoom extends VisibleRoom {
	...
	private void setLevel(int skill) {
		if (requirements[skill] == 0)
			requirements[skill] = !manager.getRoom(reference).isCritPath() ? Utils
					.random(30, skill == Skills.SUMMONING
							|| skill == Skills.PRAYER ? 100 : 106) : Math.max(
					1,
					(manager.getParty().getMaxLevel(skill) - Utils.random(10)));
	}
