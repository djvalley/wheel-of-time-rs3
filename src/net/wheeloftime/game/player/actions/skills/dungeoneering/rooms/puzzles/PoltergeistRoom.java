...
import net.wheeloftime.game.npc.dungeonnering.DungeonNPC;
...
public class PoltergeistRoom extends PuzzleRoom {
	...
	public static final int[] HERBS = { 19653, 19654, 19655, 19656, 19657,
			19658 };

	private static final int[][] CENSERS = { { -1, -1, -1 },
			{ 54095, 54099, 54103 }, { 54096, 54100, 54104 },
			{ 54097, 54101, 54105 }, { 39847, 39850, 39851 }, };

	private static final int[][] SARCOPHAGUS = { { -1, -1, -1 },
			{ 54079, 54083 }, { 54080, 54084 }, { 54081, 54085 },
			{ 39526, 39840 }, };
	...
	public void openRoom() {
		...
		poltergeist = manager.spawnNPC(reference, POLTERGEIST_ID, 5, 5, false,
				DungeonConstants.PUZZLE_NPC);
		...
		if (requiredHerb.getId() == HERBS[index]) {
			if (player.getInventory().addItem(HERBS[index], 1)) {
				...
				player.getPackets().sendGameMessage(
						"With great care, you pick a clump of the herb.");
	...
	public boolean processObjectClick1(Player player, WorldObject object) {
		...
		if (name.equals("Sarcophagus")) {
			player.getDialogueManager()
					.startDialogue(
							"SimpleMessage",
							"The inscription reads: 'Here lies Leif, posthumously honoured with the discovery of "
									+ requiredHerb.getName() + ".");
			return false;
		} else if (name.equals("Censer")
				&& object.getDefinitions().containsOption("Light")) {
			...
			if (!player.getInventory().containsItemToolBelt(
				...
				player.getPackets().sendGameMessage(
						"You need a tinderbox in order to light a censer.");
				return false;
			} else if (requiredFiremaking > player.getSkills().getLevel(
					Skills.FIREMAKING)) {
				player.getPackets().sendGameMessage(
						"You need a firemaking level of " + requiredFiremaking
								+ " to light this.");
				return false;
			}
			...
			World.spawnObject(new WorldObject(CENSERS[manager.getParty()
					.getFloorType()][2], object.getType(),
					object.getRotation(), object));
			return false;
		} else if (name.equals("Censer")
				&& object.getDefinitions().containsOption("Inspect")) {
			player.getPackets().sendGameMessage(
					"This censer would be ideal for burning something in.");
			return false;
		} else if (name.equals("Herb patch")
				&& object.getDefinitions().containsOption("Harvest")) {
			int requiredHerblore = getRequirement(Skills.HERBLORE);
			if (requiredHerblore > player.getSkills().getLevel(Skills.HERBLORE)) {
				player.getPackets().sendGameMessage(
						"You need a herblore level of " + requiredHerblore
								+ " to harvest these herbs.");
				return false;
			}
			player.getDialogueManager().startDialogue("PoltergeistFarmD", this,
					object);
			return false;
		}
		return true;
	...
	@Override
	public boolean handleItemOnObject(Player player, WorldObject object,
			Item item) {
		if (object.getDefinitions().name.equals("Censer")
				&& object.getDefinitions().containsOption("Add herb")) {
			if (item.getId() == CONSECRATED_HERB) {
				player.lock(1);
				player.getPackets().sendGameMessage(
						"You pile the herbs into the censer.");
				player.getInventory().deleteItem(item);
				World.spawnObject(new WorldObject(CENSERS[manager.getParty()
						.getFloorType()][1], object.getType(), object
						.getRotation(), object));
				return false;
			}
		}
	...
	public boolean processObjectClick2(Player player, WorldObject object) {
		if (object.getDefinitions().name.equals("Sarcophagus")) {
			if (censersLit == 4) {
				...
				if (requiredThieving > player.getSkills().getLevel(
						Skills.THIEVING)) {
					player.getPackets().sendGameMessage(
							"You need a thieving level of " + requiredThieving
									+ " to open the sarcophagus.");
					return false;
				}
				giveXP(player, Skills.THIEVING);
				player.getPackets().sendGameMessage(
						"You successfully open the sarcophagus.");
				World.spawnObject(new WorldObject(SARCOPHAGUS[manager
						.getParty().getFloorType()][1], object.getType(),
						object.getRotation(), object));
				setComplete();
				return false;
			} else {
				player.getPackets()
						.sendGameMessage("lit: " + censersLit + "/4");
				return false;
			}
		}
		return true;
	...
	public static class Poltergeist extends DungeonNPC {
		...
		public Poltergeist(int id, WorldTile tile, DungeonManager manager,
	...
	public void consecrateHerbs(Player player, int id) {
		...
		if (requiredPrayer > player.getSkills().getLevel(Skills.PRAYER)) {
			player.getPackets().sendGameMessage(
					"You need a prayer level of " + requiredPrayer
							+ " to consecrate the herbs.");
			return;
		}
