...
import net.wheeloftime.game.player.actions.skills.dungeoneering.rooms.HandledRoom;
...
public final class DungeonUtils {
	...
	public static int getBindedId(Item item) {
		...
		for (int i = 15775; i <= 16272; i++) {
			if (ItemDefinitions.getItemDefinitions(i).name.replace(" (b)", "")
					.equals(name))
				return i;
		}
	...
	public static int getGuardianCreature(int type) {
		return DungeonConstants.GUARDIAN_CREATURES[type + 1][Utils
				.random(DungeonConstants.GUARDIAN_CREATURES[type + 1].length)];
	}

	public static int getGuardianCreature() {
		return DungeonConstants.GUARDIAN_CREATURES[0][Utils
				.random(DungeonConstants.GUARDIAN_CREATURES[0].length)];
	}
	...
	public static int getHunterCreature() {
		return DungeonConstants.HUNTER_CREATURES[Utils
				.random(DungeonConstants.HUNTER_CREATURES.length)];
	}

	public static int getSlayerCreature() {
		return DungeonConstants.SLAYER_CREATURES[Utils
				.random(DungeonConstants.SLAYER_CREATURES.length)];
	}
	...
	public static Room[] selectPossibleBossRooms(int type, int complexity,
			int floorId, boolean n, boolean e, boolean s, boolean w,
			int rotation) {
		...
		for (BossRoom handledRoom : DungeonConstants.BOSS_ROOMS[type]) {
			...
			if (room.hasNorthDoor() == n && room.hasEastDoor() == e
					&& room.hasSouthDoor() == s && room.hasWestDoor() == w) {
	...
	public static boolean theresDoorTo(RoomReference checkRoom, Room[][] map) {
		if (checkRoom.getX() < 0 || checkRoom.getX() >= map.length
				|| checkRoom.getY() < 0
				|| checkRoom.getY() >= map[checkRoom.getX()].length)
			return false;
		if (checkRoom.getX() != 0
				&& map[checkRoom.getX() - 1][checkRoom.getY()] != null
				&& map[checkRoom.getX() - 1][checkRoom.getY()].hasEastDoor()) {
			return true;
		}
		if (checkRoom.getX() != map.length - 1
				&& map[checkRoom.getX() + 1][checkRoom.getY()] != null
				&& map[checkRoom.getX() + 1][checkRoom.getY()].hasWestDoor()) {
			return true;
		}
		if (checkRoom.getY() != 0
				&& map[checkRoom.getX()][checkRoom.getY() - 1] != null
				&& map[checkRoom.getX()][checkRoom.getY() - 1].hasNorthDoor()) {
			return true;
		}
		if (checkRoom.getY() != map[checkRoom.getX()].length - 1
				&& map[checkRoom.getX()][checkRoom.getY() + 1] != null
				&& map[checkRoom.getX()][checkRoom.getY() + 1].hasSouthDoor()) {
			return true;
		}
		return false;
	...
	public static Room[] selectPossibleRooms(HandledRoom[] handledRooms,
			int complexity, int floorType, boolean n, boolean e, boolean s,
			boolean w) {
		ArrayList<Room> possiblities = new ArrayList<Room>();
	...
	public static boolean isClipped(int objectId) {
		...
		for (int id : DungeonConstants.DUNGEON_DOORS)
			if (id == objectId)
				return true;
		/*
		 * for (int id : DungeonConstants.DUNGEON_SKILL_DOORS) if (id ==
		 * objectId) return true; for (int id :
		 * DungeonConstants.THIEF_CHEST_OPEN) if (id == objectId) return true;
		 * for (int id : DungeonConstants.THIEF_CHEST_LOCKED) if (id ==
		 * objectId) return true; if (objectId >=
		 * KeyDoors.ORANGE_TRIANGLE.getObjectId() && objectId <=
		 * KeyDoors.GOLD_SHIELD.getObjectId()) return true;
		 */
		return false;
	}
	...
	public static int getMaxTier(DungeonPartyManager party, int cap,
			int... skills) {
		int maxTier = cap;
