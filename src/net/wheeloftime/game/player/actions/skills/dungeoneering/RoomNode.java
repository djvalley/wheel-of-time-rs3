...
public class RoomNode {
	...
	public boolean north() {
		return children.stream().anyMatch(c -> c.y - 1 == y)
				|| (parent != null && parent.y - 1 == y);
	}

	public boolean east() {
		return children.stream().anyMatch(c -> c.x - 1 == x)
				|| (parent != null && parent.x - 1 == x);
	}
	...
	public int rotation() {
		if (parent == null) {
			throw new RuntimeException(
					"Cannot calculate rotation for base, no parent");
		}
		if (parent.y - y == -1) {
			return 0;
		}
		...
		throw new RuntimeException(
				"Cannot calculate rotation, parent is not adjacent");
	}
	...
	public List<RoomNode> getChildrenR() {
		return Stream.concat(children.stream(),
				children.stream().flatMap(r -> r.getChildrenR().stream()))
