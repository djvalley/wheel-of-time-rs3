...
public class FlipTilesRoom extends PuzzleRoom {
	...
	public void openRoom() {
		...
		for (int x = 0; x < 5; x++) {
			for (int y = 0; y < 5; y++) {
				...
				tiles[x][y] = new WorldObject(Math.random() > 0.5 ? GREEN
						: YELLOW, 22, 0, manager.getTile(reference,
						x + xOffset, y + yOffset));
	...
	public boolean processObjectClick1(Player p, WorldObject object) {
		...
		if (name.equals("Green tile") || name.equals("Yellow tile")) {
			...
			Set<WorldObject> objects = getAdjacent(pos[0] - xOffset, pos[1]
					- yOffset);
	...
	public boolean processObjectClick2(Player p, WorldObject object) {
		...
		if (name.equals("Green tile") || name.equals("Yellow tile")) {
			...
			p.applyHit(new Hit(p, (int) (p.getMaxHitpoints() * .2),
	...
	public void flipTile(final WorldObject tile) {
		...
		tile.setId(id == GREEN ? YELLOW : GREEN); // instantly update so 2
													...
													// tiles at once will not
													// bug it out, although
													// visual may be weird, rs
													// might lock the whole
													// puzzle up for 1 sec, not
													// sure tho
		for (Player team : manager.getParty().getTeam()) {
			team.getPackets().sendObjectAnimation(
					tile,
					new Animation(id == GREEN ? GREEN_TO_YELLOW
							: YELLOW_TO_GREEN));
		}
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
				World.spawnObject(new WorldObject(id == GREEN ? YELLOW : GREEN,
						22, 0, tile));
			}
		}, 1);
	}
