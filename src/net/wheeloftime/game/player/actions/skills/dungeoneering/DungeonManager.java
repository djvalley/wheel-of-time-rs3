...
import net.wheeloftime.game.npc.dungeonnering.*;
...
import net.wheeloftime.game.player.actions.skills.dungeoneering.DungeonConstants.MapRoomIcon;
...
import net.wheeloftime.game.player.actions.skills.dungeoneering.rooms.HandledPuzzleRoom;
import net.wheeloftime.game.player.actions.skills.dungeoneering.rooms.StartRoom;
import net.wheeloftime.game.player.actions.skills.dungeoneering.rooms.puzzles.PoltergeistRoom;
import net.wheeloftime.game.player.actions.skills.dungeoneering.rooms.puzzles.PoltergeistRoom.Poltergeist;
...
public class DungeonManager {
	...
	public void linkPartyToDungeon() {
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				for (Player player : party.getTeam()) {
					player.getInterfaceManager().removeOverlayInterface();
					player.getVarsManager().sendVarBit(3028, 0);
	...
	public void sendStartItems(Player player) {
		...
		if (party.getComplexity() <= 3) {
			...
			int rangeTier = DungeonUtils.getTier(player.getSkills().getLevelForXp(Skills.RANGED));
			if (rangeTier > 8)
				rangeTier = 8;
			player.getInventory()
	...
	public void sendSettings(Player player) {
		...
		if (player.getControllerManager().getController() instanceof DungeonController)
			...
			player.getControllerManager().startController("DungeonController", DungeonManager.this);
	...
	public void wearInventory(Player player) {
		...
		for (int slotId = 0; slotId < 28; slotId++) {
			...
			if (item == null)
				continue;
			if (ButtonHandler.wear(player, item.getId(), slotId, false, worn))
				worn = true;
		}
		if (worn) {
			player.getAppearence().generateAppearenceData();
	...
	public DungeonNPC spawnNPC(int id, int rotation, WorldTile tile, RoomReference reference, int type,
		...
		if (type == DungeonConstants.BOSS_NPC) {
			if (id == 9965)
				...
				n = new DungeonSkeletonBoss(id, tile, this, multiplier);
			else if (id == 10073)
	...
	public void loadRewards() {
		...
		for (Player player : party.getTeam()) {
			player.stopAll();
			double multiplier = 1;
			player.getCompCapeManager().increaseRequirement(Requirement.DUNGEONEERING, 1);
			player.getInterfaceManager().sendOverlayInterface(933);
			player.getVarsManager().sendVarBit(3028, 1);
			...
			if (GlobalEvents.isActiveEvent(Event.DUNGEONEERING)) {
				tot *= 2;
				tk *= 2;
			}
