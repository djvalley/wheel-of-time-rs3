...
public class DungeonStructure {
	...
	private List<Integer> availableKeys = IntStream.rangeClosed(0, 63).boxed()
	...
	private void generate() {
		...
		while (!queue.isEmpty()) {
			...
			// already exist
			if (next.x < 0 || next.y < 0 || next.x >= rooms.length
					|| next.y >= rooms[0].length
					|| getRoom(next.x, next.y) != null) {
				continue;
			}
		...
		minSize *= multiplier;
		if (Settings.DEBUG)
			System.out.println("max: " + maxSize + ", min: " + minSize + ", "
					+ multiplier);
		// Create gaps by removing random DE's
		int remove = rooms.length * rooms[0].length - maxSize
				+ random.nextInt(maxSize - minSize);
		for (int i = 0; i < remove; i++) {
			removeRoom(shuffledRooms().filter(r -> r.children.isEmpty())
					.findFirst().get());
		}
		...
		while (true) {
			...
			boss = shuffledRooms().filter(r -> r.children.isEmpty())
					.findFirst().get();
		...
		// way though
		setBase(shuffledRooms()
				.filter(r -> !r.isBoss
						&& r.isCritPath
						&& !(r.west() == true && r.east() == true
								&& r.north() == false && r.south() == false)
						&& !(r.west() == false && r.east() == false
								&& r.north() == true && r.south() == true))
				.findFirst().get());
		...
		rooms().filter(
				r -> !r.isBoss
						&& r.children.stream().noneMatch(c -> c.isCritPath)
						&& r.isCritPath).forEach(r -> assignKey(r, true));

		// Some extra crit locks, to make things more interesting, these may
		// 'fail' to add, but it doesn't matter
		shuffledRooms().filter(r -> !r.isBoss && r.isCritPath && r.key == -1)
				.limit((size * 2) + 1).forEach(r -> assignKey(r, true));
		...
		// already
		long bonusLockCount = rooms().filter(r -> !r.isCritPath).count() / 4
				+ random.nextInt((size + 1) * 2);
		shuffledRooms().filter(r -> !r.isBoss && r.key == -1)
				.limit(bonusLockCount).forEach(r -> assignKey(r, false));
	...
	public void assignKey(RoomNode keyRoom, boolean critLock) {
		...
		List<RoomNode> candidates = rooms().filter(
				r -> r.isCritPath == critLock && r.lock == -1).collect(
				Collectors.toList());
	...
	public List<RoomNode> getUnrelatedRooms(RoomNode room) {
		...
		while ((room = encounteredLocks.poll()) != null) {
			...
			if (room.lock != -1) {
				...
				encounteredLocks.add(rooms().filter(r -> r.key == nextLock)
						.findFirst().get());
			}
			if (room.parent != null) {
