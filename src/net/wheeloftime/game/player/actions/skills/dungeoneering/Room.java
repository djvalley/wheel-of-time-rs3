...
import net.wheeloftime.game.player.actions.skills.dungeoneering.rooms.NormalRoom;
...
public class Room {
	...
	public void removeGuardianDoors() {
		for (int i = 0; i < doors.length; i++) {
			Door door = doors[i];
			if (door != null
					&& door.getType() == DungeonConstants.GUARDIAN_DOOR)
				doors[i] = null;
		}
	}
	...
	public void removeChallengeDoors() {
		for (int i = 0; i < doors.length; i++) {
			Door door = doors[i];
			if (door != null
					&& door.getType() == DungeonConstants.CHALLENGE_DOOR)
				doors[i] = null;
		}
	}
	...
	public int getChunkXOffset(int complexity) {
		return complexity < 6 && room instanceof StartRoom ? (complexity <= 2 ? -6
				: -(5 - complexity) * 2)
				: 0;
	}

	public int getChunkYOffset(int type) {
		return (room instanceof StartRoom ? 16
				: room instanceof NormalRoom ? 48
						: room instanceof HandledPuzzleRoom ? 16 : 0)
				* type;
	}
