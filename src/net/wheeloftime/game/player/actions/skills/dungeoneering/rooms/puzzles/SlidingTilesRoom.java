...
public class SlidingTilesRoom extends PuzzleRoom {
	...
	private static final int[][] TILE_COORDS = { { 5, 9 }, { 7, 9 }, { 9, 9 },
			{ 5, 7 }, { 7, 7 }, { 9, 7 }, { 5, 5 }, { 7, 5 }, { 9, 5 }, };

	private static final int[][] VALID_MOVES = { { 1, 3 }, { 0, 2, 4 },
			{ 1, 5 }, { 0, 4, 6 }, { 1, 3, 5, 7 }, { 2, 4, 8 }, { 3, 7 },
			{ 4, 6, 8 }, { 5, 7 } };
	...
	public void openRoom() {
		...
		for (int i = 0; i < npcCount; i++) {
			...
			manager.spawnNPC(reference, DungeonUtils.getGuardianCreature(), x,
					y, true, DungeonConstants.GUARDIAN_DOOR, 0.5);
		}
		...
		for (int i = 0; i < 9; i++) {
			if (shuffledNpcOrder[i] != 0) {
				int[] coords = DungeonManager.translate(TILE_COORDS[i][0],
						TILE_COORDS[i][1], 0, 2, 2, 0);
				WorldTile base = manager.getRoomBaseTile(reference);
				tiles[i] = new SlidingTile(shuffledNpcOrder[i], new WorldTile(
						base.getX() + coords[0], base.getY() + coords[1], 0),
						manager, 0.0);
			}
		}
	}
	...
	public static boolean handleSlidingBlock(Player player, NPC npc) {
		if (!npc.getDefinitions().name.equals("Sliding block")
			...
			if (puzzle.tiles[i] == npc) {
				...
				if (i == puzzle.solveOrder[puzzle.solveIndex - 1]) {
					...
					int[] coords = DungeonManager.translate(
							...
							TILE_COORDS[puzzle.freeIndex][1], 0, 2, 2, 0);
					WorldTile base = puzzle.manager
							.getRoomBaseTile(puzzle.reference);
					npc.addWalkSteps(base.getX() + coords[0], base.getY()
							+ coords[1]);
					...
					player.getPackets()
							.sendGameMessage(
									"You strain your powers of telekenesis, but the tile just doesn't want to go there.");
					player.applyHit(new Hit(player, (int) (player
							.getMaxHitpoints() * .2), HitLook.REGULAR_DAMAGE));
					return true;
				}
			}
		}
