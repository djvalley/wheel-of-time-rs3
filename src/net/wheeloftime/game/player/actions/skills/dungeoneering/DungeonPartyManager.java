...
public final class DungeonPartyManager {
	...
	public void add(Player player) {
		synchronized (this) {
			for (Player p2 : team)
				p2.getPackets().sendGameMessage(
						player.getDisplayName() + " has joined the party.");
			team.add(player);
	...
	public void setLeader(Player player) {
		...
		player.getPackets().sendGameMessage(
				"You have been set as the party leader.");
	}
	...
	public int getDungeoneeringLevel() {
		int level = 120;
		for (Player player : team) {
			int playerLevel = player.getSkills().getLevelForXp(
	...
	public int getRangeLevel() {
		...
		int level = 0;
		for (Player player : team)
			level += player.getSkills().getLevelForXp(Skills.RANGED);
		return level / team.size();
	}
