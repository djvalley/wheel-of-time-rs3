...
public class ToxinMaze extends PuzzleRoom {
		...
		if (object.getDefinitions().name.equals("Locked chest")) {
			...
			for (Connector connector : connectors) {
				if (connector.blocked) {
					World.spawnObject(new WorldObject(-1, 10, 0, manager
							.getTile(reference, connector.x, connector.y)));
				}
			}
			if (!expired) {
				...
				player.getPackets().sendGameMessage(
						"You open the chest, but it appears to be empty.");
				return false;
			}
		}
		if (object.getId() == BARRIER_ENTRANCE && toxinTask == null
				&& !isComplete()) {
			player.getPackets()
					.sendGameMessage(
							"The barrier won't let you pass, I wonder what that lever is for...");
			return false;
		} else if (object.getId() == BARRIER
				|| object.getId() == BARRIER_ENTRANCE) {
			WorldTile in = new WorldTile(object.getX()
					+ Utils.ROTATION_DIR_X[object.getRotation()], object.getY()
					+ Utils.ROTATION_DIR_Y[object.getRotation()], 0);
			...
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					WorldTile fromTile = new WorldTile(player.getX(), player
							.getY(), player.getPlane());
					player.setNextWorldTile(target_);
					player.setNextForceMovement(new ForceMovement(fromTile, 0,
							target_, 1, Helper
									.getFaceDirection(target_, player)));
		...
		public void run() {
			if (toxinTask == null) {
				if (ticks > 0) {
					...
					manager.message(reference,
							"The ticking sound from beneath the floor is suddenly silenced.");
					manager.hideBar(reference);
				} else {
					// Maze finished after timer ran out
					manager.message(reference,
							"The toxin in the room seems to have dispersed.");
				}
				return;
			}
			ticks--;
			if (ticks > 0) {
				manager.showBar(reference, "Time remaining", (ticks
						* TICK_SPEED * 100)
						/ TOTAL_TICKS);
			} else {
				if (ticks == 0) {
					expired = true;
					manager.message(reference,
							"A toxin starts to fill the room...");
					manager.hideBar(reference);
				}
				for (Player player : manager.getParty().getTeam()) {
					if (manager.getCurrentRoomReference(player).equals(
							reference)) {
						player.applyHit(new Hit(player, (int) (player
								.getMaxHitpoints() * .1),
								HitLook.REGULAR_DAMAGE));
						player.getPackets().sendGameMessage(
								"You take damage from the toxin in the room.");
					}
				}
			}
		}
	...
	public void generate() {
			...
			for (Connector connector : connectors) {
				if (connector.blocked) {
					if (connector.left != null && connector.right != null) {
						if (connector.left.door.visited
								^ connector.right.door.visited) {
		...
		for (Connector connector : connectors) {
			if (connector.blocked) {
				World.spawnObject(new WorldObject(OBSTACLES[type][Utils
						.random(5)], 10, 0, manager.getTile(reference,
	...
	public boolean generatePath(BarrierSide current, int layer, boolean goRight) {
		 ...
		 * 
		 * @Override public int compare(BarrierSide o1, BarrierSide o2) { return
		 * o1.door.layer - o2.door.layer; } });
		 */
		for (BarrierSide option : options) {
			...
			if (!generatePath(next.otherSide, layer
					+ (layer > next.door.layer ? -1 : 1), rightFirst)) {
	...
	public void connectExit(BarrierSide curr) {
		while (true) {
			...
			if (curr.door.visited) {
				break;
			} else if (curr.layer() != 1
					&& curr.otherSide.layer() > curr.layer()) {
