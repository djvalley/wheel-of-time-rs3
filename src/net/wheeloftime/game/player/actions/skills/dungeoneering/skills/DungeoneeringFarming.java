package net.wheeloftime.game.player.actions.skills.dungeoneering.skills;
...
public class DungeoneeringFarming {
	...
	public static void initHarvest(final Player player, final Harvest harvest,
			final WorldObject object) {
		Integer harvestCount = (Integer) player.getTemporaryAttributtes().get(
				Key.HARVEST_COUNT);
		final boolean isTextile = harvest.isTextile();
		final String productName = ItemDefinitions
				.getItemDefinitions(harvest.product).getName().toLowerCase();

		if (isTextile) {
			if (player.getSkills().getLevel(Skills.FARMING) < harvest.lvl) {
				player.getPackets().sendGameMessage(
						"You need a Farming level of " + harvest.lvl
								+ " in order to pick " + productName + ".");
				return;
			}
		}
		...
		if (!isTextile && !OwnedObjectManager.isPlayerObject(player, object)) {
			player.getPackets().sendGameMessage(
					"This is not your farming patch.");
			return;
		}
		if (harvestCount == 0) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					World.spawnObject(new WorldObject(
							isTextile ? object.getId() + 1
									: DungeonConstants.EMPTY_FARMING_PATCH,
							object.getType(), object.getRotation(), object));
				}
			});
			return;
		}
	...
	public static void plantHarvest(Item item, final Player player,
			WorldObject object, DungeonManager dungeon) {
		...
		if (harvest == null)
			...
			player.getPackets().sendGameMessage(
					"You need a Farming level of "
							+ harvest.lvl
							+ " in order to plant a "
							+ ItemDefinitions.getItemDefinitions(harvest.seed)
									.getName().toLowerCase() + ".");
			return;
		}
		...
		for (int i = 0; i < objects.length; i++)
			objects[i] = new WorldObject(object.getId() + i
					+ (harvest.ordinal() * 3), object.getType(),
					object.getRotation(), new WorldTile(object));
		dungeon.getFarmKeyList().add(
				OwnedObjectManager.addOwnedObjectManager(player, objects,
						DungeonConstants.FARMING_CYCLES));
	...
	public static void clearHarvest(final Player player,
			final WorldObject object) {
		player.setNextAnimation(new Animation(3659));
		player.lock(2);
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				World.spawnObject(new WorldObject(
						DungeonConstants.EMPTY_FARMING_PATCH, object.getType(),
						object.getRotation(), object));
				player.getPackets().sendGameMessage(
						"You empty the harvest patch.");
			}
		}, 2);
	}
}
