...
public class FlowerRootsRoom extends PuzzleRoom {
	...
	private static final int[] DOOR_LEAVES = { 35838, 35839, 35842, 35847,
			35849 };
	private static final int[][][] PLANTS = {
			{
					// LARGE
					{ 35507, 35520 }, { 35523, 35525 }, { 35562, 35568 },
					{ 35569, 35576 } },
			{
					// SMALL 1
					{ 35577, 35588 }, { 35602, 35604 }, { 35606, 35609 },
					{ 35611, 35613 } },
			{
					// SMALL 2
					{ 35616, 35625 }, { 35655, 35685 }, { 35689, 35708 },
					{ 35709, 35712 } },
			{
					// SMALL 3
					{ 35715, 35718 }, { 35719, 35720 }, { 35734, 35739 },
					{ 35778, 35780 } },
			{
					// SMALL 4
					{ 35799, 35800 }, { 35804, 35808 }, { 35809, 35812 },
					{ 35830, 35835 } },
	...
	public boolean processObjectClick1(final Player player,
		...
		for (int type = 0; type < 5; type++) {
			for (int color = 0; color < 4; color++) {
				if (object.getId() == PLANTS[type][color][0]
						|| object.getId() == PLANTS[type][color][1]) { // [1]
																		// is
																		// clickable
																		// for
																		// big
					...
					if (type == 0) {
							...
							player.getPackets()
									.sendGameMessage(
											"You need a woodcutting level of "
													+ getRequirement(Skills.WOODCUTTING)
													+ " to chop down this plant.");
							return false;
						}
						HatchetDefinitions defs = Woodcutting.getHatchet(
								player, true);
						if (defs == null) {
							player.getPackets()
									.sendGameMessage(
						...
						for (Player team : manager.getParty().getTeam()) {
							team.getPackets().sendObjectAnimation(object,
									new Animation(SMALL_FLOWER_DESPAWN));
						}
						WorldTasksManager.schedule(new WorldTask() {
							...
							public void run() {
								...
								manager.spawnObject(reference, -1, 10, 0,
										coords[0], coords[1]);
	...
	public void setComplete() {
		...
		for (Player team : manager.getParty().getTeam()) {
			for (int x = 0; x < 16; x++) {
				for (int y = 0; y < 16; y++) {
					...
					if (p != null) {
						p.locked = true;
						if (p.type == 0) {
							team.getPackets().sendObjectAnimation(
									manager.getObjectWithType(reference, 10, x,
											y),
									new Animation(BIG_FLOWER_DESPAWN));
						} else {
							team.getPackets().sendObjectAnimation(
									manager.getObjectWithType(reference, 10, x,
											y),
			...
			for (WorldObject leaf : leaves) {
				team.getPackets().sendObjectAnimation(leaf,
						new Animation(LEAF_DESPAWN));
			}
		}
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			...
			synchronized (manager) {
				...
				for (int x = 0; x < 16; x++) {
					for (int y = 0; y < 16; y++) {
						...
						if (p != null && !p.locked) {
							if (p.type == 0 || odd) {
								replaceObject(manager.getObjectWithType(
										reference, 10, x, y),
										PLANTS[p.type][p.currentColor][1]);
								p.currentColor++;
								p.currentColor &= 0x3;
							}
						}
					...
					for (int y = 0; y < 16; y++) {
						...
						if (p != null && !p.locked) {
							if (p.type == 0 || odd) {
								replaceObject(manager.getObjectWithType(
										reference, 10, x, y),
										PLANTS[p.type][p.currentColor][0]);
							}
						}
					}
				}
