...
public class ColouredRecessRoom extends PuzzleRoom {
	...
	public static final int[][] LOCATIONS = { { 5, 10 }, { 10, 10 }, { 10, 5 },
			{ 5, 5 }, };
		...
		for (int i = 0; i < blocks.length; i++) {
			while_: while (true) {
				WorldTile tile = manager.getTile(reference,
						4 + Utils.random(8), 4 + Utils.random(8));
	...
	public void checkComplete() {
		...
		outer: for (Block block : blocks) {
			for (int tileColor = 0; tileColor < LOCATIONS.length; tileColor++) {
				...
				if (manager.getTile(reference, location[0], location[1])
						.matches(block)) {
	...
	public class Block extends DungeonNPC {
		...
		public void handle(final Player player, final boolean push) {
			...
			if (nPos[0] + ldx < 4 || nPos[0] + ldx > 11 || nPos[1] + ldy < 4
					|| nPos[1] + ldy > 11) {
				player.getPackets().sendGameMessage(
						"You cannot push the block there.");
				return;
			}
			final WorldTile nTarget = transform(dx, dy, 0);
			final WorldTile pTarget = player.transform(dx, dy, 0);

			if (!World.isTileFree(0, nTarget.getX(), nTarget.getY(), 1)
					|| !World.isTileFree(0, pTarget.getX(), pTarget.getY(), 1)) {
				player.getPackets().sendGameMessage(
						"Something is blocking the way.");
				return;
			}
			if (!ColouredRecessRoom.this.canMove(null, nTarget)
					|| (pull && !ColouredRecessRoom.this.canMove(null, pTarget))) {
				player.getPackets().sendGameMessage(
	...
	public boolean processObjectClick1(Player p, WorldObject object) {
		if (object.getId() == SHELVES[type]) {
			p.getDialogueManager().startDialogue("ColouredRecessShelvesD",
					(Object[]) null);
			return false;
		}
		return true;
