...
import net.wheeloftime.game.player.actions.skills.dungeoneering.rooms.BossRoom;
...
public final class Dungeon {
	...
	public static void main(String[] args) {
		...
		while (true) {
			System.out.println("TOOK: "
					+ (Utils.currentTimeMillis() - lastDung) + ", ");
			...
			for (int i = test.map[0].length - 1; i >= 0; i--) {
				for (int j = 0; j < test.map.length; j++) {
					...
					if (test.map[j][i] != null) {
						...
						/*
						 * if (test.map[j][i].getCreationIndex() ==
						 * Integer.MAX_VALUE) { s += "B"; } else { s +=
						...
						s += ")"
								+ test.map[j][i].getRoom().getDoorDirections().length
								+ " ";
						if (key != -1) {
							s += " K" + (key - 18202) / 2;
						}
						for (int l = 0; l < test.map[j][i].getRoom()
								.getDoorDirections().length; l++) {
							int lock0 = 0;// (test.map[j][i].getDoorTypes()[l]
							// >> 16 & 0xFFFF) - 50208;
							Door door = test.map[j][i].getDoor(l);
							lock0 = door != null
									&& door.getType() == DungeonConstants.KEY_DOOR ? door
									.getId() : -1;
							int rotation = (test.map[j][i].getRoom()
									.getDoorDirections()[l] + test.map[j][i]
									.getRotation()) & 0x3;
							if (lock0 >= 0 && lock0 < 64) {
			...
			for (int x = 0; x < map.length; x++) {
				if (map[x][y] != null) {
					...
					if (map[x][y].hasNorthDoor()) {
						g.drawLine(x * 20 + 30, (7 - y) * 20 + 50, x * 20 + 30,
								((7 - y) - 1) * 20 + 50);
					}
					if (map[x][y].hasEastDoor()) {
						g.drawLine(x * 20 + 30, (7 - y) * 20 + 50,
								(x + 1) * 20 + 30, ((7 - y)) * 20 + 50);
					}
			...
			for (int x = 0; x < map.length; x++) {
				if (map[x][y] != null) {
					...
					if (key != -1) {
						g.setFont(new Font("TimesRoman", Font.BOLD, 7));
						g.setColor(Color.white);
						g.drawString("\033" + (key - 18202) / 2, x * 20 + 36,
								(7 - y) * 20 + 56);
					}
					...
					for (int l = 0; l < map[x][y].getRoom().getDoorDirections().length; l++) {
						...
						if (lock0 >= 0 && lock0 < 64) {
							if (rotation == DungeonConstants.NORTH_DOOR) {
								...
								g.setFont(new Font("TimesRoman", Font.BOLD, 5));
								g.setColor(Color.black);
								g.drawString("\033" + lock0, x * 20 + 24,
										(7 - y) * 20 + 52);
							}
						}
	...
	public Dungeon(DungeonManager manager, int floorId, int complexity, int size) {
		...
		DungeonStructure structure = new DungeonStructure(size, random,
				complexity, manager == null ? true : manager.getParty()
						.isKeyShare());
		...
		children.add(base);
		long eligiblePuzzleRooms = children
				.stream()
				.filter(r -> r.children.size() > 0
						&& r.children.stream().allMatch(c -> c.lock == -1))
				.count();
		double puzzleChance = complexity < 6 ? 0 : 0.1 * children.size()
				/ eligiblePuzzleRooms;
		for (RoomNode node : children) {
			creationCount++;
			boolean puzzle = false;
			if (node == base) {
				possibilities = DungeonUtils.selectPossibleRooms(
						DungeonConstants.START_ROOMS, complexity, type,
						base.north(), base.east(), base.south(), base.west());
			} else if (node.isBoss) {
				possibilities = DungeonUtils.selectPossibleBossRooms(type,
						complexity, floorId, node.north(), node.east(),
						node.south(), node.west(), node.rotation());
			} else if (node.children.size() > 0
					&& node.children.stream().allMatch(c -> c.lock == -1)
					&& puzzleChance > random.nextDouble()) {
				puzzle = true;
				possibilities = DungeonUtils.selectPossibleRooms(
						DungeonConstants.PUZZLE_ROOMS, complexity, type,
						node.north(), node.east(), node.south(), node.west(),
						node.rotation());
			} else {
				possibilities = DungeonUtils.selectPossibleRooms(
						DungeonConstants.NORMAL_ROOMS, complexity, type,
						node.north(), node.east(), node.south(), node.west());
			}
			map[node.x][node.y] = possibilities[random
					.nextInt(possibilities.length)];
			if (node.isCritPath) {
				critCount++;
				map[node.x][node.y].setCritPath(true);
			}
			if (node.key != -1)
				map[node.x][node.y].setDropId(KeyDoors.values()[node.key]
						.getKeyId());
			for (int doorDir = 0; doorDir < map[node.x][node.y].getRoom()
					.getDoorDirections().length; doorDir++) {
				int rotation = (map[node.x][node.y].getRoom()
						.getDoorDirections()[doorDir] + map[node.x][node.y]
						.getRotation()) & 0x3;
				RoomNode neighbor = structure.getRoom(node.x
						+ Utils.ROTATION_DIR_X[rotation], node.y
						+ Utils.ROTATION_DIR_Y[rotation]);
				if (neighbor.parent == node) {
					if (puzzle) {
						map[node.x][node.y].setDoor(doorDir, new Door(
								DungeonConstants.CHALLENGE_DOOR));
					} else if (neighbor.lock != -1) {
						map[node.x][node.y].setDoor(doorDir, new Door(
								DungeonConstants.KEY_DOOR, neighbor.lock));
					} else if (complexity >= 5 && random.nextInt(3) == 0) {
						int doorIndex = random
								.nextInt(DungeonConstants.SkillDoors.values().length);
						...
						int level = manager == null ? 1
								: neighbor.isCritPath ? (manager.getParty()
										.getMaxLevel(sd.getSkillId()) - random
										.nextInt(10))
										: random.nextInt(sd.getSkillId() == Skills.SUMMONING
												|| sd.getSkillId() == Skills.PRAYER ? 100
												: 106);
						map[node.x][node.y].setDoor(doorDir, new Door(
								DungeonConstants.SKILL_DOOR, doorIndex,
								level < 1 ? 1 : level));
					} else if (complexity >= 3 && random.nextInt(2) == 0) {
						map[node.x][node.y].setDoor(doorDir, new Door(
								DungeonConstants.GUARDIAN_DOOR));
					}
	...
	public Room getRoom(RoomReference reference) {
		if (reference.getX() < 0 || reference.getY() < 0
				|| map.length <= reference.getX()
				|| map[reference.getX()].length <= reference.getY())
			return null;
