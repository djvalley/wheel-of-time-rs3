package net.wheeloftime.game.player.actions.skills.runecrafting;
...
public class SiphonActionNodes extends Action {
	...
	private static final WorldTile[][] NODE_TILES = {
			...
			{ new WorldTile(3938, 6126, 1), new WorldTile(3981, 6139, 1),
	...
	private SiphonActionNodes(Nodes nodes, WorldObject node) {
		this.nodes = nodes;
		this.node = node;
	}
	...
	public static void init() {
		for (int j = 0; j < 5; j++) {
			...
			for (int i = 0; i < (NODE_TILES[0].length + NODE_TILES[1].length); i++) {
				Nodes node = getNode(randomNodeId());
				WorldTile tile = getNodeWorldTile(node);
				World.spawnObject(new WorldObject(randomNodeId(), 10, 0, tile
						.getX(), tile.getY(), tile.getPlane(),
						getFixedLife(node)));
			}
		}
	}

	private static WorldTile getNodeWorldTile(Nodes node) {
	...
	private static int randomNodeId() {
	...
	private static int getFixedLife(Nodes node) {
		return Utils.random(node.getLevelRequired()) + 20;
	}
	...
	private void processNodeDestroy(final Player player) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				World.spawnObject(new WorldObject(randomNodeId(), node
						.getType(), node.getRotation(), tile.getX(), tile
						.getY(), tile.getPlane(), getFixedLife(nodes)));
				stop();
			}
		}, 2);
	}
	...
	public int processWithDelay(final Player player) {
		if (started) {
			if (node.getLife() == 0)
				...
				if (level < 50 ? Utils.random(2) == 1 : Utils.random(1) == 1) {
					player.getInventory()
							.addItem(
									nodes.getRuneId()[Utils.random(nodes
											.getRuneId().length)], 1);
				...
				World.sendProjectile(node, node, player, 3060, 31, 35, 35, 0,
						2, 0);
				WorldTasksManager.schedule(new WorldTask() {
					@Override
					public void run() {
