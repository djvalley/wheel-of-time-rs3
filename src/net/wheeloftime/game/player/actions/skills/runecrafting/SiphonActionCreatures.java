...
public class SiphonActionCreatures extends Action {
	...
	private static enum Creature {
		...
		private Creature(int npcId, double xp, int runeId, int npcEmoteId,
				int levelRequired, int deathEmote) {
			this.npcId = npcId;
			this.xp = xp;
			this.runeId = runeId;
	...
	public boolean checkAll(final Player player) {
		...
		if (creature.hasFinished())
			return false;
		if (!player.withinDistance(creature, 6)
				|| !player.clipedProjectile(creature, true,
						creature.getDefinitions().size)) {
	...
	public int processWithDelay(final Player player) {
		if (started) {
			...
			if (Utils.random(Utils.random(4, 6)) == 0) {
				if (player.getControllerManager().getController() != null
					...
					((RunespanController) player.getControllerManager()
							.getController()).addRunespanPoints(creatures
							.getRuneId());
