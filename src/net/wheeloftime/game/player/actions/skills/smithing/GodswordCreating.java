package net.wheeloftime.game.player.actions.skills.smithing;
...
public class GodswordCreating {
	...
	public static void joinPieces(Player player, boolean anvil) {
		if (!anvil) {
			player.getPackets()
					.sendGameMessage(
							"Those pieces of the godsword can't be joined together like that - try forging them.");
		} else {
			if (!player.getInventory().containsItemToolBelt(Smithing.HAMMER)) {
				player.getDialogueManager().startDialogue("SimpleMessage",
						"You need a hammer in order to work with shards.");
				return;
			}
			if (!player.getSkills().hasLevel(Skills.SMITHING, 80))
				return;
			if (!player.getInventory().containsItems(SHARDS)) {
				player.getDialogueManager().startDialogue("SimpleMessage",
						"You need godsword shards to forge a godsword blade.");
				return;
			}
			player.lock(2);
	...
	private static final Item[] SHARDS = new Item[] { new Item(11710, 1),
			new Item(11712, 1), new Item(11714, 1) };
	...
	public static void attachKeys(Player player) {
		if (!player.getInventory()
				.containsItems(KEYS, new int[] { 1, 1, 1, 1 })) {
			player.getPackets().sendGameMessage(
					"You need all key pieces in order to form a frozen key.");
			return;
		}
		for (int key : KEYS)
			player.getInventory().deleteItem(key, 1);
		player.getInventory().addItem(new Item(ItemIdentifiers.FROZEN_KEY, 1));
		player.getPackets().sendGameMessage(
				"You attach the key pieces together to form a frozen key.");
	}
	...
	public static void attachHilt(Player player, int gs) {
		...
		player.getPackets().sendGameMessage(
				"You attach the hilt to the blade and make a "
						+ ItemDefinitions.getItemDefinitions(gsId).name + ".");
	}
