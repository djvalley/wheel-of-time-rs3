...
class FallenStars {
	...
	private static final int STAR_SMALL = 0;
	private static final int STAR_MEDIUM = 1;
	private static final int STAR_BIG = 2;
	private static final int STAR_HUGE = 3;

	private static final int[] SELECTABLE_XP_STARS = new int[] { 30550, 30523,
			29923, 29896 };
	private static final int[] SELECTABLE_XP_STARS_TYPES = new int[] {
			STAR_SMALL, STAR_MEDIUM, STAR_BIG, STAR_HUGE };
	private static final int[][] FALLEN_STAR_IDS = new int[][] {
	...
	private static boolean isSelectable(int id) {
		for (int SELECTABLE_XP_STAR : SELECTABLE_XP_STARS) {
			if (SELECTABLE_XP_STAR == id)
				return true;
		}
		return false;
	}
	...
	private static boolean isStarLamp(int id) {
		for (int[] FALLEN_STAR_ID : FALLEN_STAR_IDS) {
			for (int i = 0; i < FALLEN_STAR_ID.length; i++) {
				if (FALLEN_STAR_ID[i] == id)
					return true;
			}
		}
		return false;
