package net.wheeloftime.game.player.actions.skills.construction;
...
import net.wheeloftime.game.DynamicRegion;
...
import net.wheeloftime.game.npc.construction.Guard;
import net.wheeloftime.game.npc.construction.ServantNPC;
import net.wheeloftime.game.player.CompletionistCapeManager.Requirement;
import net.wheeloftime.game.player.FriendsIgnores;
import net.wheeloftime.game.player.Player;
import net.wheeloftime.game.player.Skills;
import net.wheeloftime.game.player.actions.skills.construction.HouseConstants.Builds;
import net.wheeloftime.game.player.actions.skills.construction.HouseConstants.HObject;
import net.wheeloftime.game.player.actions.skills.construction.HouseConstants.POHLocation;
import net.wheeloftime.game.player.actions.skills.construction.HouseConstants.Roof;
import net.wheeloftime.game.player.actions.skills.construction.HouseConstants.Room;
import net.wheeloftime.game.player.actions.skills.construction.HouseConstants.Servant;
import net.wheeloftime.game.player.controllers.Controller;
import net.wheeloftime.game.player.controllers.HouseController;
 ...
 * HouseController provides support between player interaction inside house and house manager
...
public class House implements Serializable {
	...
	public static int TELEPORTED = 2;
	 ...
	 */
	private void openRoomCreationMenu( int roomX, int roomY, int plane ) {
		if (!buildMode) {
			player.getDialogueManager().startDialogue("SimpleMessage",
		...
		if (room != null) {
			...
			if (room.room == Room.THRONE_ROOM && room.plane == 1) {
				...
				if (bellow != null && bellow.room == Room.OUTBLIETTE) {
					player.getDialogueManager()
							.startDialogue("SimpleMessage",
									"You can't remove a throne room that is supporting a oubliette.");
					return;
				}
			}
			...
			for (int index = 0; index < HouseConstants.Room.values().length - 2; index++) {
				...
				if (player.getSkills().hasLevel(Skills.CONSTRUCTION, refRoom.getLevel())
	...
	public void createRoom(int slot) {
		...
		if (roomsR.size() >= getMaxQuantityRooms()) {
			player.getPackets().sendGameMessage(
					"You have reached the maximum quantity of rooms.");
			return;
		}
	...
	private int getMaxQuantityRooms() {
		int consLvl = player.getSkills().getLevel(Skills.CONSTRUCTION);
		int maxRoom = 20;
	...
	public void createRoom(RoomReference room) {
		...
		player.getCompCapeManager().increaseRequirement(Requirement.HOUSE_BUILDING, room.room.getPrice());
		roomsR.add(room);
		refreshNumberOfRooms();
		refreshHouse();
	}
	...
	public void openBuildInterface(WorldObject object, final Builds build) {
		...
		player.getPackets().sendCSVarInteger(841, requirementsValue);
	...
	private boolean hasRequirementsToBuild( boolean warn, Builds build, HObject piece) {
		int boost = 0;
		if (!build.isWater() && player.getInventory().containsItemToolBelt(ItemIdentifiers.CRYSTAL_SAW))
			boost += 3;
		if (!player.getSkills().hasLevel(Skills.CONSTRUCTION, 3, piece.getLevel()))
			return false;
		if (player.getRights() < 2) {
			if (!player.getInventory().containsItems(piece.getRequirements())) {
				if (warn)
					player.getPackets().sendGameMessage("You don't have the right materials.");
				return false;
			}
			if (build.isWater() ? !hasWaterCan()
					: (!player.getInventory().containsItemToolBelt(
							...
							.getInventory().containsItemToolBelt(ItemIdentifiers.CRYSTAL_SAW)))) {
				if (warn)
					player.getPackets()
							.sendGameMessage(
	...
	private void refreshChallengeMode() {
		...
		for (RoomReference reference : roomsR) {
			if (reference.plane != 0)
				continue;
			reference.objects.stream( ).filter( o -> o.getPiece( ).getNPCId( ) != -1 ).forEach( o -> refreshObject( reference, o, !remove, true ) );
		}
		if (remove)
			clearChallengeNPCs();
		else {
	...
	private void clearChallengeNPCs() {
		for (RoomReference rref : roomsR) {
			...
			challengeModeNPCs.forEach( NPC:: finish );
	...
	public boolean isDoor(WorldObject object) {
		return object.getDefinitions().name.equalsIgnoreCase("Door hotspot") || object.getId() == 15313 || object.getId() == 15314 || object.getId() == 15317;
	}
	...
	private boolean isDoorSpace( WorldObject object ) {
		return object.getDefinitions().name.equalsIgnoreCase("Door space");
	}

	private boolean isWindowSpace( WorldObject object ) {
	...
	public void switchLock(Player player) {
		...
		if (locked)
			player.getDialogueManager().startDialogue("SimpleMessage",
					"Your house is now locked to all visitors.");
		else if (buildMode)
			player.getDialogueManager()
					.startDialogue("SimpleMessage",
			...
			player.getDialogueManager().startDialogue("SimpleMessage",
					"Visitors can now enter your house.");
	}
	...
	public static void enterHouse(Player player, String displayname) {
		...
		if (!player.withinDistance(new WorldTile(1355, 7304, 60)) && !player.withinDistance(owner.getHouse().location.getTile(), 16) && !player.withinDistance(Settings.HOME_LOCATION, 150)) {
			player.getPackets()
					.sendGameMessage(
							"This house is at "
	...
	private boolean joinHouse( final Player player ) {
		...
		if (loaded) {
			...
			GameExecutorManager.slowExecutor.execute( ( ) -> {
				try { // sets bounds before finishing load therefore the
						// load boolean
					boundChuncks = MapBuilder.findEmptyChunkBound(64, 64);
					createHouse(true);
				} catch (Throwable e) {
					Logger.handle(e);
				}
	...
	public void finish() {
		kickGuests();
		// no need to leavehouse for owner, Controller does that itself
	}
	...
	private void sendStartInterface( Player player ) {
		player.lock();
	...
	private RoomReference getPortalRoom( ) {
		for (RoomReference room : roomsR) {
			if (room.room == HouseConstants.Room.GARDEN
					|| room.room == HouseConstants.Room.FORMAL_GARDEN) {
				for (ObjectReference o : room.objects)
	...
	private boolean addRoom( ) {
		return roomsR.add(new RoomReference( Room.GARDEN, 3, 3, 1, 0 ));
	}
	...
	private void refreshNumberOfRooms( ) {
	...
	private void refreshArriveInPortal( ) {
	...
	private RoomReference getRoom( int x, int y, int plane ) {
		for (RoomReference room : roomsR)
	...
	public void createHouse(final boolean tp) {
		...
		if (spawnedObjects != null) {
			spawnedObjects.stream( ).filter( object -> object != null ).forEach( World:: removeObject );
		}
	...
	private static boolean isDungeon( Room room ) {
