package net.wheeloftime.game.player.actions.skills.herblore;
...
public class WeaponPoison {
	...
	private static final int[][] POISON = new int[][] { { 187, 5937, 5940 },
			{ 17572, 17596, 17620 } };
	...
	public static boolean poison(Player player, Item item1, Item item2,
		...
		for (int i = 0; i < POISON[dungeoneering ? 1 : 0].length; i++) {
			Item weapon = InventoryOptionsHandler.contains(
					POISON[dungeoneering ? 1 : 0][i], item1, item2);
			if (weapon == null)
				continue;
	...
	private static boolean poison(Player player, Item item, int type,
			boolean dungeoneering) {
		...
		player.getInventory()
				.deleteItem(POISON[dungeoneering ? 1 : 0][type], 1);
		player.getInventory()
				.addItem(
						dungeoneering ? Drinkables.DUNGEONEERING_VIAL
								: Drinkables.VIAL, 1);
		player.getPackets().sendGameMessage(
				"You dip the tip of the " + item.getName()
						+ " in the weapon poison.");
		return true;
	}
