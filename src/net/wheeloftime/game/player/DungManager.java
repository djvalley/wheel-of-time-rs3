...
public class DungManager implements Serializable {
	...
	public boolean enterResourceDungeon(WorldObject object) {
		...
		for (ResourceDungeon dung : ResourceDungeon.values()) {
			if (object.getId() == dung.outsideId
				...
				if (player.getSkills().getLevel(Skills.DUNGEONEERING) < dung.level) {
		...
		if (party == null) {
			...
			for (int i = 0; i < 5; i++) {
				Player teamMate = i >= party.getTeam().size() ? null : party
						.getTeam().get(i);
				player.getPackets().sendCSVarString(2371 + i,
						teamMate == null ? "" : teamMate.getDisplayName());
				player.getPackets().sendCSVarInteger(
						1153 + i,
						teamMate == null ? 0 : teamMate.getSkills()
								.getCombatLevel());
				player.getPackets().sendCSVarInteger(
						1158 + i,
						teamMate == null ? 0 : teamMate.getSkills().getLevel(
								Skills.DUNGEONEERING));
				player.getPackets().sendCSVarInteger(
						1163 + i,
						teamMate == null ? 0 : teamMate.getSkills()
			...
			if (!(p2.getControllerManager().getController() instanceof Kalaboss)) {
				player.getPackets()
						.sendGameMessage(
	...
	public void enterDungeon(boolean selectSize) {
		...
		for (Player p2 : party.getTeam()) {
			...
			if (p2.getPlane() != 0
					...
					|| p2.isLocked()
					|| !(p2.getControllerManager().getController() instanceof Kalaboss)) {
				player.getPackets().sendGameMessage(
	...
	public void refreshNames() {
		...
		for (Player p2 : party.getTeam())
			player.getPackets().sendCSVarString(
					2376 + (index++),
					"<col=FFFFFF><shad>" + p2.getDisplayName()
							+ "</col></shad>");
	}
	...
	public void formParty() {
		if (party != null)
			return;
		if (!(player.getControllerManager().getController() instanceof Kalaboss)
				|| player.getPlane() != 0) {
			player.getPackets().sendGameMessage(
	...
	public void leaveParty() {
		if (party != null) {
			...
			player.getInterfaceManager().removeInterface(945);
		}
	}
