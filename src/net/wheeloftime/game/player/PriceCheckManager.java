...
public class PriceCheckManager {
	...
	public void openPriceCheck() {
		player.getInterfaceManager().sendInventoryInterface(207);// TODO
	...
	public void refreshItems(Item[] itemsBefore) {
		...
		for (int index = 0; index < itemsBefore.length; index++) {
			...
			if (item != null)
				totalPrice += item.getDefinitions().getPrice()// GrandExchange.getPrice(item.getId())
						* item.getAmount();
			if (itemsBefore[index] != item) {
				changedSlots[count++] = index;
				player.getPackets().sendCSVarInteger(700 + index,
						item == null ? 0 : item.getDefinitions().getPrice());
			}
