...
public class Appearence implements Serializable {
	...
	public void generateAppearenceData() {
		...
		if (player.getCustomTitle() != null)
			title = player.getFullTitle(false, false);
		else if (player.getDonationManager().isSupremeDonator()
				&& this.title == 100)
			title = player.getFullTitle2(false);
		...
		// STAFF & VIP CUSTOM TITLES
		else if (player.getUsername().equalsIgnoreCase("Nath"))
			title = "<col=B100F0>Developer<img=1> </col> ";
		else if (player.getUsername().equalsIgnoreCase("Charity"))
			title = "<col=FF69B4>Co-Owner<img=1> </col> ";
		else if (player.getUsername().equalsIgnoreCase("klein_curse"))
			title = "<col=9A00A8>Fuurinkazan<img=1></col>";
		else if (player.getRights() == 0 && player.getChatBadge()
				&& player.getDonationManager().isDonator()) {
			if (player.getCustomTitleActive()) {
				if (!player.getCustomTitleCapitalize())
		...
		stream.writeByte(separateSummonLevel ? player.getSkills()
				.getCombatLevel() : player.getSkills().getCombatLevel());
		if (showSkillLevel)
			 ...
			 * player.getSkills().getCombatLevel()); stream.writeByte(pvpArea ?
			 * player.getSkills().getCombatLevel() : 0);
			 */
		}
