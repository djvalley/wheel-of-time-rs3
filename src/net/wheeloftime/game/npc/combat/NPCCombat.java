...
public final class NPCCombat {
	...
	public boolean checkAll() {
		...
		int maxDistance;
		int agroRatio = 20;
		if (npc.getId() >= 18538 && npc.getId() <= 18545) {
			distanceX = 0;
			distanceY = 0;
		}
		...
		if (!npc.isCantFollowUnderCombat()) {
			...
			if (npc instanceof Nex) {
				...
				maxDistance = npc.isForceFollowClose() ? 0 : ((npc
						.getCombatDefinitions().isFollowClose() && npc
						.getDefinitions().hasOption("attack"))
	...
	private boolean forceCheckClipAsRange(Entity target) {
		return target instanceof PestPortal || target instanceof Lucien;
	}
