package net.wheeloftime.game.npc.combat.impl.vorago;
...
public class VoragoCombat extends CombatScript {
	...
	public static void sendWaterfall() {// TODO make this better
		...
		case 0:// North-West
			x = rago.Centre[3].getX() - 10;
			y = rago.Centre[3].getY() + 10;
		...
		case 1:// North-East
				// 3040, 5984, 0old
				// 3038, 5982, 0now
			x = rago.Centre[3].getX() + 10;// 3048
			y = rago.Centre[3].getY() + 10;// 5992
			z = rago.Centre[3].getPlane();
		...
		for (Player p : VoragoHandler.getPlayers()) {
			...
			p.getInterfaceManager().sendOverlayInterface(1492);
		}
		...
		// 3040, 5984, 0 original
		World.spawnObjectTemporary(new WorldObject(84967, 11, rago.rotation,
				rago.wfTile), 10200);// Spawns the waterfalls for 10.2s

		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
				rago.setNextAnimation(new Animation(20323));
				rago.setNextGraphics(new Graphics(4013));
				for (Player p : VoragoHandler.getPlayers()) {
					p.getInterfaceManager().removeOverlayInterface();
	...
	public void sendClones(Entity target) {
		...
		rago.Clonee = (Entity) targets[Utils.random(targets.length)];// picks a
																		// random
																		// target
		...
		int combatStyle = ((Player) rago.Clonee).getCombatDefinitions()
				.getType(Equipment.SLOT_WEAPON);// Spawns clone based on what
												// combat style player is using
	...
	public void sendReflect(NPC npc) {
		...
		Entity targets[] = VoragoHandler.getPlayers().toArray(
				new Player[VoragoHandler.getPlayers().size()]);
		rago.Reflectee = (Entity) targets[Utils.random(targets.length)];// Picks
																		// random
																		// target
		rago.Reflectee.setNextGraphics(new Graphics(4012));
		((Player) rago.Reflectee)
				.getPackets()
				.sendEntityMessage(1, 15263739, ((Player) rago.Reflectee),
			...
			if (p == rago.Reflectee) {
				...
				p.getPackets()
						.sendEntityMessage(1, 15263739, p,
								"Vorago reflects incoming damage to surrounding foes!</col");
			}
		}
		WorldTasksManager.schedule(new WorldTask() {
			private int count = 0;
			...
			public void run() {
				if (count == 16) {
					...
					((Player) rago.Reflectee)
							.getPackets()
							.sendEntityMessage(1, 15263739,
									((Player) rago.Reflectee),
									"<col=007A00>Vorago releases his mental link with you.</col");
	...
	public Entity getFarthest(NPC npc) {// Returns the farthest away player
		...
		// this
		// VoragoHandler.removePlayer((Player) targets[index]);//Causes some
		// issues
		// return getFarthest(npc);//Recurses to find the new farthest player
		// } else {
	...
	private void sendMeleeAttack(NPC npc, Entity target) {
		...
		if (!Utils.isOnRange(npc, target, 1)) {// If its not in MD then send a
												// blue bomb
	...
	private void sendBlueBomb(NPC npc, Boolean anim) {// Blue bombs are
		...
		Projectile projectile = World.sendProjectileNew(npc, farthest, 4016,
				100, 20, 10, 0.4, 1, 0);
		int cycleTime = (int) (Utils.projectileTimeToCycles(projectile
				.getEndTime()) - 1);
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
				...
				for (Entity t : players) {// Hits all players close to the
					...
					if (rago.getPhase() == 5) {
						rago.pushBackDamage += (damage * VoragoHandler
								.getPlayersCount());// Adds to the P5 push back
													// count
					}
				}
			}
	...
	private void sendRedBomb(NPC npc) {
		...
		RBDmg = 2000;
		((Player) farthest).getPackets().sendEntityMessage(1, 15263739,
				((Player) farthest),
				"<col=DE1414>Vorago has sent a bomb after you. Run!</col>");
		int random = Utils.random(4);
		if (random <= 1)
			sendBlueBomb(npc, false);// Sends a blue bomb at the same time
		Projectile redBomb = World.sendProjectileNew(npc, farthest, 4023, 150,
				20, 0, 0.3, 0, 0);
		...
		if (rago.special) {// Handles all the special attacks
			...
			case 5: // final (p5)
				...
				case 1:// Smash
					...
					// check for x/y for message here
					return 7;
