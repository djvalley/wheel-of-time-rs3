...
import net.wheeloftime.game.npc.dungeonnering.Sagittare;
	...
	public int attack(NPC npc, Entity target) {
		...
		case 2:// Multi magic
			...
			for (Entity t : npc.getPossibleTargets()) {
				if ((attack == 0 || attack == 1) && t != target)
					continue;
				World.sendProjectile(npc, t, attack == 0 ? 2533 : 2535, 65, 50,
						54, 35, 5, 0);
				if (attack == 0)
					delayHit(
							npc,
		...
		case 4:
			...
			for (Entity t : npc.getPossibleTargets()) {
				...
				World.sendProjectile(npc, t, isMagicAttack ? 2537 : 2540, 65,
						50, 54, 35, 5, 0);
				if (isMagicAttack) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (cycles == 2) {
					...
					WorldTile teleport = Utils.getFreeTile(
							manager.getTile(rRef, 6, 6), 1);

					if (stage != 1 && stage != -1) {
						int corner = Utils.random(4);
						if (corner == 0)// this is good
							teleport = manager.getTile(rRef, 1,
									Utils.random(14) + 1);// 1,1
						// 14,
						// 1,
						// 14,
						// 14,
						...
						// 14
							...
							teleport = manager.getTile(rRef,
									14 - Utils.random(14), 1);
						else if (corner == 2)// this is good
							teleport = manager.getTile(rRef, 14,
									14 - Utils.random(14));
						else
							// this is good
							teleport = manager.getTile(rRef,
									Utils.random(14) + 1, 14);
					}

					for (int x = -1; x < 2; x++) {// 3x3 area
						for (int y = -1; y < 2; y++) {
							World.sendProjectile(boss,
									center.transform(x, y, 0), 2533, 250, 0,
									40, 0, 0, 0);
						}
					}
					...
					targetL: for (Entity target : boss.getPossibleTargets()) {
						...
						for (int x = -1; x < 2; x++) {// 3x3 area
							for (int y = -1; y < 2; y++) {
								WorldTile projectileTile = center.transform(x,
										y, 0);
								if (player.getX() != projectileTile.getX()
										|| player.getY() != projectileTile
												.getY())
									continue targetL;
							}
						}
						...
						player.getPackets().sendGameMessage(
								"You have been injured and can't move.");
						int hit = (int) (boss.getMaxHit() * .1 + getMaxHit(
								boss, (int) (boss.getMaxHit() * .90),
								NPCCombatDefinitions.RANGE, player));
						player.applyHit(new Hit(boss, hit,
