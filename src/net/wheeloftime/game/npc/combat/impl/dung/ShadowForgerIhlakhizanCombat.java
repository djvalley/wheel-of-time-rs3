	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (Utils.random(4) == 0) {
			if (Utils.random(3) == 0) {
				...
				for (int i = 0; i < 10; i++) {
					final WorldTile tile = ((ShadowForgerIhlakhizan) npc)
							.getManager().getTile(forger.getReference(),
									2 + Utils.random(12), 2 + Utils.random(12));
					if (Utils.colides(npc.getX(), npc.getY(), npc.getSize(),
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							...
							for (Player player : forger.getManager().getParty()
									.getTeam()) {
								if (player.isDead()
								...
								player.applyHit(new Hit(
										npc,
										Utils.random(npc
												.getMaxHit(NPCCombatDefinitions.RANGE)) + 1,
				...
				for (Entity t : npc.getPossibleTargets()) {
					if (t instanceof Player) {
						...
						player.getPackets().sendGameMessage(
								"The shadow-forger starts to glow.");
					}
				}
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						for (Entity t : npc.getPossibleTargets()) {
							t.applyHit(new Hit(npc, Utils.random((int) (t
							...
							if (t instanceof Player) {
								WorldTasksManager.schedule(new WorldTask() {
									...
									public void run() {
										...
										if (ticks == 1) {
											...
											target.setNextAnimation(new Animation(
													10070));
											target.setNextForceMovement(new ForceMovement(
													target, 0, tile, 2, target
															.getDirection()));
										} else if (ticks == 2) {
								...
								for (int stat = 0; stat < 7; stat++) {
									if (stat == Skills.HITPOINTS)
										continue;
									int drain = player.getSkills().getLevel(
											stat) / 2;
