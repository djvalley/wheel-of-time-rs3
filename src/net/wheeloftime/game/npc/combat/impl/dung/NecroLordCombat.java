...
import net.wheeloftime.game.npc.dungeonnering.NecroLord;
...
public class NecroLordCombat extends CombatScript {
	...
	public int attack(NPC npc, final Entity target) {
		...
		if (Utils.random(10) == 0) {
			final int skeletonCount = boss.getManager().getParty().getTeam()
					.size();
					...
					if (cycles == 2) {
						for (int i = 0; i < skeletonCount; i++) {
							WorldTile tile = Utils.getFreeTile(
									boss.getManager().getTile(
											boss.getReference(),
											Utils.random(2) == 0 ? 5 : 10, 5),
									4);
							projectileTile.add(tile);
							World.sendProjectile(boss, tile, 2590, 65, 0, 30,
									0, 16, 0);
						}
					} else if (cycles == 4) {
		...
		case 3:
			...
			World.sendProjectile(npc, tile, attack == 2 ? 178 : 168, 40, 18,
					55, 70, 5, 0);
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					for (Entity t : boss.getPossibleTargets()) {
						...
						if (damage > 0) {
							if (attack == 2)
								...
								t.applyHit(new Hit(boss, Utils.random(boss
										.getMaxHit()) + 1, HitLook.MAGIC_DAMAGE));
							}
							t.setNextGraphics(new Graphics(attack == 2 ? 179
									: 169, 60, 65));
						}
					}
				}
