...
import net.wheeloftime.game.npc.dungeonnering.KalGerWarmonger;
	...
	public int attack(NPC npc, Entity target) {
		...
		if (boss.isUsingMelee()) {
			...
			for (Player player : manager.getParty().getTeam()) {
				if (Utils.colides(player.getX(), player.getY(),
						player.getSize(), boss.getX(), boss.getY(), 5)) {
					smash = true;
					break;
				}
			}
			if (smash) {
				...
				for (Player player : manager.getParty().getTeam()) {
					...
					if (Utils.inCircle(player, boss, 5))// 5 square radius
														// (imperfect circle)
						player.applyHit(new Hit(boss, Utils.random(300,
								boss.getMaxHit()), HitLook.REGULAR_DAMAGE));
				}
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						boss.setPullTicks(0);
						for (Player player : manager.getParty().getTeam())
							// we obv need to reset the camera ^.^
							player.getPackets().sendStopCameraShake();
					}
				});
			} else if (!Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(),
