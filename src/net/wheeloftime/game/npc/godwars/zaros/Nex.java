...
public class Nex extends NPC {
	...
	private boolean firstStageAttack, hasShadowTraps, isFlying, rangeActivated;
	...
	public enum NexPhase {
		...
		SMOKE("Fumus!", new ForceTalk("Fill my soul with smoke!"), 3325, 3310,
				1, 4),

		SHADOW("Umbra!", new ForceTalk("Darken my shadow!"), 3313, 3307, 2, 3),

		BLOOD("Cruor!", new ForceTalk("Flood my lungs with blood!"), 3299,
				3298, 3, 2),

		ICE("Glacies!", new ForceTalk("Infuse me with the power of ice!"),
				3304, 3327, 4, 1),
		...
		private NexPhase(String minionName, ForceTalk message, int firstSound,
				int secondSound, int phaseValue, int flag) {
	...
	public void processNPC() {
		...
		if (currentPhase == NexPhase.ZAROS || getId() == 15776) {
			if (prayerTicks > 0) {
				...
				resetPrayerTicks();
				// transformNex(); would fuck up the hitpoints of nex
			}
		}
	}
	...
	public void handleIngoingHit(Hit hit) {
		if (isSiphioning()
				|| (currentPhase == NexPhase.BLOOD && hit.getSource()
						.getEffectsManager().hasActiveEffect(EffectType.BLEED))) {
			hit.setHealHit();
			heal(hit.getDamage());
		}
		if (hit.getDamage() >= 1500)
			playSoundEffect(ZarosGodwars.HIT_SOUNDS[Utils
					.random(ZarosGodwars.HIT_SOUNDS.length)]);
		if (currentPhase == null)
			return;
		if (getHitpoints() - hit.getDamage() <= getMaxHitpoints()
				- (currentPhase.getPhaseValue() * 40000)) {
			if ((currentPhase.getPhaseValue() >= 0)
					&& (currentPhase.getPhaseValue() <= nexMinions.length)) {
				if (!nexMinions[currentPhase.getPhaseValue() - 1]
						.isBarrierBroken()) {
					nexMinions[currentPhase.getPhaseValue() - 1].breakBarrier();
					setNextForceTalk(new ForceTalk(currentPhase.getMinionName()
							+ ", dont fail me!"));
					setCapDamage(0);
				}
			}
		...
		if (currentPhase == NexPhase.ZAROS || getId() == 15776) {
			...
			if (getId() == 13449 && hit.getLook() == HitLook.MELEE_DAMAGE
				...
				if (source != null) {
					...
					hit.setDamage((int) (hit.getDamage() * (getId() == 13449 ? source
							.getMeleePrayerMultiplier() : source
	...
	public void sendDeath(final Entity source) {
		if (source instanceof Player) {
			((Player) source).getTimersManager().removeTimer(RecordKey.NEX);
			((Player) source).getCompCapeManager().increaseRequirement(
					Requirement.NEX, 1);
		}
		increaseKills(RecordKey.NEX, false);
		...
		if (source instanceof Player) {
			...
			if (players != null) {
				for (Player p : players) {
					if (p == null)
						continue;
					Controller controler = p.getControllerManager()
							.getController();
					if (controler != null && controler instanceof GodWars) {
						GodWars godController = (GodWars) controler;
				...
				if (possibleTargets != null) {
					for (Entity entity : possibleTargets) {
						...
						World.sendProjectile(target, new WorldTile(getX() + 2,
								getY() + 2, getPlane()), 2261, 24, 0, 41, 35,
								30, 0);
						World.sendProjectile(target, new WorldTile(getX() + 2,
								getY(), getPlane()), 2261, 41, 0, 41, 35, 30, 0);
						World.sendProjectile(target, new WorldTile(getX() + 2,
								getY() - 2, getPlane()), 2261, 41, 0, 41, 35,
								30, 0);
						World.sendProjectile(target, new WorldTile(getX() - 2,
								getY() + 2, getPlane()), 2261, 41, 0, 41, 35,
								30, 0);
						...
						World.sendProjectile(target, new WorldTile(getX(),
								getY() - 2, getPlane()), 2261, 41, 0, 41, 35,
								30, 0);
						entity.applyHit(new Hit(target, Utils.random(600),
		...
		for (Entity entity : getPossibleTargets()) {
			if (entity instanceof Player) {
				...
				player.getPackets().sendGameMessage(
						"The clouds around you dissapate.");
