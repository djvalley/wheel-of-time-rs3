...
public class Voidshifter extends Familiar {
	...
	public void processNPC() {
		super.processNPC();
		if (getOwner().getHitpoints() <= (getOwner().getMaxHitpoints() * .1)
				&& !getOwner().isDead()) {
			getOwner().getPackets().sendGameMessage(
					"Your void shifter sacrafices its life to protect you.");
			Magic.sendTeleportSpell(getOwner(), 14388, -1, 1503, 1502, 0, 0,
					new WorldTile(2662, 2649, 0), 3, true,
