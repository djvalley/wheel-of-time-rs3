...
public class Littlehoardstalker extends Familiar {
	...
	public Littlehoardstalker(Player owner, Pouch pouch, WorldTile tile,
	...
	public void processNPC() {
		...
		if (forageTicks == 300) {
			...
			getBob().getBeastItems().add(
					new Item(DungeonConstants.HOARDSTALKER_ITEMS[1][Utils
							.random(5)], 1));
		}
	}
	...
	public boolean submitSpecial(Object object) {
		...
		for (int skill = 0; skill < Skills.SKILL_NAME.length; skill++) {
			...
			player.getSkills().set(skill,
					player.getSkills().getLevel(skill) + 2);
		}
		return true;
	}
}
