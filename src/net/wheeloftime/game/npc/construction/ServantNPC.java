package net.wheeloftime.game.npc.construction;
...
import net.wheeloftime.game.player.Bank;
...
public class ServantNPC extends NPC {
	...
	public ServantNPC(House house) {
		super(house.getServant().getId(), house.getPortal().transform(1, 0, 0),
				-1, true, true);
		...
		if (owner.getSkills().getLevel(Skills.CONSTRUCTION) < servant
				.getLevel()) {
	...
	public void makeFood(final Builds[] builds) {
		...
		if (house.isBuildMode()) {
			owner.getPackets().sendGameMessage(
					"Your servant cannot prepare food while in building mode.");
			return;
		}
		String basicResponse = "I appologise, but I cannot serve "
				+ (owner.getAppearence().isMale() ? "Sir" : "Madam")
				+ " without";
		final RoomReference kitchen = house.getRoom(Room.KITCHEN), diningRoom = house
				.getRoom(Room.DINING_ROOM);
		if (kitchen == null) {
			owner.getDialogueManager().startDialogue("SimpleNPCMessage",
					getId(), basicResponse + " a proper kitchen.");
			return;
		} else if (diningRoom == null) {
			owner.getDialogueManager().startDialogue("SimpleNPCMessage",
					getId(), basicResponse + " a proper dining room.");
			return;
		} else {
			...
			if (!diningRoom.containsBuild(HouseConstants.Builds.DINING_TABLE)) {
				owner.getDialogueManager().startDialogue("SimpleNPCMessage",
						getId(), basicResponse + " a dining table");
				return;
			}
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					if (count == 1) {
						...
						setNextWorldTile(new WorldTile(Utils.getFreeTile(
								kitchenTile, 2)));
						...
						if (calculatedCount % 3 == 0) {
							setNextAnimation(new Animation(
									build == Builds.STOVE ? 897 : 3659));
							index++;
						} else if (calculatedCount % 2 == 0)
							calcFollow(house.getWorldObjectForBuild(kitchen,
									build), true);
					} else if (count == totalCount + 3) {
						setNextWorldTile(Utils.getFreeTile(diningRoomTile, 2));
					} else if (count == totalCount + 4
							|| count == totalCount + 5) {
						WorldTile diningTable = house.getWorldObjectForBuild(
								diningRoom, Builds.DINING_TABLE);
						if (count == totalCount + 4)
							calcFollow(diningTable, true);
						else {
							setNextAnimation(new Animation(808));
							int rotation = kitchen.getRotation();
							for (int x = 0; x < (rotation == 1 || rotation == 3 ? 2
									: 4); x++)
								for (int y = 0; y < (rotation == 1
										|| rotation == 3 ? 4 : 2); y++)
									World.addGroundItem(
											new Item(
													builds.length == 6 ? 7736
															: builds.length == 5 ? house
																	.getServant()
																	.getFoodId()
																	: HouseConstants.BEERS[kitchen
																			.getBuildSlot(Builds.BARRELS)]),
											diningTable.transform(x, y, 0),
											null, false, 300);
	...
	public void requestType(int item, int quantity, final byte type) {
		if (Engine.shutdown || Engine.delayedShutdownStart != 0) {
			owner.getDialogueManager()
					.startDialogue("SimpleMessage",
							"Your servant cannot make it to the bank before the system update is complete.");
			return;
		}
		...
		if (!bank.containsItem(defs.isNoted() ? defs.getCertId() : item)
				&& type == 0) {
			owner.getDialogueManager().startDialogue("SimpleNPCMessage",
					getId(),
					"It appears you do not have this item in your bank.");
			return;
		} else if (quantity > inventorySize) {
			owner.getDialogueManager().startDialogue(
					"SimpleNPCMessage",
					getId(),
					"I'm sorry to inform you that I can only hold "
							+ inventorySize + " items during a session.");
			return;
		}
		...
		if (plank != null && type == 1) {
			...
			if (owner.getInventory().getCoinsAmount() < cost) {
				owner.getDialogueManager()
						.startDialogue("SimpleNPCMessage", getId(),
								"You do not have enough coins to cover the costs of the sawmill.");
				return;
			}
		}
		...
		if (type == 1 || type == 2) {
			if (type == 2) {
				...
				owner.getInventory().removeItemMoneyPouch(
						new Item(995,
								(int) (quantity * (plank.getCost() * 0.7))));
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				setCantInteract(false);
				if (!owner.isRunning() || !house.isLoaded()
						|| !house.getPlayers().contains(owner)) {
					if (type == 1 || type == 2) {
				...
				house.incrementPaymentStage();
				if (type == 0) {
					bank.withdrawItemButOne(finalItem);
				} else if (type == 1) {
					owner.getInventory().addItem(plank.getId(),
							completeQuantity);
				} else if (type == 2) {
					owner.getInventory().addItem(finalItem, completeQuantity);
				} else {
					for (int i = 0; i < completeQuantity; i++) {
						bank.depositItem(owner.getInventory().getItems()
								.getThisItemSlot(finalItem), completeQuantity,
								false);
					}
				}
				owner.getDialogueManager()
						.startDialogue(
								"SimpleNPCMessage",
								getId(),
								type == 3 ? "I have successfully deposited your items into your bank. No longer will the items be at risk from thieves."
										: "I have returned with the items you asked me to retrieve.");
			}
