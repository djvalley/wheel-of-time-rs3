...
import net.wheeloftime.game.player.dialogues.impl.NeemDrupeSqueeze;
...
public class PolyporeCreature extends NPC {
	...
	public PolyporeCreature(int id, WorldTile tile, int mapAreaNameHash,
			boolean canBeAttackFromOutOfArea, boolean spawned) {
		super(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
		realId = id;
		setRandomWalk(id == 14694 ? NORMAL_WALK | FLY_WALK : id == 14698 ? 0
				: NORMAL_WALK);
		if (id == 14698)
	...
	public void processMovement() {
		...
		if (realId == 14696 && getNextWalkDirection() != -1) {
			...
			this.setNextForceMovement(new ForceMovement(this, 1, null, 0, Utils
					.getAngle(Utils.DIRECTION_DELTA_X[getNextWalkDirection()],
	...
	public static void sprinkleOil(final Player player, NPC target) {
		if (target == null) {
				...
				if (npcIndexes != null) {
					for (int npcIndex : npcIndexes) {
						...
						if (n == null || n.isDead() || n.hasFinished()
								|| n.getPlane() != player.getPlane()
								|| n.getCombat().getTarget() != player
								|| !(n instanceof PolyporeCreature)
								|| ((PolyporeCreature) n).realId != n.getId())
							continue;
			...
			if (!(target instanceof PolyporeCreature) || target.isDead()
					|| target.hasFinished())
				return;
			if (((PolyporeCreature) target).realId != target.getId()) {
				player.getPackets().sendGameMessage(
						"That creature has already been weakened.");
				return;
			}
		}
