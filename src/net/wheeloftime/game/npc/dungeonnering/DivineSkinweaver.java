...
public final class DivineSkinweaver extends DungeonBoss {
	...
	private static final int[][] HOLES = { { 0, 10 }, { 5, 15 }, { 11, 15 },
			{ 15, 10 }, { 15, 5 } };
	...
	public DivineSkinweaver(int id, WorldTile tile, DungeonManager manager,
			RoomReference reference) {
		super(id, tile, manager, reference);
		holeClosed = new boolean[5];
		skeletons = Collections
				.synchronizedList(new ArrayList<DungeonSkeletonBoss>());
	}

	void removeSkeleton(DungeonSkeletonBoss skeleton) {
		...
		if (!requestedClose && count < holeClosed.length) {
			...
			if (killedCount == 3) {
				requestedClose = true;
				killedCount = 0;
				this.setNextForceTalk(new ForceTalk(CLOSE_HOLE_MESSAGES[Utils
						.random(CLOSE_HOLE_MESSAGES.length)]));
				for (Player p2 : getManager().getParty().getTeam()) {
					if (!getManager().isAtBossRoom(p2))
						continue;
					p2.getPackets().sendGameMessage(
							"Divine skinweaver: <col=99CC66>"
									+ getNextForceTalk().getText());
				}
			}
		}
	}
	...
	private int[] getOpenHole() {
		...
		for (int[] hole : HOLES) {
			WorldObject object = getManager().getObjectWithType(getReference(),
					49289, 0, hole[0], hole[1]);
	...
	public void processNPC() {
		...
		if (respawnDelay > 0) {
			respawnDelay--;
		} else if (skeletons != null && holeClosed != null
				&& count < holeClosed.length && targets.size() != 0
				&& skeletons.size() < 5) {
			int[] coords = getOpenHole();
			if (coords != null) {
				skeletons.add((DungeonSkeletonBoss) getManager().spawnNPC(
						SKELETONS[Utils.random(SKELETONS.length)], 0,
						new WorldTile(coords[0], coords[1], 0), getReference(),
						DungeonConstants.BOSS_NPC, 0.4D));
				respawnDelay = 10;
			}
		}
		if (healDelay > 0) {
		...
		for (Entity target : targets) {
			if (target.getHitpoints() >= target.getMaxHitpoints())
				continue;
			if (healTarget == null
					|| Utils.getDistance(this, healTarget) > Utils.getDistance(
							this, target))
				healTarget = target;
		}
		if (healTarget == null)
	...
	public void talkTo(Player player) {
		...
		if (count < holeClosed.length || skeletons.size() > 0) {
			player.getDialogueManager().startDialogue("SimpleNPCMessage",
					getId(), "Chat later and kill the skeletons now, brah.");
			return;
		}
		setNextForceTalk(new ForceTalk(
				"I see little danger in this room so move on to the next with my thanks."));
	...
	public void blockHole(Player player, WorldObject object) {
		...
		if (!requestedClose) {
			player.getPackets()
					.sendGameMessage(
							"The portal is fully powered and shocks you with a large burst of energy.");
			player.applyHit(new Hit(player,
					(int) (player.getMaxHitpoints() * 0.2),
