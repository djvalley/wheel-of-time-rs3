...
public class Gravecreeper extends DungeonBoss {
	...
	public Gravecreeper(int id, WorldTile tile, DungeonManager manager,
	...
	public boolean removeBurnedTile(WorldTile center) {
		for (BurnTile bTile : burnedTiles) {
			if (bTile.center.getX() == center.getX()
					&& bTile.center.getY() == center.getY()) {
			...
			for (Entity t : getPossibleTargets()) {
				...
				tileLoop: for (WorldTile tile : bTile.tiles) {
					if (p2.getX() != tile.getX() || p2.getY() != tile.getY())
						continue tileLoop;
					p2.applyHit(new Hit(this, (int) Utils.random(
							getMaxHit() * .1, getMaxHit() * .25),
							HitLook.REGULAR_DAMAGE));
					p2.getPrayer().drain(20);
					if (p2.getPrayer().hasPrayersOn()) {
						p2.getPrayer().closeAllPrayers(true);
					}
				}
			}
		}
	...
	private static final String[] SPECIAL_SHOUTS = { "Burrrrrry", "Digggggg",
			"Brrainnns" };
	...
	public void useSpecial() {
		...
		setForceWalk(walkTo);
		setNextForceTalk(new ForceTalk(SPECIAL_SHOUTS[specialDelay == -1 ? 2
				: Utils.random(SPECIAL_SHOUTS.length)]));
						...
						WorldTasksManager.schedule(new WorldTask() {
							...
							public void run() {
								if (getManager().isDestroyed())
									return;
								setNextWorldTile(getManager().getTile(
										getReference(),
										3 + Utils.random(4) * 3,
										3 + Utils.random(4) * 3));
	...
	public WorldTile getNearestPlinch() {
		...
		for (int x = 0; x < plinths.length; x++) {
			for (int y = 0; y < plinths[x].length; y++) {
				WorldObject plinth = getManager().getObjectWithType(
						getReference(), 22, 3 + x * 3, 3 + y * 3);
				if (plinth == null)
					continue;
	...
	public void triggerPlinths() {
		...
		for (int x = 0; x < plinths.length; x++) {
			for (int y = 0; y < plinths[x].length; y++) {
				if (plinths[x][y] == null)
					continue;
				WorldTile altarLoc = getManager().getTile(getReference(),
						TOMB_LOC_POS_2[y][x][0], TOMB_LOC_POS_2[y][x][1]);
				...
				if (!triggeredPlinths[x][y]) {
					...
					for (Entity t : possibleTargets) {
						if (Utils.isOnRange(t.getX(), t.getY(), t.getSize(),
								altarLoc.getX(), altarLoc.getY(), 1, 2))
							t.applyHit(new Hit(this, Utils.random((int) (t
									.getMaxHitpoints() * 0.1)) + 1,
	...
	public boolean cleanseTomb(Player player, WorldObject tomb) {
		...
		for (int x = 0; x < plinths.length; x++) {
			for (int y = 0; y < plinths[x].length; y++) {
				if (TOMB_LOC_POS_2[y][x][0] == pos[0]
						&& TOMB_LOC_POS_2[y][x][1] == pos[1]) {
					player.lock(1);
					player.setNextAnimation(new Animation(645));
					player.getPackets()
							.sendGameMessage(
									"Blessing the grave costs prayer points, but evil retreats.");
	...
	public void activateTombs() {
		for (int x = 0; x < plinths.length; x++) {
			for (int y = 0; y < plinths[x].length; y++) {
				if (plinths[x][y] == null)
					continue;
				WorldObject altar = getManager().getObjectWithType(
						getReference(), 10, TOMB_LOC_POS_2[y][x][0],
		...
		for (int x = 0; x < plinths.length; x++) {
			for (int y = 0; y < plinths[x].length; y++) {
				...
				if (plinth == null)
					continue;
				if (plinths[x][y] == null
						&& Utils.random(15) < getManager().getParty().getTeam()
								.size() * 3) {
