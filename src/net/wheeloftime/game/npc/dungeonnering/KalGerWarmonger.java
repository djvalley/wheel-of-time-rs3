...
public class KalGerWarmonger extends DungeonBoss {
	...
	private static final int[] WEAPONS = { -1, 56057, 56054, 56056, 56055,
			56053 };
	...
	public KalGerWarmonger(int id, WorldTile tile,
			final DungeonManager manager, final RoomReference reference) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				sphere = new WarpedSphere(reference, 12842, manager.getTile(
						reference, 11, 12), manager, 0.0);
				beginFlyCount();
			}
		}, 3);
	}
	...
	public Hit handleOutgoingHit(Hit hit, Entity target) {
		if (annoyanceMeter == 10) {
			...
			if (target instanceof Player) {
				...
				player.getPackets()
						.sendGameMessage(
								"You have been injured and cannot use protective prayers.");
			}
			...
			public void run() {
				for (Entity t : getPossibleTargets()) {
					if (Utils.inCircle(t, boss, 8))
						t.applyHit(new Hit(boss, Utils.random(300, 990),
	...
	private void processNextType() {
		...
		if (typeTicks == 1) {
			...
			nextFlyTile = getManager().getTile(getReference(), FLY_LOCATION[0],
					FLY_LOCATION[1], SIZE, SIZE);
			setNextForceMovement(new ForceMovement(this, 1, nextFlyTile, 5,
					Utils.getAngle(nextFlyTile.getX() - getX(),
							nextFlyTile.getY() - getY())));
		} else if (typeTicks == 6) {
			setNextGraphics(new Graphics(2870));
			...
			outer: for (int x = 0; x < 16; x++) {
				for (int y = 0; y < 16; y++) {
					WorldObject next = getManager().getObjectWithType(
							getReference(), 10, x, y);
				...
				if (ticks == 1) {
					...
					for (Entity t : possibleTargets) {
						if (t instanceof Player) {
							...
							YkLagorThunderousCombat.sendPullAttack(tile,
									player, false);
						}
					}
				} else if (ticks == 10) {
					...
					for (Entity t : possibleTargets) {
						...
						t.applyHit(new Hit(boss, Utils.random(33, 87),
								HitLook.REGULAR_DAMAGE));
					}
				}
			}
	...
	private void stealPlayerEffects() {
		...
		for (Player player : getManager().getParty().getTeam()) {
			if (!getManager().getCurrentRoomReference(player).equals(
					getReference()))
				continue;
			boolean usingPiety = player.getPrayer().isActive(Prayer.PIETY);
			boolean usingTurmoil = player.getPrayer().isActive(Prayer.TURMOIL);
			if (!usingPiety && !usingTurmoil)
				continue;
			player.getPackets().sendGameMessage(
					"The Warmonger steals your "
							+ (usingPiety ? "Piety" : "Turmoil") + " effects!");
			stolenEffects = true;
		}
	}
