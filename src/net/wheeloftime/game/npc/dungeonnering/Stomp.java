...
public final class Stomp extends DungeonBoss {
	...
	public Stomp(int id, WorldTile tile, DungeonManager manager,
		...
		if (getId() == 9781)
			...
			if (count > 0) {
				if (count == IVULNERABLE_TIMER - 3) {
					...
					for (int[] s : shadows) {
						WorldObject object = getManager()
								.spawnObjectTemporary(getReference(), 49268,
										10, 0, s[0], s[1], 30000);
						for (Entity target : possibleTargets)
							if (target.getX() == object.getX()
									&& target.getY() == object.getY())
								target.applyHit(new Hit(this,
										1 + Utils.random((int) (target
												.getMaxHitpoints() * 0.8)),
						...
						for (Entity target : getPossibleTargets()) {
							if (target instanceof Player) {
								...
								player.getPackets().sendGameMessage(
										"The portal weakens, harming Stomp!");
							}
						}
					} else
	...
	public void refreshLodestone(int index) {
		...
		int id = count == IVULNERABLE_TIMER ? (49274 + lodeStoneType * 2 + index)
				: lodestones[index] ? lodeStoneType == 0 ? 51099
						: lodeStoneType == 1 ? 51601 : 51647 : (49270 + index);

		getManager().spawnObject(getReference(), id, 10, 2,
				index == 1 ? 10 : 5, 10);
	}
	...
	public void chargeLodeStone(Player player, int index) {
		...
		if (player.getInventory().containsItem(CRYSTAL[lodeStoneType], 1)) {
			...
			player.getPackets().sendGameMessage(
					"You place the crystal into the device and it powers up.");
				...
				for (Entity target : getPossibleTargets()) {
					if (target instanceof Player) {
						...
						p2.getPackets().sendGameMessage(
								"The lodestone has been fully activated.");
					}
				}
			}
		} else
			player.getPackets().sendGameMessage(
					"You need a "
							+ ItemDefinitions
									.getItemDefinitions(CRYSTAL[lodeStoneType])
									.getName().toLowerCase()
	...
	public void charge() {
		...
		for (int count = 0; count < 11; count++) {
			l: for (int i = 0; i < DungeonConstants.SET_RESOURCES_MAX_TRY; i++) {
				int x = 2 + Utils.random(12);
				int y = 2 + Utils.random(9);
				if (containsShadow(x, y)
						|| !getManager().isFloorFree(getReference(), x, y))
					continue;
		...
		for (int count = 0; count < 2; count++) {
			l: for (int i = 0; i < DungeonConstants.SET_RESOURCES_MAX_TRY; i++) {
				...
				if (containsShadow(x, y)
						|| !getManager().isFloorFree(getReference(), x, y))
					continue;
				getManager().spawnItem(getReference(),
						new Item(CRYSTAL[lodeStoneType]), x, y);
				break l;
			}
		}
	}
