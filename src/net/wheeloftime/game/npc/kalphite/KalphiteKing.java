package net.wheeloftime.game.npc.kalphite;
...
public class KalphiteKing extends NPC {
	...
	public void switchPhase() {
		int currentPhase = getCurrentAttackPhase();
	...
	public int getCurrentAttackPhase() { // Melee mage or range NOT attack cycle
											// phase!
		return getId() - 16697;
	}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (part1) {
					...
					setFinished(false);
					appearFromEarth(target);
				}
			}
		}, 6, 5);
	}

	private void appearFromEarth(Entity target) {
				...
				for (Entity target : getPossibleTargets()) {
					if (Utils.colides(KalphiteKing.this, target)) {
						target.applyHit(new Hit(KalphiteKing.this, Utils
								.random(3000, 5000), HitLook.REGULAR_DAMAGE));
						if (target instanceof Player)
	...
	public void activateShield() {
		...
		for (Entity e : this.getPossibleTargets()) {
			((Player) e).getPackets().sendGameMessage("King activated shield!");
		}
	}
