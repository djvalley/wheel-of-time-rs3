				...
				for (int i = 0; i < wallCount; i++) {
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
										...
										if (!((wallId == 3158 && victim.getX() == npc
											...
											if (victim.getY() == npc.getBase()
												...
												if (message == null) {
													...
													hit = Utils.random(2000,
															2500);
												} else {
													victim.getPackets()
															.sendGameMessage(
																	message);
													hit = Utils.random(1000,
															1700);
												}
							...
							long arriveTime = startTime + (60000);//
							double speed = Utils.getProjectileSpeed(npc
									.getBase().transform(33, 38, 0), npc
									.getBase().transform(33, 19, 0), 120, 20,
									startTime, arriveTime);
							Projectile projectile = World.sendProjectileNew(npc
									.getBase().transform(33, 38, 0), npc
									.getBase().transform(33, 19, 0), wallId, 0,
									0, 10, speed, 0, 0);
						}
