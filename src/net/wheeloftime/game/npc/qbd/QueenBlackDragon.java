...
public final class QueenBlackDragon extends NPC {
	...
	public void setHitpoints(int hitpoints) {
		...
		if (lastHitpoints != hitpoints) {
			attacker.getPackets().sendCSVarInteger(1923,
	...
	public void sendDeath(Entity source) {
		switch (phase) {
		case 1:
			attacker.getPackets().sendCSVarInteger(1924, 1);
			...
			break;
		case 2:
			attacker.getPackets().sendCSVarInteger(1924, 3);
			attacker.getPackets().sendAddObject(
			...
			break;
		case 3:
			attacker.getPackets().sendCSVarInteger(1924, 5);
			attacker.getPackets().sendAddObject(
			...
			break;
		case 4:
			attacker.getPackets().sendCSVarInteger(1924, 7);
			attacker.getPackets().sendAddObject(
	...
	public void processNPC() {
		...
		if (ticks == -20) {
			...
			World.removeObject(new WorldObject(70778, 10, 0, base.transform(33,
					31, 0)));
			World.spawnObject(new WorldObject(70790, 10, 0, base.transform(31,
					29, 0)));
			World.spawnObject(new WorldObject(70775, 10, 0, base.transform(31,
					29, -1)));
		} else if (ticks == -1) {
			return;
		}
	...
	public void spawnWorm() {
		...
		final WorldTile destination = base.transform(28 + Utils.random(12),
				28 + Utils.random(6), 0);
		final Projectile projectile = World.sendProjectileNew(this,
				destination, 3141, 128, 0, 20, 6, 5, 3);
		if (getPhase() > 4) {
			return;
		}
		World.sendGraphics(attacker, new Graphics(3142,
				projectile.getEndTime() + 15, projectile.getEndHeight()),
				destination);
	...
	public void setPhase(int phase) {
		...
		case 5:
			...
			attacker.getCompCapeManager().increaseRequirement(
					Requirement.QUEEN_BLACK_DRAGON, 1);
			break;
		// Qbd is dead roar.
		}
	}
