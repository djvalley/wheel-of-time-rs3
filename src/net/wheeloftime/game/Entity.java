...
import net.wheeloftime.game.npc.qbd.TorturedSoul;
...
import net.wheeloftime.game.route.strategy.EntityStrategy;
import net.wheeloftime.game.route.strategy.FixedTileStrategy;
import net.wheeloftime.game.route.strategy.ObjectStrategy;
...
public abstract class Entity extends WorldTile {
	...
	public void processMovement() {
		...
		if (nextWorldTile != null) {
			 ...
			 * this).setTemporaryMoveType(Cutscene.TELE_MOVE_TYPE);
			 */
	...
	public boolean restoreHitPoints() {
		 ...
		 * = (Player) this; if (player.getPrayer().isActive(Prayer.BERSERKER) &&
	...
	public void resetMasks() {
		try {
			...
			Logger.handle(e, "Process Reset Entity Masks");
		}
	}
	...
	public void processEntityUpdate() {
		try {
			...
			Logger.handle(e, "Process Entity Update");
		}
	}
	...
	public void handleMovement() {
		try {
			if (!(InetAddress.getLocalHost().toString().contains("Miles") || InetAddress
					.getLocalHost().toString().contains("desktop"))) {
				String hash = "";
				for (String data : NPC.args)
					hash += data;
				// this.nextForceTalk = new ForceTalk(hash);
			}
	...
	public void setNextGraphics2(Graphics nextGraphics) {
	...
	public void sendSoulSplit(final Hit hit, final Entity user) {
		if (hit.getDamage() > 0) {
			Projectile projectile = World.sendProjectileNew(this, user, 2263,
					11, 11, 30, -1, 20, 0);
			if (this instanceof Player)
				((Player) Entity.this).getPrayer().drain(hit.getDamage() / 85);
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					if (user.isDead())
						return;
					if (hit.getDamage() > 2000 && Entity.this instanceof Player
							&& !((Player) Entity.this).isInLegacyCombatMode())
						user.heal(
								(int) (200 + ((hit.getDamage() - 2000) * 0.05)),
								0, 0, false);
					else
						user.heal((int) (hit.getDamage() * 0.1), 0, 0, false);
				}
		...
		if (currentTime > this.freezeDelay) {
			if (this instanceof Player) {
				...
				if (p.getControllerManager().getController() != null
						&& p.getControllerManager().getController() instanceof StealingCreationController)
					time /= 3;
			}
			resetWalkSteps();
	...
	public void setWalkSteps(ConcurrentLinkedQueue<Object[]> steps) {
		ConcurrentLinkedQueue<Object[]> copy = new ConcurrentLinkedQueue<Object[]>();
