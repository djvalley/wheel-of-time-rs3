package net.wheeloftime.game.map.bossInstance;

import java.util.List;
import java.util.concurrent.CopyOnWriteArrayList;

import net.wheeloftime.game.WorldTile;
import net.wheeloftime.game.map.MapInstance;
...
public abstract class BossInstance {
	...
	public void enterInstance(Player player, boolean login) {
		synchronized (BossInstanceHandler.LOCK) {
			if (!login)
				player.useStairs(-1,
						getTile(settings.getBoss().getInsideTile()), 0, 2);
			players.add(player);
			playMusic(player);
			if (!isPublic())
				player.getPackets().sendGameMessage(
						"Welcome to this session against <col=00FFFF>"
								+ getInstanceName()
								+ "</col>. This arena will expire in "
								+ (settings.getTimeRemaining() / 60000)
								+ " minutes.");
			if (!login) {
				player.getControllerManager().removeControllerWithoutCheck();
				player.getControllerManager().startController(
						settings.getBoss().getControllerName(), this);
			}
			player.setLastBossInstanceKey(owner == null ? null : owner
					.getUsername());
		}
	}

	public void playMusic(Player player) {
		player.getMusicsManager().forcePlayMusic(
				settings.getBoss().getMusicId());
	}
	...
	public WorldTile getTile(int x, int y, int plane) {
		...
		if (owner != null) {
			...
			tile = map.getTile((x - originalPos[0] * 8)
					% (map.getRatioX() * 64),
