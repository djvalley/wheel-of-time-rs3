...
public class ItemsContainerNew implements Serializable {
	...
	private int getSlotForIdInternal(int id, int preferred) {
		if (preferred >= 0 && preferred < data.length
				&& data[preferred] != null && data[preferred].getId() == id)
			return preferred;
	...
	public boolean swap(int slot1, int slot2) {
		if (slot1 < 0 || slot1 >= data.length || slot2 < 0
				|| slot2 >= data.length)
			return false;
		Item i1 = data[slot1];
		Item i2 = data[slot2];
	...
	private boolean removeNonStackableInternal(int id, int amount,
		...
		if (preferredSlot >= 0 && preferredSlot < data.length
				&& data[preferredSlot] != null
				&& data[preferredSlot].getId() == id) {
