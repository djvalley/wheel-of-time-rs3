...
import net.wheeloftime.game.player.controllers.UnderGroundDungeon;
...
public class LightSource {
	...
	private static boolean hasPermenantSource(Player player) {
		for (Item item : player.getInventory().getItems().getItems()) {
			if (item == null)
				continue;
			int slot = getSlot(item.getId(), false);
	...
	public static boolean extinguishSource(Player player, int itemSlot,
			boolean forceExtinguish) {
		...
		if (slot == -1)
			return false;
		else if (!forceExtinguish
				&& player.getControllerManager().getController() != null
				&& player.getControllerManager().getController() instanceof UnderGroundDungeon) {
			player.getPackets().sendGameMessage(
					"You cannot extinguish the " + item.getName().toLowerCase()
							+ " as you will not have a light source.");
			return false;
		}
		player.getInventory().replaceItem(LIGHT_SOURCES[0][slot],
				item.getAmount(), itemSlot);
		player.getPackets().sendGameMessage(
				"You extinguish the " + item.getName().toLowerCase() + ".");
		return true;
	}
	...
	public static boolean lightSource(Player player, int itemSlot) {
		...
		if (slot == -1)
			...
			player.getPackets().sendGameMessage(
					"You need a tinderbox in order to light the "
							+ item.getName().toLowerCase() + ".");
			return false;
		}
		player.getInventory().replaceItem(LIGHT_SOURCES[1][slot],
				item.getAmount(), itemSlot);
		player.getPackets().sendGameMessage(
				"You light the " + item.getName().toLowerCase() + ".");
		return true;
	}
}
