package net.wheeloftime.game.item.actions;
...
import net.wheeloftime.game.player.content.activities.minigames.clanwars.FfaZone;
import net.wheeloftime.game.player.controllers.CrucibleController;
...
public final class Drinkables {
	...
	public enum Drink {
		...
		SUPER_SARADOMIN_BREW_POTION(new int[] {
				28191,
				28193,
				28195,
				28197
		...
		SUPER_GUTHIX_REST_FLASK(new int[] {
				28239,
				28241,
				28243,
				28245,
		...
		SUPER_SARADOMIN_BREW_FLASK(new int[] {
				28227,
				28229,
				28231,
				28233,
		...
		WEAK_MELEE_POTION(new int[] {
				17560
		}, Effects.WEAK_MELEE_POTION),

		WEAK_MAGIC_POTION(new int[] {
				17556
		}, Effects.WEAK_MAGIC_POTION),

		WEAK_RANGE_POTION(new int[] {
				17558
		}, Effects.WEAK_RANGE_POTION),

		WEAK_DEFENCE_POTION(new int[] {
				17562
		}, Effects.WEAK_DEFENCE_POTION),

		WEAK_STAT_RESTORE(new int[] {
				17564
		}, Effects.WEAK_STAT_RESTORE_POTION),

		WEAK_CURE_POTION(new int[] {
				17568
		}, Effects.WEAK_STAT_CURE_POTION),
		...
		REGULAR_MELEE_POTION(new int[] {
				17586
		}, Effects.REGULAR_MELEE_POTION),

		REGULAR_MAGIC_POTION(new int[] {
				17582
		}, Effects.REGULAR_MAGIC_POTION),

		REGULAR_RANGE_POTION(new int[] {
				17584
		}, Effects.REGULAR_RANGE_POTION),
		...
		STRONG_MELEE_POTION(new int[] {
				17610
		}, Effects.STRONG_MELEE_POTION),

		STRONG_MAGIC_POTION(new int[] {
				17606
		}, Effects.STRONG_MAGIC_POTION),

		STRONG_RANGE_POTION(new int[] {
				17608
		}, Effects.STRONG_RANGE_POTION),
		...
		STRONG_CURE_POTION(new int[] {
				17616
		}, Effects.STRONG_STAT_CURE_POTION),
		...
		HOLY_OVERLOAD(
				...
				new Effects[] { Effects.OVERLOAD, Effects.PRAYER_RENEWAL }),
		...
		SEARING_OVERLOAD(
				...
				new Effects[] { Effects.OVERLOAD, Effects.SUPER_ANTI_FIRE }),
		...
		OVERLOAD_SALVE(
				...
				new Effects[] { Effects.OVERLOAD, Effects.SUPER_ANTI_FIRE, Effects.PRAYER_RENEWAL, Effects.PRAYER_POTION, Effects.SUPER_ANTIPOISON }),
		...
		REPLENISHMENT_POTION(new int[] { 33042, 33040, 33038, 33036, 33034, 33032 }, new Effects[] { Effects.SUPER_RESTORE, Effects.ENERGY_POTION }),

		WYRMFIRE_POTION(new int[] { 33054, 33052, 33050, 33048, 33046, 33044 }, new Effects[] { Effects.ANTI_FIRE, Effects.SUPER_ANTI_FIRE }),
		...
		EXTREME_SHARPSHOOTERS_POTION(new int[] { 33090, 33088, 33086, 33084, 33082, 33080 }, Effects.EXTREME_SHARPSHOOTERS),
		...
		BRIGHTFIRE_POTION(new int[] { 33174, 33172, 33170, 33168, 33166, 33164 }, new Effects[] { Effects.PRAYER_RENEWAL, Effects.SUPER_ANTI_FIRE }),

		SUPER_PRAYER_RENEWAL_POTION(new int[] { 33186, 33184, 33182, 33180, 33178, 33176 }, new Effects[] { Effects.SUPER_PRAYER, Effects.PRAYER_RENEWAL }),
		;

		private int[] id;
		private Effects[] effect;

		Drink(int[] id, Effects effect) {
			this.id = id;
			this.effect = new Effects[] { effect };
		}
		
		Drink(int[] id, Effects[] effects) {
			this.id = id;
			this.effect = effects;
		}
		
		public static Drink getFlask(Drink potion) {
			return Arrays.stream(Drink.values()).filter(drink -> drink.toString().equalsIgnoreCase(potion.toString().replace("POTION", "FLASK"))).findAny().get();
		}
		...
		public Drink toFlask() {
			if (isFlask())
				return this;
			for (int i = 0; i < Drink.values().length; i++) {
				Drink pot = Drink.values()[i];
				if (pot == null || !pot.isFlask())
					continue;
				String checkName = Drink.values()[i].name().toLowerCase().replace("_flask", "");
				String thisName = Drink.values()[ordinal()].name().toLowerCase().replace("_potion", "");
		...
		public Drink toPotion() {
			for (int i = 0; i < Drink.values().length; i++) {
				Drink pot = Drink.values()[i];
				if (pot == null || !pot.isPotion())
					continue;
				String checkName = Drink.values()[i].name().toLowerCase().replace("_potion", "");
				String thisName = Drink.values()[ordinal()].name().toLowerCase().replace("_flask", "");
				if (checkName.equals(thisName))
					return pot;
			}
			return null;
	...
	private enum Effects {
		...
		WEAK_RANGE_POTION(Skills.RANGED) {
			@Override
			public int getAffectedSkill(Player player, int skillId,
					int actualLevel, int realLevel) {
				int level = actualLevel > realLevel ? realLevel : actualLevel;
		...
		WEAK_STAT_RESTORE_POTION(Skills.ATTACK, Skills.STRENGTH,
				Skills.DEFENCE, Skills.MAGIC, Skills.RANGED, Skills.AGILITY,
		...
		REGULAR_RANGE_POTION(Skills.RANGED) {
			@Override
			public int getAffectedSkill(Player player, int skillId,
					int actualLevel, int realLevel) {
				int level = actualLevel > realLevel ? realLevel : actualLevel;
		...
		STRONG_RANGE_POTION(Skills.RANGED) {
			@Override
			public int getAffectedSkill(Player player, int skillId,
					int actualLevel, int realLevel) {
				int level = actualLevel > realLevel ? realLevel : actualLevel;
		...
		ZAMORAK_BREW(Skills.ATTACK, Skills.STRENGTH, Skills.DEFENCE) {
			...
			public boolean canDrink(Player player) {
				...
				player.getPrayer()
						.boost(
								(int) ((player.getPrayer().getPoints() * 1.08) + 20));
				return true;
			}
		},
		...
		SANFEW_SERUM(Skills.ATTACK, Skills.STRENGTH, Skills.DEFENCE,
				Skills.MAGIC, Skills.RANGED, Skills.AGILITY, Skills.COOKING,
		...
		SUPER_RANGE_POTION(Skills.RANGED) {
			@Override
			public int getAffectedSkill(Player player, int skillId,
					int actualLevel, int realLevel) {
				int level = actualLevel > realLevel ? realLevel : actualLevel;
		...
		EXTREME_RAN_POTION(Skills.RANGED) {
		...
		RECOVER_SPECIAL() {
			...
			@Override
			public boolean canDrink(Player player) {
				if (player.getControllerManager().getController() instanceof CrucibleController) {
		...
		SARADOMIN_BREW("You drink some of the foul liquid.", Skills.ATTACK,
				Skills.DEFENCE, Skills.STRENGTH, Skills.MAGIC, Skills.RANGED) {
			...
			public int getAffectedSkill(Player player, int skillId,
					int actualLevel, int realLevel) {
				if (skillId == Skills.DEFENCE) {
					int boost = (int) (realLevel * 0.04) + 1;
					int level = actualLevel > realLevel ? realLevel
							: actualLevel;
					return level + boost;
				} else {
		...
		SUPREME_OVERLOAD() {
			...
			@Override
			public void extra(final Player player) {
				Effect currentEffect = player.getEffectsManager().getEffectForType(EffectType.SUPREME_OVERLOAD);
				...
				player.getEffectsManager().startEffect(
						new Effect(EffectType.SUPREME_OVERLOAD, 601));
			}
		},
		...
		SUPER_WARMASTER(Skills.ATTACK, Skills.DEFENCE, Skills.STRENGTH, Skills.MAGIC, Skills.RANGED) {
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				final int level = actualLevel > realLevel ? realLevel : actualLevel;
				return (int) (level + 2 + (realLevel * 0.12));
		...
		EXTREME_BRAWLERS(Skills.ATTACK, Skills.STRENGTH, Skills.DEFENCE) {
			...
			@Override
			public boolean canDrink(Player player) {
				if (player.getControllerManager().getController() instanceof Wilderness || player.getControllerManager().getController() instanceof CrucibleController || FfaZone.isOverloadChanged(player)) {
		...
		EXTREME_SHARPSHOOTERS(Skills.RANGED, Skills.DEFENCE) {
		...
		EXTREME_WARMASTERS(Skills.ATTACK, Skills.STRENGTH, Skills.DEFENCE, Skills.RANGED, Skills.MAGIC) {
		...
		RANGERS_AID("You drink a glass of Ranger's Aid.", Skills.RANGED,
				Skills.DEFENCE) {
			@Override
			public int getAffectedSkill(Player player, int skillId,
					int actualLevel, int realLevel) {
	...
	public static boolean drink(Player player, Item item, int slot) {
		...
		if (player.getPotDelay() > Utils.currentTimeMillis())
			return true;
		if (!player.getControllerManager().canPot(drink))
			return true;
		for (Effects effect : drink.effect)
			if (!effect.canDrink(player))
				return true;
		...
		for (Effects effect : drink.effect)
			for (int skillId : effect.affectedSkills)
				player.getSkills().set(skillId, effect.getAffectedSkill(player, skillId, player.getSkills().getLevel(skillId), player.getSkills().getLevelForXp(skillId)));
		player.setNextAnimationNoPriority(new Animation((player
				.getCombatDefinitions().isCombatStance() && !player
				.isInLegacyCombatMode()) ? 18003 : 18000));
		if (drink.isFlask() || drink.isPotion()) {
			player.getPackets()
					.sendGameMessage("You drink some of your "
	...
	public static void decantPotsInv(Player player) {
		...
		for (Drink potion : Drink.values()) {
			if (potion == null || potion.isFlask() || !potion.toString().toLowerCase().replace("_", " ").contains("potion"))
				continue;
			int potionDoses = 0;
			for (int i = potion.getId().length - 1; i >= 0; i--) {
				int id = potion.getId()[i];
				if (player.getInventory().containsItem(id, 1)) {
					potionDoses += player.getInventory().getAmountOf(id) * (potion.getId().length - i);
				}
				if (player.getInventory().containsItem(ItemDefinitions.getItemDefinitions(id).getCertId(), 1)) {
					potionDoses += player.getInventory().getAmountOf(ItemDefinitions.getItemDefinitions(id).getCertId()) * (potion.getId().length - i);
				}
			}
			...
			int dosesRemoved = 0;
			for (int i = 4; i > 0; i--) {
				while (dosesRemoved <= potionDoses && (player.getInventory().containsItem(potion.getIdForDoses(i), 1) || player.getInventory().containsItem(ItemDefinitions.getItemDefinitions(potion.getIdForDoses(i)).getCertId(), 1))) {
			...
			for (int i = 4; i > 0; i--) {
				...
				if (amount == 0 || dosesRemoved <= 0)
					continue;
				Item item = new Item(ItemDefinitions.getItemDefinitions(potion.getIdForDoses(i)).getCertId(), amount);
	...
	public static void resetOverLoadEffect(Player player) {
		if (!player.isDead()) {
			...
			realLevel = player.getSkills().getLevelForXp(Skills.RANGED);
			if (actualLevel > realLevel)
				player.getSkills().set(Skills.RANGED, realLevel);
		}
		player.getPackets()
				.sendGameMessage(
	...
	public static void applyOverLoadEffect(Player player) {
		...
		player.getSkills().set(Skills.ATTACK,
				(int) (level + 3 + (realLevel * 0.24)));//old 0.24
		...
		realLevel = player.getSkills().getLevelForXp(Skills.RANGED);
		level = actualLevel > realLevel ? realLevel : actualLevel;
		player.getSkills().set(Skills.RANGED,
				(int) (level + 3 + (realLevel * 0.24)));
	}
