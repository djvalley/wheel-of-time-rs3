...
public class Consumables {
	...
	public static enum Effect {
		...
		WILD_PIE(7208, 7210) {
			...
			public void activateEffect(Player player) {
				...
				int level2 = player.getSkills().getLevel(Skills.RANGED);
				int realLevel2 = player.getSkills().getLevelForXp(Skills.RANGED);
				player.getSkills().set(Skills.RANGED, level2 >= realLevel2 ? realLevel2 + 4 : level2 + 4);
			}
		},
		...
		MINT_CAKE(ItemIdentifiers.MINT_CAKE) {
			@Override
			public void activateEffect(Player player) {
	...
	public static boolean eat(Player player, int slot, Item item) {
		...
		if (!player.getControllerManager().canEat(heal))
			return true;
		player.stopAll(false, false);
		player.setNextAnimation((player.getCombatDefinitions().isCombatStance() && !player.isInLegacyCombatMode()) ? COMBAT_EAT_ANIM : EAT_ANIM);

		int pieces = item.getDefinitions().getCSOpcode(2970);
		player.addFoodDelay(pieces == 2 ? 600 : 1800);
		PlayerCombat.addCombatDelay(player, 3);// TODO test
		...
		if (player.getCombatDefinitions().getCurrentTarget() != null
			...
			PlayerCombat.addAdrenaline(player, -10);
		return true;
	}
