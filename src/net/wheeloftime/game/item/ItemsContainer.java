...
public final class ItemsContainer<T extends Item> implements Serializable {
	...
	public boolean add(T item) {
		if (alwaysStackable || item.getDefinitions().isStackable()
			...
			if (item.getId() == 995) {
				if (item.getAmount() + getNumberOf(995) > Integer.MAX_VALUE
						|| item.getAmount() + getNumberOf(995) <= 0) {
					return false;
				}
			}
			for (int i = 0; i < data.length; i++) {
			...
			if (item.getAmount() > 1) {
				if (freeSlots() >= item.getAmount()) {
					for (int i = 0; i < item.getAmount(); i++) {
						int index = freeSlot();
						data[index] = new Item(item.getId(), 1, 0/*
																 * item.getDegrade
																 * ()
																 */);
					}
	...
	public int getFreeSlots(Item... items) {
		int s = 0;
		for (Item aData : data) {
			if (aData != null) {
				for (Item item : items) {
