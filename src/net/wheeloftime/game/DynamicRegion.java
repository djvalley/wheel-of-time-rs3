		...
		for (int dynZ = 0; dynZ < 4; dynZ++) {
			for (int dynX = 0; dynX < 8; dynX++) {
				for (int dynY = 0; dynY < 8; dynY++) {
					...
					int archiveId = Utils.getMapArchiveId((mapID >> 8),
							(mapID & 0xFF));
					byte[] mapContainerData = Cache.STORE.getIndexes()[5]
							.getFile(archiveId, 3);
					byte[] landContainerData = Cache.STORE.getIndexes()[5]
							.getFile(archiveId, 0);
						...
						for (int plane = 0; plane < 4; plane++) {
							for (int x = 0; x < 64; x++) {
								for (int y = 0; y < 64; y++) {
									...
									if ((value & 0x2) != 0) {
										mapSettings[plane][x][y] = (byte) stream
												.readByte();
