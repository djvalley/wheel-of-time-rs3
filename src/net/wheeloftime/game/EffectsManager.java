...
import net.wheeloftime.game.item.actions.Drinkables;
...
import net.wheeloftime.game.player.AcceptAid;
...
public class EffectsManager implements Serializable {
		...
		for (Effect effect : effects) {
			...
			if (action == BUFF) {
				if (isPlayer) {
					...
					if (effect.type == EffectType.OVERLOAD) {
					...
					} else if (effect.type == EffectType.REGENERATE
							&& (player.isUnderCombat() || player
									.getCombatDefinitions()
									.getSpecialAttackPercentage() <= 5)) {
						removeEffect(effect.type);
					...
					} else if (effect.type == EffectType.FIRE_IMMUNITY
							|| effect.type == EffectType.SUPER_FIRE_IMMUNITY) {
						if (effect.cycle == (effect.type == EffectType.FIRE_IMMUNITY ? 10
								: 20))
							player.getPackets()
									.sendGameMessage(
											"<col=480000>Your resistance to dragonfire is about to run out.</col>");
						...
						if (player.isUnderCombat()
							...
							|| effect.type == EffectType.DEATHS_SWIFTNESS
							|| effect.type == EffectType.SUNSHINE) {
						if (player.getCombatDefinitions().getType(
								Equipment.SLOT_WEAPON) != (effect.type == EffectType.DEATHS_SWIFTNESS ? Combat.RANGE_TYPE
								: Combat.MAGIC_TYPE))
							removeEffect(effect.type);
						if (effect.type == EffectType.METAMORPHISIS) {
							if (effect.cycle == 33) {
								...
								player.getAppearence()
										.transformIntoNPC(
												type == Combat.TYPE_EARTH ? 15983
														: type == Combat.TYPE_AIR ? 15984
																: type == Combat.TYPE_WATER ? 15982
																		: 15981);
							}
						} else if (effect.type == EffectType.DEATHS_SWIFTNESS
								|| effect.type == EffectType.SUNSHINE) {
							if (effect.cycle < 46 && effect.cycle % 4 == 0) {
								int mainHandDamage = player
										.getCombatDefinitions().getHandDamage(
												false) / 10, damage = (int) Utils
										.random(mainHandDamage * 0.10,
												mainHandDamage * 0.20);
								Entity target = player.getCombatDefinitions()
										.getCurrentTarget();
								if (target == null
										|| !Utils.isOnRange(
												(WorldTile) effect.args[0],
												target, 0, 3, target.getSize()))
									return;
								Hit hit = new Hit(
										entity,
										damage,
										effect.type == EffectType.DEATHS_SWIFTNESS ? HitLook.RANGE_DAMAGE
												: HitLook.MAGIC_DAMAGE);
								hit.setAbilityMark();
								target.applyHit(hit);
								PlayerCombat.autoRelatie(player, target);
							}
						}
					} else if (effect.type == EffectType.ICE_ASYLUM) {
						if (effect.cycle % 7 == 0) {
							List<Integer> playerIndexes = World.getRegion(
									player.getRegionId()).getPlayerIndexes();
							...
							for (int idx : playerIndexes) {
								...
								if (p2 == null
										...
										&& !p2.getAcceptAid().isAcceptingAid(
												AcceptAid.BENEFICIAL_BOOSTS,
												player) || distance >= 8)
									continue;
								double healPercentage = (8 - distance) * 0.01;
								healPercentage = healPercentage >= 0.07 ? 0.07
										: healPercentage;
								int healedAmount = (int) (healPercentage * p2
										.getMaxHitpoints()), healCapRemaining = (int) effect.args[1];
								if (healCapRemaining < healedAmount)
									healedAmount = healCapRemaining;
								effect.args[1] = healCapRemaining
										- healedAmount;
				...
				Entity target = ((PlayerCombat) a).getTarget();
				if (effect.type == EffectType.CONCENTRATED_BLAST) {
					if (effect.cycle == 9 || effect.cycle == 7
							|| effect.cycle == 5) {
						Projectile projectile = World.sendProjectileNew(player,
								target, (int) effect.args[0], 32, 42,
								effect.cycle == 9 ? 15 : 0, 5, 0, 10);
						PlayerCombat.delayHit(target,
								Utils.projectileTimeToCycles(projectile
										.getEndTime()) - 1,
								PlayerCombat.getHit(player, target, true,
										Combat.MAGIC_TYPE, 1.0,
										effect.cycle == 8 ? 0.75
												: effect.cycle == 4 ? 0.82
														: 0.87, false, true,
										false));
						target.setNextGraphics(new Graphics(
								(int) effect.args[1], projectile.getEndTime(),
								0));
					}
				} else if (effect.type == EffectType.ASPHYXIATE) {
					if (effect.cycle == 9 || effect.cycle == 7
							|| effect.cycle == 5 || effect.cycle == 3) {
						Projectile projectile = World.sendProjectileNew(player,
								target, (int) effect.args[0], 32, 42, 15, 5, 0,
								10);
						Hit hit = PlayerCombat.getHit(player, target, true,
								Combat.MAGIC_TYPE, 1.0, 1.88, false, true,
								false);
					...
					if (effect.cycle == 1) {
						Entity[] targets = (boolean) effect.args[2] ? PlayerCombat
								.getMultiAttackTargets(player, target)
								: new Entity[] { target };
						for (Entity t : targets) {
							Projectile projectile = World.sendProjectileNew(
									t == target ? player : target, t,
									(int) effect.args[0], 41, 39, 10, 5, 0, 95);
							Hit hit = PlayerCombat.getHit(player, t, true,
									Combat.RANGE_TYPE, 1.15,
									1.25 + Utils.random(0.94), false, true,
									false);
							PlayerCombat.delayHit(t, Utils
									.projectileTimeToCycles(projectile
											.getEndTime()) - 1, hit);
							t.setNextGraphics(new Graphics(
									(int) effect.args[1], projectile
											.getEndTime(), 0));
							if (hit.getDamage() > 0)
					...
					if (effect.cycle > 1) {
						Entity[] targets = (boolean) effect.args[3] ? PlayerCombat
								.getMultiAttackTargets(player, target)
								: new Entity[] { target };
						entity.setNextAnimation(new Animation(
								(int) effect.args[0]));
						for (Entity t : targets) {
							Projectile projectile = World.sendProjectileNew(
									t == target ? player : target, t,
									(int) effect.args[1], 41, 39,
									(int) effect.args[2], 8, 0, 0);
							Hit hit = PlayerCombat.getHit(player, t, true,
									Combat.RANGE_TYPE, 1.0, 0.94, false, true,
									false);
							if (effect.cycle == 9 && hit.getDamage() > 0)
					...
					if (effect.cycle == 8 || effect.cycle == 6
							|| effect.cycle == 4 || effect.cycle == 2) {
						Entity[] targets = (boolean) effect.args[2] ? PlayerCombat
								.getMultiAttackTargets(player, target)
								: new Entity[] { target };
						for (Entity t : targets) {
							Projectile projectile = World.sendProjectileNew(
									t == target ? player : target, t,
									(int) effect.args[0], 41, 39, 10, 5, 0, 95);
							for (Entity t2 : PlayerCombat
									.getMultiAttackTargets(player, t, 0, 9,
											true)) {
								Hit hit = PlayerCombat.getHit(player, t2, true,
										Combat.RANGE_TYPE, 1.0,
										1.50 + (double) effect.args[1], false,
										true, false);
								...
								PlayerCombat.delayHit(t2, Utils
										.projectileTimeToCycles(projectile
												.getEndTime()) - 1, hit);
							}
						}
					...
					if (effect.cycle > 5)
						effect.args[0] = new Integer(
								((Integer) effect.args[0]) + 1);
					else if (effect.cycle == 5)
						player.getHintIconsManager().addHintIcon(target, 3, -1,
								false);
					...
					if (effect.cycle == 8 || effect.cycle == 6
							|| effect.cycle == 4 || effect.cycle == 2) {
						for (Entity t : PlayerCombat.getMultiAttackTargets(
								player, target, 1, 9, true)) {
							if (!((boolean) effect.args[0]) && t != target)
								continue;
							Hit hit = PlayerCombat.getHit(player, t, true,
									Combat.MELEE_TYPE, 1.0, 0.94, false, true,
									false);
							if (t == target)
								effect.args[0] = hit.getDamage() > 0;
							PlayerCombat.delayHit(t, 0, hit);
						}
					}
				} else if (effect.type == EffectType.FURY) {
					if (effect.cycle == 6 || effect.cycle == 4
							|| effect.cycle == 1) {
						Hit hit = PlayerCombat.getHit(player, target, true,
								Combat.MELEE_TYPE, 1.0,
								0.75 + (double) effect.args[0], false, true,
								false);
						if (hit.getDamage() > 0) // 10% dmg increase each
													// successful hit
							effect.args[0] = (double) effect.args[0] + 0.10;
						PlayerCombat.delayHit(target, 0, hit);
					}
				} else if (effect.type == EffectType.ASSAULT) {
					if (effect.cycle == 7 || effect.cycle == 5
							|| effect.cycle == 3 || effect.cycle == 1) {
						Hit hit = PlayerCombat.getHit(player, target, true,
								Combat.MELEE_TYPE, 1.0, 2.19, false, true,
								false);
						PlayerCombat.delayHit(target, 0, hit);
					}
				} else if (effect.type == EffectType.DESTROY) {
					if (effect.cycle == 9 || effect.cycle == 8
							|| effect.cycle == 5 || effect.cycle == 4) {
						...
						Hit hit = PlayerCombat.getHit(player, target, true,
								Combat.MELEE_TYPE, 1.0, 1.88, false, true,
								false);
						PlayerCombat.delayHit(target, 0, hit);
					}
						...
						int angle = (int) Math
								.round(Math.toDegrees(Math.atan2(
										(player.getX() * 2 + player.getSize())
												- (target.getX() * 2 + target
														.getSize()),
										(player.getY() * 2 + player.getSize())
												- (target.getY() * 2 + target
														.getSize()))) / 45d) & 0x7;
						for (Entity t : PlayerCombat.getMultiAttackTargets(
								player, target, 1, 9, true)) {
							int nextAngle = (int) Math
									.round(Math.toDegrees(Math.atan2(
											(player.getX() * 2 + player
													.getSize())
													- (t.getX() * 2 + t
															.getSize()),
							...
							Hit hit = PlayerCombat.getHit(player, t, true,
									Combat.MELEE_TYPE, 1.0,
									1.10 + (double) effect.args[0], false,
									true, false);
				...
				HitLook look = HitLook.MELEE_DAMAGE;
				Graphics graphics = new Graphics(-1);
				if (effect.args.length > 0) {
					look = (HitLook) effect.args[0];
					if (effect.args.length > 1)
						graphics = (Graphics) effect.args[1];
				}
				...
				if (effect.cycle % effectDelay == 0) {
					...
					} else if (effect.type == EffectType.COMBUST
							|| effect.type == EffectType.FRAGMENTATION
							|| effect.type == EffectType.SLAUGHTER) {
						WorldTile tile = (WorldTile) effect.args[5];
						if (tile.getX() != entity.getX()
								|| tile.getY() != entity.getY()
								|| tile.getPlane() != entity.getPlane())
							damage *= (effect.type == EffectType.SLAUGHTER ? 3
									: 2);
					...
					if (look == HitLook.HEALED_DAMAGE)
						...
						if (hit.getSource() instanceof Player
								&& effect.args.length >= 5)
							PlayerCombat.autoRelatie((Player) effect.args[4],
									entity);
					}
					if (graphics != null)
	...
	public void refreshBuffEffect(Effect effect) {
		...
		if (type == EffectType.ADRENALINE_GAIN_DECREASE) {
			if (player.getVarsManager().sendVarBit(2794,
					effect.cycle > 0 ? 1 : 0))
				player.updateBuffs();
			...
			if (type.var != -1 || type.varbit != -1) {
				boolean isVar = type.var != -1
						&& player.getVarsManager().sendVar(
								type.var,
								type == EffectType.ANTIPOISON ? -effect.cycle
										: effect.cycle);
				boolean isVarbit = type.varbit != -1
						&& player.getVarsManager().sendVarBit(type.varbit,
								effects.contains(effect) ? 1 : 0);
		...
		if (refresh || effect.cycle % 25 == 0) {
			if (type.grMap != -1) {
				if (effect.type == EffectType.DEVOTION) {
					...
					player.getPackets().sendExecuteScript(9379, type.grMap, 0,
							effect.cycle);
				} else
					player.getPackets().sendExecuteScript(4252, type.grMap,
							effect.cycle);
			}
			player.updateBuffs();
		}
	...
	public boolean removeEffect(EffectType type) {
		...
		if (effect.type.action == COMBO_BUFFS && entity instanceof Player)
			PlayerCombat.setWeaponAbilityDelay((Player) entity);
	...
	public static byte BUFF = 0, DEBUFF = 1, HIT_MARK = 2, COMBO_BUFFS = 3,
			SHIELD_BUFF = 4;
	...
	public static enum EffectType {
		POISON(HIT_MARK, 722, -1, 14900) {
			...
			public boolean canStartEffect(Effect effect, Entity e) {
				Effect currentEffect = e.getEffectsManager().getEffectForType(
						this);
				if (currentEffect != null) {
		...
		VESTA_IMMUNITY(BUFF) {
			...
			public boolean canStartEffect(Effect effect, Entity e) {
				if (e instanceof Player)
					((Player) e)
							.getPackets()
							.sendEntityMessage(1, 0xFFFFFF, (e),
									"You are now immune to attacks from players for 10 seconds.");
				return true;
			}
			...
			public void onRemoval(Entity e) {
				if (e instanceof Player)
					((Player) e).getPackets().sendGameMessage(
							"Your resistance to attacks return to normal.");
			}
		},
		...
		MIRRORBACK_SPIDER(BUFF) {
			...
			@Override
			public void onRemoval(Entity e) {
				MirrorBackSpider spider = (MirrorBackSpider) e
						.getEffectsManager().getEffectForType(this).args[0];
				if (!spider.isDead())
		...
		CURSE_EFFECT(DEBUFF, -1, 2084, -1) {
			@Override
			public boolean canStartEffect(Effect effect, Entity e) {
				return !e.getEffectsManager().hasActiveEffect(
		...
		SUPREME_OVERLOAD(BUFF, -1, 500, 23129) {
			@Override
			public boolean canStartEffect(Effect effect, Entity e) {
				return !e.getEffectsManager().hasActiveEffect(OVERLOAD);
			}
		...
		OVERLOAD(BUFF, -1, 500, 23129) {
			@Override
			public boolean canStartEffect(Effect effect, Entity e) {
				return !e.getEffectsManager().hasActiveEffect(SUPREME_OVERLOAD);
			}
		...
		BOUND(DEBUFF, -1, 2057, 14884) {
			...
			public boolean canStartEffect(Effect effect, Entity e) {
				...
				e.getEffectsManager().startEffect(
						new Effect(EffectType.BOUND_IMMUNITY, effect.cycle
								+ (int) effect.args[1]));
				return true;
			}
		},
		...
		VIRUS(DEBUFF) {
			...
			public void onRemoval(Entity e) {
				...
				if (e instanceof Player)
					((Player) e).getPackets().sendGameMessage(
							"The smoke clouds around you dissapate");
			}
		},
		...
		BLOOD_SACRIFICE(DEBUFF) {
			...
			public boolean canStartEffect(Effect effect, Entity e) {
				...
				player.getPackets().sendGameMessage(
						"<col=480000>Nex has marked you as a sacrifice, RUN!");
				player.setNextColour(new Colour(50, 325 - 50, 220, 163, 163,
						220));
				return true;
			}
			...
			public void onRemoval(Entity e) {
				...
				if (currentEffect != null) {
					...
					if (Utils.isOnRange(nex, player, 3)) {
						...
						nex.heal(player.getMaxHitpoints());

						player.getPackets()
								.sendGameMessage(
										"You didn't make it far enough in time - Nex fires a punishing attack!");
						player.applyHit(new Hit(nex, (int) (player
								.getMaxHitpoints() * 0.1),
		...
		DETONATE(COMBO_BUFFS, -1, 2080, -1) {
			...
			public void onRemoval(Entity e) {
				Effect currentEffect = e.getEffectsManager().getEffectForType(
						this);
				if (currentEffect != null && currentEffect.args == null
						|| currentEffect == null) // Just
													// reset
													// the
													// animation.
		...
		BINDING_SHOT(DEBUFF, -1, 2088, -1) {
			...
			public boolean canStartEffect(Effect effect, Entity e) {
				...
				e.getEffectsManager()
						.startEffect(
								new Effect(EffectType.BOUND_IMMUNITY,
										effect.cycle + 5));
				return true;
			}
		},
		...
		SUPER_FIRE_IMMUNITY(BUFF, -1, 498, 14904) {
			...
			public boolean canStartEffect(Effect effect, Entity e) {
				...
				if (currentEffect != null)
					e.getEffectsManager()
							.removeEffect(EffectType.FIRE_IMMUNITY);
				return true;
			}
			...
			public void onRemoval(Entity e) {
				if (e instanceof Player)
					((Player) e)
							.getPackets()
							.sendGameMessage(
									"<col=480000>Your resistance to dragonfire has run out.</col>");
			}
		},
		...
		METAMORPHISIS(BUFF, -1, 2081, -1) {
			...
			public boolean canStartEffect(Effect effect, Entity e) {
				...
				if (type == Combat.TYPE_AIR || type == Combat.TYPE_WATER)
					player.setNextGraphics(new Graphics(
							type == Combat.TYPE_WATER ? 3547 : 3563));
				else if (type == Combat.TYPE_EARTH || type == Combat.TYPE_FIRE) {
					player.setNextGraphics(new Graphics(
							type == Combat.TYPE_FIRE ? 3550 : 3548));
					player.setNextAnimation(new Animation(
							type == Combat.TYPE_EARTH ? 18406 : 18418));
				}
				return true;
			}
		...
		SUNSHINE(BUFF, -1, 18139, -1) {
			...
			public boolean canStartEffect(Effect effect, Entity e) {
				...
				World.sendGraphics(e, new Graphics(3856, 0, 10, 0, true),
						(WorldTile) effect.args[0]);
				return true;
			}
		},
		...
		DEATHS_SWIFTNESS(BUFF, -1, 18140, -1) {
			...
			public boolean canStartEffect(Effect effect, Entity e) {
				...
				World.sendGraphics(e, new Graphics(3869, 0, 10),
						(WorldTile) effect.args[0]);
				return true;
			}
		},
				...
				if (e instanceof Player) {
					...
					for (int skill = 0; skill < 25; skill++) {
						...
						player.getSkills().set(skill,
								player.getSkills().getLevelForXp(skill));
					}
				}
				return true;
			}
		...
		NATURAL_INSTINCT(BUFF, -1, 18141, -1) {
			...
			public boolean canStartEffect(Effect effect, Entity e) {
				...
				if (target instanceof Player) {
					...
					int adrenaline = playerTarget.getCombatDefinitions()
							.getSpecialAttackPercentage();
					playerTarget.getCombatDefinitions()
							.desecreaseSpecialAttack(adrenaline);
					if (e instanceof Player)
						((Player) e).getCombatDefinitions()
								.increaseSpecialAttack(adrenaline);
				}
		...
		ICE_ASYLUM(BUFF, -1, 22462, -1) {
			...
			public boolean canStartEffect(Effect effect, Entity e) {
				...
				World.sendGraphics(e, new Graphics(4668, 0, 10),
						(WorldTile) effect.args[0]);
				return true;
			}
		},
	...
	public static void makePoisoned(Entity e, int damage, int cycles) {
		e.getEffectsManager().startEffect(
				new Effect(EffectType.POISON,
						cycles == -1 ? ((damage * 5) - 40) / 10 : cycles,
						HitLook.POISON_DAMAGE, null, damage, 17));
	}

	public static void startHealEffect(Entity e, int damage) {
		e.getEffectsManager().startEffect(
				new Effect(EffectType.HEAL, 60, HitLook.HEALED_DAMAGE,
						new Graphics(93, 0, 0, 0, true), damage, 6));
	}

	public static void startBleedEffect(Entity e, int cycles, int delay,
			int damage) {
		e.getEffectsManager().startEffect(
				new Effect(EffectType.BLEED, cycles, HitLook.REGULAR_DAMAGE,
						null, damage, delay));
	}
	...
	public static boolean healPoison(Entity entity) {
		...
		if (!player.getEffectsManager().hasActiveEffect(EffectType.POISON)) {
			player.getPackets().sendGameMessage(
					"You're not currently in need of curing.");
			return false;
		}
		for (int i = 0; i < 28; i++) {
			Item item = player.getInventory().getItem(i);
			if (item == null
					|| (!Drinkables.Drink.ANTIPOISON_POTION.contains(item
							.getId())
							&& !Drinkables.Drink.SUPER_ANTIPOISON.contains(item
									.getId())
							&& !Drinkables.Drink.ANTIPOISON_FLASK.contains(item
									.getId()) && !Drinkables.Drink.SUPER_ANTIPOISON_FLASK
								.contains(item.getId())))
				continue;
		...
		player.getPackets().sendGameMessage(
				"You don't have anything to cure the poison.");
		return false;
	}

	public static void addBoundEffect(Entity entity, int delay, boolean freeze,
			int immunityDelay) {
		entity.getEffectsManager().startEffect(
				new Effect(EffectType.BOUND, delay, freeze, immunityDelay));
	}
