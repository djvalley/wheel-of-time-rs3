...
public class AnimationDefinitions {
	...
	private void readValues(InputStream buffer, int opcode) {
		if (opcode == 1) {
			...
			int i_7_ = opcode == 12 ? buffer.readUnsignedByte() : buffer
					.readUnsignedShort();
			...
			if (opcode == 19 || opcode == 119) {
				...
				anIntArray1362[opcode == 19 ? buffer.readUnsignedByte()
						: buffer.readUnsignedShort()] = buffer
						.readUnsignedByte();
				...
				int slot = opcode == 20 ? buffer.readUnsignedByte() : buffer
						.readUnsignedShort();
