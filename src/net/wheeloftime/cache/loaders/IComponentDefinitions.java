...
public class IComponentDefinitions {
	...
	public static void main(String[] args) throws IOException {
		...
		IComponentDefinitions[] defs = getInterface(1038);
	...
	final void decode(InputStream stream) {
		...
		if (type == 6) {
			...
			aBoolean4707 = (i_19_ & 0x4) == 4;
			aBoolean4727 = (i_19_ & 0x8) == 8;
			if (bool) {
		...
		if (type == 4) {
			...
			/*
			 * else { anInt4759 = stream.readUnsignedShort(); if ((anInt4759 ^
			 * 0xffffffff) == -65536) { anInt4759 = -1; } }
			 */
			if ((newInt ^ 0xffffffff) <= -3) {
				aBoolean4832 = (stream.readUnsignedByte() ^ 0xffffffff) == -2;
			}
			aString4790 = stream.readString();
			System.err.println("text [" + (ihash >> 16) + "," + (ihash & 0xFFF)
					+ "]:" + aString4790);
		...
		varps = method4150(stream);
		anIntArray4833 = method4150(stream);
		anIntArray4789 = method4150(stream);
		varcs = method4150(stream);
		varcstrs = method4150(stream);
	}
	...
	private final Object[] decodeScript(InputStream buffer) {
		...
		hasScripts = true;
		System.err.println("Call found[" + (ihash >> 16) + ","
				+ (ihash & 0xFFF) + "]:" + Arrays.toString(objects));
		return objects;
	}
	...
	private final int[] method4150(InputStream buffer) {
		...
		System.err.println("Trigger found:" + Arrays.toString(is));
		return is;
	}
