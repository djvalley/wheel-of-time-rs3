package net.wheeloftime.cache.loaders;
...
import net.wheeloftime.cache.filestore.store.Store;
...
public final class IndexedColorImageFile {
	...
	public void generatePallete() {
		...
		for (int index = 0; index < images.length; index++) {
			...
			for (int pixel = 0; pixel < pixelsIndexes[index].length; pixel++) {
				...
				int medintrgb = new Color(c.getRed(), c.getGreen(), c.getBlue())
						.getRGB();// convertToMediumInt(rgb);
	...
	private static class DetailFilter extends RGBImageFilter {
		...
		public boolean done() {
			...
			// System.out.println(diffR+", "+diffG+", "+diffB+", "+colours.size()+", "+increase);
			if (colours.size() <= 256)
				return true;
			colours.clear();
		...
		public int filterRGB(int x, int y, int rgb) {
			Color c = new Color(rgb, true);
			rgb = c.getRGB();
			int blue = c.getBlue() / diffB * diffB;
			int green = c.getGreen() / diffG * diffG;
			int red = c.getRed() / diffR * diffR;
