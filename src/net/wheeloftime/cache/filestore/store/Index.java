...
import net.wheeloftime.cache.filestore.io.InputStream;
import net.wheeloftime.cache.filestore.io.OutputStream;
import net.wheeloftime.cache.filestore.util.crc32.CRC32HGenerator;
import net.wheeloftime.cache.filestore.util.whirlpool.Whirlpool;
import net.wheeloftime.cache.filestore.utils.Constants;
import net.wheeloftime.cache.filestore.utils.Utils;
...
public final class Index {
	...
	protected Index(MainFile index255, MainFile mainFile, int[] keys) {
		...
		if (archive.getData() == null) {
			Logger.log(this, "The data in the archive is null.");
			return;
		}
	...
	public boolean putArchive(int archiveId, Store originalStore,
		...
		return putArchive(getId(), archiveId, originalStore, rewriteTable,
				resetCache);
	}
	...
	public boolean putArchive(int id, int archiveId, Store originalStore,
		...
		try {
			...
			ArchiveReference originalReference = originalIndex.table
	...
	public boolean encryptArchive(int archiveId, int[] oldKeys, int[] keys,
			boolean rewriteTable, boolean resetCache) {
		if (!archiveExists(archiveId))
			return false;
