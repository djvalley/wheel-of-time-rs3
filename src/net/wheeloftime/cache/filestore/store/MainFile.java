...
package net.wheeloftime.cache.filestore.store;
...
public final class MainFile {
	...
	private static final ByteBuffer tempBuffer = ByteBuffer
			.allocateDirect(TOTAL_BLOCK_LEN);
	...
	// newProtocol
	protected MainFile(int id, RandomAccessFile data, RandomAccessFile index,
			boolean newProtocol) throws IOException {
		this.id = id;
	...
	public byte[] getArchiveData(int archiveId) {
		synchronized (data) {
			try {
				...
				index.read(tempBuffer,
						((long) archiveId * (long) IDX_BLOCK_LEN));
				tempBuffer.flip();
				...
				int remaining = size;
				int chunk = 0;
				int blockLen = !newProtocol || archiveId <= 0xffff ? BLOCK_LEN
						: EXPANDED_BLOCK_LEN;
				int headerLen = !newProtocol || archiveId <= 0xffff ? HEADER_LEN
						: EXPANDED_HEADER_LEN;
				while (remaining > 0) {
					if (block == 0) {
					...
					data.read(tempBuffer,
							((long) block * (long) TOTAL_BLOCK_LEN));
					tempBuffer.flip();
					...
					if ((archiveId != currentFile && archiveId <= 65535)
							|| chunk != currentChunk || id != currentIndex) {
						return null;
					}
					if (nextBlock < 0
							|| nextBlock > data.size() / TOTAL_BLOCK_LEN) {
						return null;
					}
	...
	public boolean putArchiveData(int archiveId, ByteBuffer archive, int size,
			boolean exists) {
		synchronized (data) {
			try {
				int block;
				if (exists) {
					if (archiveId * IDX_BLOCK_LEN + IDX_BLOCK_LEN > index
							.size()) {
						return false;
					}
					...
					block = (int) (data.size() + TOTAL_BLOCK_LEN - 1)
							/ TOTAL_BLOCK_LEN;
					if (block == 0) {
						block = 1;
					}
