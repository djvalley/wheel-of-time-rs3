		...
		try {
			...
			for (int linesCount = 0; linesCount < page.getLines().size(); linesCount++) {
				...
				if (line.equalsIgnoreCase(COMBAT_LEVEL_LINE)) {
					linesCount++;
					String combatLevelsLine = getFormatedString(page.getLines()
							.get(linesCount));
					...
					linesCount++;
					String isAggresive = getFormatedString(page.getLines().get(
							linesCount));
					...
					linesCount++;
					String attackStyleLine = page.getLines().get(linesCount)
							.toLowerCase();
					...
					if (attackSpeedLine.contains("Unknown"))
						...
						String speed = attackSpeedLine.substring(
